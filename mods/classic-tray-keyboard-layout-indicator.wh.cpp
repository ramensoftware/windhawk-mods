...
// Уникальный GUID для иконки — предотвращает дубликаты
...
void RemoveIcon() {
    // Удаляем по GUID — работает даже если hWnd изменился
    NOTIFYICONDATAW nid = {};
...
void UpdateIcon(HKL hkl) {
    ...
    // Не получилось изменить — удалить и добавить заново
...
DWORD WINAPI TrayThread(LPVOID) {
    ...
    // Удаляем старую иконку (если осталась от предыдущего запуска)
    RemoveIcon();
...
BOOL Wh_ModInit() {
    ...
    // Удаляем старую иконку до запуска потока
    NOTIFYICONDATAW nid = {};
    nid.cbSize = sizeof(nid);
    nid.uFlags = NIF_GUID;
    nid.guidItem = g_iconGuid;
...
void Wh_ModUninit() {
    ...
    // Гарантированно удаляем иконку по GUID
    NOTIFYICONDATAW nid = {};
    nid.cbSize = sizeof(nid);
    nid.uFlags = NIF_GUID;
    nid.guidItem = g_iconGuid;
