...
#error "Unsupported architecture"
                               ...
                               DividerGeometry *geometry)
    {
        ...
        else
        {
            winrt::Windows::Foundation::Rect previousBounds{};
            if (!previousIcon ||
                !TryGetElementBounds(overlayCanvas, previousIcon,
                                     &previousBounds))
            {
                return false;
            }
        ...
        for (size_t index = 1; index < icons.size(); index++)
        {
            ...
            if (!icons[index - 1] || !icons[index] ||
                !TryGetElementBounds(overlayCanvas, icons[index - 1],
                                     &previousBounds) ||
                !TryGetElementBounds(overlayCanvas, icons[index],
                                     &currentBounds))
            {
                continue;
            }
    ...
    activeSeparators.push_back(
        {resolvedSeparator,
         ...
         false});
                ...
                bool hasGapContributions = std::any_of(
                    gapContributions.begin(), gapContributions.end(),
