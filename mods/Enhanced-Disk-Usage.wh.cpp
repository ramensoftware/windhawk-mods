...
ARGB g_barNormalStart, g_barNormalEnd, g_barFullStart, g_barFullEnd, g_trackColor, g_borderColor;
...
typedef HRESULT (WINAPI* GetThemeClassList_t)(HTHEME hTheme, LPWSTR pszClassList, int cchClassList);
GetThemeClassList_t GetThemeClassList_Ptr;

// --- Helpers ---
static ARGB ParseHexARGB(PCWSTR hex, ARGB fallback) {
    if (!hex || wcslen(hex) < 1) return fallback;
    std::wstring s(hex); if (s[0] == L'#') s = s.substr(1);
    try { 
        unsigned long val = std::stoul(s, nullptr, 16);
        if (s.length() == 6) val |= 0xFF000000;
        return (ARGB)val; 
...
void LoadSettings() {
    ...
    s = Wh_GetStringSetting(L"formatString"); g_formatString = s ? s : L"%s free | %s used of %s"; Wh_FreeStringSetting(s);
    g_boldUsed = Wh_GetIntSetting(L"boldUsed") != 0;
    g_removeSpace = Wh_GetIntSetting(L"removeSpace") != 0;
    s = Wh_GetStringSetting(L"boldStyle");
    g_boldStyle = (s && wcscmp(s, L"serif") == 0) ? BoldStyle::Serif : BoldStyle::SansSerif;
    Wh_FreeStringSetting(s);
...
std::wstring CleanNumericString(const std::wstring& s) {
    std::wstring result; bool start = false;
    for (wchar_t c : s) { 
        wchar_t check = (c == 0xA0) ? L' ' : c;
        if (!start) { 
            if (iswdigit(check) || check == L'.' || check == L',' || check == L'-') { 
                start = true; result += check; 
            } 
            continue; 
        } 
        if (check != L' ') result += check; 
    }
    return result;
}
...
std::wstring MakeBoldText(const std::wstring& s) {
    std::wstring res;
    for (wchar_t c : s) {
        if (c >= L'0' && c <= L'9') { res += (wchar_t)0xD835; res += (wchar_t)((g_boldStyle == BoldStyle::Serif ? 0xDFCE : 0xDFEC) + (c - L'0')); 
                                    }
        else if (c >= L'A' && c <= L'Z') { 
            res += (wchar_t)0xD835; res += (wchar_t)((g_boldStyle == BoldStyle::Serif ? 0xDC00 : 0xDDD4) + (c - L'A')); 
                                         }
        else if (c >= L'a' && c <= L'z') { 
            res += (wchar_t)0xD835; res += (wchar_t)((g_boldStyle == BoldStyle::Serif ? 0xDC1A : 0xDDEE) + (c - L'a')); 
                                         }
        else res += c;
    }
    return res;
...
static void PaintEnhancedBar(HDC hdc, LPCRECT pRect, int iStateId, bool isFill) {
    Graphics graphics{
        hdc
    };
    ...
    RectF rect{
        (REAL)pRect->left, (REAL)pRect->top, (REAL)(pRect->right - pRect->left), (REAL)(pRect->bottom - pRect->top)
    };
...
bool FindSpaceStats(const std::wstring& t, std::wstring& f, std::wstring& tot) {
    ...
    const wchar_t* units[] = {
        L" bytes", L" KB", L" MB", L" GB", L" TB", L" B"
    };
...
int WINAPI DrawTextW_Hook(HDC hdc, LPCWSTR psz, int cch, LPRECT prc, UINT fmt) {
    if (!psz || !prc) return DrawTextW_Orig(hdc, psz, cch, prc, fmt);
    ...
    if (FindSpaceStats(t, fs, ts)) {
        ...
        double fv, tv; wchar_t fu[16], tu[16];
        
        if (swscanf(cf.c_str(), L"%lf %15s", &fv, fu) == 2 && swscanf(ct.c_str(), L"%lf %15s", &tv, tu) == 2) {
            std::wstring us = std::wstring(StrFormatByteSizeW((ULONGLONG)std::max(0.0, (tv * GetUnitMultiplier(tu) - fv * GetUnitMultiplier(fu))), fu, 16));
            
            if (g_removeSpace) us.erase(std::remove(us.begin(), us.end(), L' '), us.end());
            std::wstring fS = g_formatString; 
            size_t p1 = fS.find(L"%s"), p2 = fS.find(L"%s", p1+2), p3 = fS.find(L"%s", p2+2);
            std::wstring s1 = fS.substr(0, p1) + fs + fS.substr(p1+2, p2-p1-2), s2 = g_boldUsed ? MakeBoldText(us) : us, s3 = fS.substr(p2+2, p3-p2-2) + ts + fS.substr(p3+2);
            
            if (fmt & DT_CALCRECT) { 
                std::wstring ft = s1 + s2 + s3; 
                return DrawTextW_Orig(hdc, ft.c_str(), 
                                      (int)ft.length(), prc, fmt);
            }
            UINT dF = (fmt & ~(DT_END_ELLIPSIS | DT_PATH_ELLIPSIS | DT_WORD_ELLIPSIS)) | DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_TOP;
            SIZE sz1, sz2, sz3; GetTextExtentPoint32W(hdc, s1.c_str(), (int)s1.length(), &sz1); 
            GetTextExtentPoint32W(hdc, s2.c_str(), (int)s2.length(), &sz2); 
            GetTextExtentPoint32W(hdc, s3.c_str(), (int)s3.length(), &sz3);
            int x = prc->left; 
            if (fmt & DT_CENTER) x += (prc->right - prc->left - (sz1.cx + sz2.cx + sz3.cx)) / 2; 
            else if (fmt & DT_RIGHT) x = prc->right - (sz1.cx + sz2.cx + sz3.cx);
            RECT r = *prc; r.top += g_lineYOffset; 
            r.bottom += g_lineYOffset; r.left = x; 
            r.right = x + sz1.cx; 
...
int WINAPI DrawTextExW_Hook(HDC hdc, LPWSTR psz, int cch, LPRECT prc, UINT fmt, LPDRAWTEXTPARAMS pDtp) {
    if (!psz || !prc) return DrawTextExW_Orig(hdc, psz, cch, prc, fmt, pDtp);
    std::wstring t(psz, cch == -1 ? wcslen(psz) : cch); std::wstring fs, ts;
    ...
    if (FindSpaceStats(t, fs, ts)) {
        ...
        if (swscanf(cf.c_str(), L"%lf %15s", &fv, fu) == 2 && swscanf(ct.c_str(), L"%lf %15s", &tv, tu) == 2) {
            ...
            if (g_removeSpace) us.erase(std::remove(us.begin(), us.end(), L' '), us.end());
            std::wstring fS = g_formatString; size_t p1 = fS.find(L"%s"), p2 = fS.find(L"%s", p1+2), p3 = fS.find(L"%s", p2+2);
            std::wstring s1 = fS.substr(0, p1) + fs + fS.substr(p1+2, p2-p1-2), s2 = g_boldUsed ? MakeBoldText(us) : us, s3 = fS.substr(p2+2, p3-p2-2) + ts + fS.substr(p3+2);
            ...
            if (fmt & DT_CALCRECT) { 
                ...
                b.push_back(0); 
                return DrawTextExW_Orig(hdc, b.data(), (int)ft.length(), prc, fmt, pDtp); 
            }
            ...
            b1.push_back(0); 
            DrawTextExW_Orig(hdc, b1.data(), (int)s1.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
            r.left = r.right; r.right = r.left + sz2.cx; r.top += g_boldYOffset; r.bottom += g_boldYOffset;
            std::vector<wchar_t> b2(s2.begin(), s2.end()); 
            b2.push_back(0); DrawTextExW_Orig(hdc, b2.data(), (int)s2.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
            r.left = r.right; 
            r.right = r.left + sz3.cx; 
            r.top -= g_boldYOffset; 
            r.bottom -= g_boldYOffset;
            ...
            b3.push_back(0); 
            return DrawTextExW_Orig(hdc, b3.data(), (int)s3.length(), &r, fmt | DT_NOPREFIX | DT_LEFT, pDtp);
        }
    }
...
BOOL Wh_ModInit() {
    ...
    if (uxtheme) {
        GetThemeWindow_Ptr = (GetThemeWindow_t)GetProcAddress(uxtheme, "GetThemeWindow");
        GetThemeClassList_Ptr = (GetThemeClassList_t)GetProcAddress(uxtheme, "GetThemeClassList");
        Wh_SetFunctionHook((void*)GetProcAddress(uxtheme, "DrawThemeBackground"), (void*)HookedDrawThemeBackground, (void**)&DrawThemeBackground_Orig);
    }
    Wh_SetFunctionHook((void*)GetProcAddress(GetModuleHandle(L"user32.dll"), "DrawTextW"), (void*)DrawTextW_Hook, (void**)&DrawTextW_Orig);
    Wh_SetFunctionHook((void*)GetProcAddress(GetModuleHandle(L"user32.dll"), "DrawTextExW"), (void*)DrawTextExW_Hook, (void**)&DrawTextExW_Orig);
    return TRUE;
}
void Wh_ModUninit() { 
