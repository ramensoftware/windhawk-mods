// ==WindhawkMod==
// @id              advanced-explorer-command-bar
// @name            Advanced Explorer Command Bar
// @description     Adds right-aligned custom command bar buttons with independent labels, shell triggers, and multi-file %sel% batch execution
// @version         1.0
// @author          ArvindSaini978
// @github          https://github.com/arvindsaini978
// @homepage        https://github.com/arvindsaini978
...
// ==WindhawkModReadme==
/*
# Advanced Explorer Command Bar

An extended version of the Windows 11 File Explorer command bar mod that introduces independent UI labeling and global batch-selection execution for custom actions.

### What This Mod Does

* **Right-Aligned Standalone Layout:** Buttons are anchored to the right side of the File Explorer command bar, operating as a clean, independent action deck.
* **Independent Button Labels:** Configure a visible button label (`labelText`) separate from the hover tooltip (`name`). Labels display alongside icons matching the native Windows 11 command bar style, or can be collapsed to icon-only per button.
* **Global Multi-File `%sel%` Processing:** Bypasses Explorer's single-item argument limit. Using `"%sel%"` in command parameters now enumerates every highlighted item via `IFolderView2` and passes all file paths in quotes simultaneously, enabling seamless bulk batch processing for external tools (e.g., PowerToys PowerRename, batch converters, scripts).
* **Native Command Execution:** Adds built-in shell triggers including native **Preview Pane** (`internal:TogglePreview`) and **Details pane** (`internal:ToggleDetails`) alongside custom application launchers.
* **Direct Icon Resolution:** Supports custom icon glyphs (Segoe Fluent Icons), direct `.ico` file paths, and DLL resource extractions (`.dll,index`).

---
*Based on the original [Explorer Command Bar](https://windhawk.net/mods/explorer-command-bar) mod by DanRotaru.*

![Advanced Explorer Command Bar Preview](https://raw.githubusercontent.com/ArvindSaini978/windhawk-mods/assets/Preview.png)
...
- items:
  - - type: button
    ...
    - showLabel: false
      $name: Show label
      $description: Show the text label next to the icon.
    - labelText: ""
      $name: Custom Label Text
      $description: The text to show next to the icon. If empty, the Name (tooltip) is used.
...
void OnActionInvoked(mux::FrameworkElement const& elementForWindow,
    ...
    // Default behavior for normal commands - Upgraded for Multi-Select!
    RunShellWorkOnWorkerThread([hWnd, item]() {
        ActionItem modifiedItem = item;

        if (modifiedItem.parameters.find(L"%sel%") != std::wstring::npos) {
            std::wstring multiSel;
            ExplorerContext context = GetExplorerContext(hWnd);
            if (context.shellView) {
                IFolderView2* folderView = nullptr;
                if (SUCCEEDED(context.shellView->QueryInterface(IID_PPV_ARGS(&folderView)))) {
                    IShellItemArray* selection = nullptr;
                    if (SUCCEEDED(folderView->GetSelection(TRUE, &selection))) {
                        DWORD count = 0;
                        selection->GetCount(&count);
                        for (DWORD i = 0; i < count; i++) {
                            IShellItem* shellItem = nullptr;
                            if (SUCCEEDED(selection->GetItemAt(i, &shellItem))) {
                                PWSTR path = nullptr;
            ...
            if (!multiSel.empty()) multiSel.pop_back();

            // Safely replace "%sel%" or %sel% with the massive list of quoted paths
            size_t pos = modifiedItem.parameters.find(L"\"%sel%\"");
            if (pos != std::wstring::npos) {
                modifiedItem.parameters.replace(pos, 7, multiSel);
            } else {
                pos = modifiedItem.parameters.find(L"%sel%");
                if (pos != std::wstring::npos) {
                    modifiedItem.parameters.replace(pos, 5, multiSel);
                }
            }
        }

        LaunchItemForWindow(hWnd, modifiedItem);
    });
}
...
muxc::AppBarButton CreateBareButton(int index,
    ...
    // Use custom label if provided, otherwise fallback to tooltip
...
muxc::AppBarButton CreateActionButton(ActionItem const& item, int index) {
    muxc::AppBarButton button =
    CreateBareButton(index, item.name, item.labelText, MakeCommandButtonIcon(item), item.showLabel);
...
void UpdateCommandBar(muxc::CommandBar const& commandBar) {
    ...
    if (commandBar.Name() == L"FileExplorerCommandBar") {
        ...
        // Force the secondary bar to show labels next to the icons
