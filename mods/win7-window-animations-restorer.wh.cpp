...
class ScopedScreenDc { public: explicit ScopedScreenDc(const RECT& rc) noexcept { const RECT prim{0,0,LONG(GetSystemMetrics(SM_CXSCREEN)),LONG(GetSystemMetrics(SM_CYSCREEN))}; if(rc.left>=prim.left&&rc.top>=prim.top&&rc.right<=prim.right&&rc.bottom<=prim.bottom){ m_dc=GetDC(nullptr); return; } m_dc=CreateDCW(L"DISPLAY",nullptr,nullptr,nullptr); if(m_dc) m_owned=true; else m_dc=GetDC(nullptr); } ~ScopedScreenDc(){ if(!m_dc) return; if(m_owned) DeleteDC(m_dc); else ReleaseDC(nullptr,m_dc); } HDC get() const noexcept { return m_dc; } explicit operator bool() const noexcept { return m_dc!=nullptr; } ScopedScreenDc(const ScopedScreenDc&)=delete; ScopedScreenDc& operator=(const ScopedScreenDc&)=delete; private: HDC m_dc=nullptr; bool m_owned=false; };
class ScopedHandle { public: ScopedHandle()=default; explicit ScopedHandle(HANDLE h) noexcept : m_h(h){} ~ScopedHandle(){reset();} ScopedHandle(const ScopedHandle&)=delete; ScopedHandle& operator=(const ScopedHandle&)=delete; void reset(HANDLE h=nullptr) noexcept { if(m_h&&m_h!=INVALID_HANDLE_VALUE) CloseHandle(m_h); m_h=h; } HANDLE release() noexcept { HANDLE t=m_h; m_h=nullptr; return t; } HANDLE get() const noexcept { return m_h; } explicit operator bool() const noexcept { return m_h&&m_h!=INVALID_HANDLE_VALUE; } private: HANDLE m_h=nullptr; };
class ScopedProp { public: ScopedProp(HWND hwnd,LPCWSTR name) noexcept : m_hwnd(hwnd),m_name(name){ m_set=SetPropW(hwnd,name,HANDLE(1))!=FALSE; } ~ScopedProp(){ if(m_set&&m_hwnd&&IsWindow(m_hwnd)) RemovePropW(m_hwnd,m_name); } bool ok() const noexcept { return m_set; } ScopedProp(const ScopedProp&)=delete; ScopedProp& operator=(const ScopedProp&)=delete; private: HWND m_hwnd; LPCWSTR m_name; bool m_set=false; };
class ScopedThreadpoolWork { public: explicit ScopedThreadpoolWork(PTP_WORK w) noexcept : m_w(w){} ~ScopedThreadpoolWork(){ if(m_w){ WaitForThreadpoolWorkCallbacks(m_w,FALSE); CloseThreadpoolWork(m_w); } } PTP_WORK get() const noexcept { return m_w; } explicit operator bool() const noexcept { return m_w!=nullptr; } ScopedThreadpoolWork(const ScopedThreadpoolWork&)=delete; ScopedThreadpoolWork& operator=(const ScopedThreadpoolWork&)=delete; private: PTP_WORK m_w; };
template<class F> class ScopedExit { public: explicit ScopedExit(F f) noexcept : m_f(f){} ~ScopedExit(){ if(m_armed) m_f(); } void Dismiss() noexcept { m_armed=false; } ScopedExit(const ScopedExit&)=delete; ScopedExit& operator=(const ScopedExit&)=delete; private: F m_f; bool m_armed=true; };
...
enum class AnimationType { None=0, Close, Minimize, RestoreFromMinimized };
...
bool g_animateMinimize=true; bool g_animateClose=true;
static void LoadSettings(){ g_animateMinimize=Wh_GetIntSetting(L"animateMinimize")!=0; g_animateClose=Wh_GetIntSetting(L"animateClose")!=0; }
...
static size_t CaptureBytes(const CaptureBits& b){ return b.pixels.size()*sizeof(uint32_t); }
static void EvictLocked(){
    while(!g_captureLru.empty()&&(g_captureCache.size()>kMaxCachedCaptures||g_cacheBytes>kMaxCachedBytes)){
        HWND v=g_captureLru.back(); g_captureLru.pop_back();
        auto it=g_captureCache.find(v);
        if(it!=g_captureCache.end()){ g_cacheBytes-=std::min(g_cacheBytes,CaptureBytes(it->second.bits)); g_captureCache.erase(it); }
    }
}
static void CacheCapture(HWND hwnd,CaptureBits&& bits){
    if(!hwnd||bits.empty()) return;
    if(CaptureBytes(bits)>kMaxCachedBytes) return;
    try{ std::lock_guard<std::mutex> lock(g_cacheMutex); auto it=g_captureCache.find(hwnd);
        if(it!=g_captureCache.end()){ g_cacheBytes-=std::min(g_cacheBytes,CaptureBytes(it->second.bits)); it->second.bits=std::move(bits); g_cacheBytes+=CaptureBytes(it->second.bits); g_captureLru.splice(g_captureLru.begin(),g_captureLru,it->second.lruIt); EvictLocked(); return; }
        g_captureLru.push_front(hwnd); g_cacheBytes+=CaptureBytes(bits); g_captureCache.emplace(hwnd,CacheEntry{std::move(bits),g_captureLru.begin()});
        EvictLocked();
    }catch(...){}
}
...
static void ForgetCapture(HWND hwnd){ try{ std::lock_guard<std::mutex> lock(g_cacheMutex); auto it=g_captureCache.find(hwnd); if(it!=g_captureCache.end()){ g_cacheBytes-=std::min(g_cacheBytes,CaptureBytes(it->second.bits)); g_captureLru.erase(it->second.lruIt); g_captureCache.erase(it);} }catch(...){} }
...
static bool GetFrameBoundsPhysical(HWND hwnd, RECT* rc){
    ...
    if(SUCCEEDED(hr)&&IsRectUsable(ext)&&RECTW(ext)>=8&&RECTH(ext)>=8
        &&RECTW(ext)<=kMaxCaptureSide&&RECTH(ext)<=kMaxCaptureSide){ *rc=ext; return true; }
...
static bool CaptureWindowForClose(HWND hwnd, CaptureBits& out, bool composedOnly){
    ...
    if(composedOnly){ out={}; return false; }
    {
        BITMAPINFO bmi{}; bmi.bmiHeader.biSize=sizeof(BITMAPINFOHEADER); bmi.bmiHeader.biWidth=w; bmi.bmiHeader.biHeight=-h; bmi.bmiHeader.biPlanes=1; bmi.bmiHeader.biBitCount=32; bmi.bmiHeader.biCompression=BI_RGB;
        void* bits=nullptr; ScopedScreenDc screenDc(rc); if(!screenDc) return false;
...
static bool GetTaskbarTargetPhysical(HWND hwnd,RECT* rc){
    ...
    HWND best=nullptr; RECT bestRc{};
    for(const wchar_t* cls : {L"Shell_TrayWnd",L"Shell_SecondaryTrayWnd"}){
        for(HWND tb=FindWindowExW(nullptr,nullptr,cls,nullptr); tb; tb=FindWindowExW(nullptr,tb,cls,nullptr)){
            RECT r{}; if(!GetWindowRect(tb,&r)||!IsRectUsable(r)) continue;
            if(!best){ best=tb; bestRc=r; }
            if(MonitorFromWindow(tb,MONITOR_DEFAULTTONEAREST)==target){ best=tb; bestRc=r; break; }
        }
    }
    ...
    if(!list){
        ...
        HWND sw=rebar?FindWindowExW(rebar,nullptr,L"MSTaskSwWClass",nullptr):nullptr;
        if(sw) list=FindWindowExW(sw,nullptr,L"MSTaskListWClass",nullptr);
    }
    RECT area=bestRc;
    if(list){ RECT lr{}; if(GetWindowRect(list,&lr)&&IsRectUsable(lr)) area=lr; }
    const LONG cx=(area.left+area.right)/2, cy=(area.top+area.bottom)/2;
    rc->left=cx-12; rc->top=cy-12; rc->right=cx+12; rc->bottom=cy+12;
...
static bool GetMinimizeRectPhysical(HWND hwnd,RECT* rc){
    ...
    if(GetTaskbarTargetPhysical(hwnd,rc)) return true;
...
static uint32_t SampleBicubic(const uint32_t* src,int sw,int sh,float u,float v){
    if(sw<=2||sh<=2) return SampleBilinear(src,sw,sh,u,v);
...
static void RasterTriangle(uint32_t* dst,int stride,int dw,int dh,const uint32_t* src,int sw,int sh,Vertex v0,Vertex v1,Vertex v2,BYTE alpha,bool hq,int clipY0,int clipY1){
    float area=Edge(v0,v1,v2); if(std::fabs(area)<0.5f) return; const double inv=1.0/double(area);
    ...
    minX=std::clamp(minX,0,dw-1); maxX=std::clamp(maxX,0,dw-1);
    minY=std::clamp(std::max(minY,clipY0),0,dh-1); maxY=std::clamp(std::min(maxY,clipY1),0,dh-1);
    const float af=float(alpha)/255.f;
    const double w0dx=double(v2.y-v1.y)*inv, w1dx=double(v0.y-v2.y)*inv, w2dx=double(v1.y-v0.y)*inv;
    const double udx=w0dx*v0.u+w1dx*v1.u+w2dx*v2.u, vdx=w0dx*v0.v+w1dx*v1.v+w2dx*v2.v;
    for(int y=minY;y<=maxY;++y){ uint32_t* row=dst+size_t(y)*size_t(stride);
        Vertex p{float(minX)+0.5f,float(y)+0.5f,0,0};
        double w0=double(Edge(v1,v2,p))*inv, w1=double(Edge(v2,v0,p))*inv, w2=double(Edge(v0,v1,p))*inv;
        double u=w0*v0.u+w1*v1.u+w2*v2.u, v=w0*v0.v+w1*v1.v+w2*v2.v;
        for(int x=minX;x<=maxX;++x, w0+=w0dx, w1+=w1dx, w2+=w2dx, u+=udx, v+=vdx){
            if(w0<0||w1<0||w2<0) continue;
            uint32_t s=hq?SampleBicubic(src,sw,sh,float(u),float(v)):SampleBilinear(src,sw,sh,float(u),float(v));
...
struct RasterBands{
    uint32_t* dst=nullptr; int stride=0,dw=0,dh=0; const uint32_t* src=nullptr; int sw=0,sh=0;
    const Vertex* c=nullptr; BYTE alpha=255; bool hq=false;
    int y0=0,bandH=1,bands=1; std::atomic<int> next{0};
};
static void RasterBand(RasterBands& j,int band){
    const int y0=j.y0+band*j.bandH, y1=y0+j.bandH-1;
    RasterTriangle(j.dst,j.stride,j.dw,j.dh,j.src,j.sw,j.sh,j.c[0],j.c[1],j.c[2],j.alpha,j.hq,y0,y1);
    RasterTriangle(j.dst,j.stride,j.dw,j.dh,j.src,j.sw,j.sh,j.c[0],j.c[2],j.c[3],j.alpha,j.hq,y0,y1);
}
static void CALLBACK RasterWorkCallback(PTP_CALLBACK_INSTANCE,PVOID ctx,PTP_WORK){
    auto* j=static_cast<RasterBands*>(ctx);
    for(int b=j->next.fetch_add(1); b<j->bands; b=j->next.fetch_add(1)) RasterBand(*j,b);
}
static void RasterQuad(uint32_t* dst,int stride,int dw,int dh,const uint32_t* src,int sw,int sh,const Vertex c[4],BYTE alpha,bool hq){
    ...
    const int y0=std::clamp(int(std::floor(std::min({c[0].y,c[1].y,c[2].y,c[3].y}))),0,dh-1);
    const int y1=std::clamp(int(std::ceil (std::max({c[0].y,c[1].y,c[2].y,c[3].y}))),0,dh-1);
    const int rows=y1-y0+1;
    static int s_cpus=0;
    if(!s_cpus){ SYSTEM_INFO si{}; GetSystemInfo(&si); s_cpus=int(si.dwNumberOfProcessors?si.dwNumberOfProcessors:1); }
    int workers=1;
    if(rows>128&&size_t(rows)*size_t(dw)>size_t(400000)&&s_cpus>1)
        workers=std::min({s_cpus,4,rows/64});
    if(workers>1){
        RasterBands j; j.dst=dst; j.stride=stride; j.dw=dw; j.dh=dh; j.src=src; j.sw=sw; j.sh=sh; j.c=c; j.alpha=alpha; j.hq=hq;
        j.bands=workers*2; j.bandH=(rows+j.bands-1)/j.bands; j.bands=(rows+j.bandH-1)/j.bandH; j.y0=y0;
        ScopedThreadpoolWork work(CreateThreadpoolWork(RasterWorkCallback,&j,nullptr));
        if(work){
            for(int i=1;i<workers;++i) SubmitThreadpoolWork(work.get());
            RasterWorkCallback(nullptr,&j,nullptr);
            return;
        }
    }
    RasterTriangle(dst,stride,dw,dh,src,sw,sh,c[0],c[1],c[2],alpha,hq,y0,y1);
    RasterTriangle(dst,stride,dw,dh,src,sw,sh,c[0],c[2],c[3],alpha,hq,y0,y1);
}
class PresentGdi{
   public:
...
static bool PresentOverlay(HWND hwndOverlay, PresentGdi& gdi, const AnimRequest& req, const RECT& rcCurrent, const Win7TransformParams& params, bool hqFinal=false) {
    ...
    bool tiny3d=!isClose&&std::fabs(params.rotX)<0.35f&&std::fabs(params.rotY)<0.35f&&std::fabs(params.transZ)<0.25f;
    ...
    if(tiny3d){
        ...
        for(int y=0;y<dh;++y) std::memset(static_cast<uint32_t*>(bits)+size_t(y)*size_t(stride),0,size_t(dw)*4);
...
enum class AnimMsg:UINT{ FirstFrame=1,Drain=2,Hide=3,GhostArm=4 };
...
static void RunAnimation(HWND hwndOverlay,AnimRequest& req){
    ...
    if(req.gdi) req.gdi->EnsureSource(req.capture);
...
static bool WaitForAnimWndThread(){
    ...
    if(g_hAnimWndThread){ HWND ha=g_hwndAnim.load(); if(ha&&IsWindow(ha)) return true; if(g_dwAnimThreadId) PostThreadMessageW(g_dwAnimThreadId,WM_QUIT,0,0);
        ...
        if(WaitForSingleObject(g_hAnimWndThread,2000)!=WAIT_OBJECT_0){ g_fDisabled.store(true); Wh_Log(L"overlay thread did not exit in time"); return false; }
        CloseHandle(g_hAnimWndThread); g_hAnimWndThread=nullptr; g_dwAnimThreadId=0;
    }
    ScopedHandle hEvent(CreateEventW(nullptr,TRUE,FALSE,nullptr)); if(!hEvent) return false;
    g_hAnimWndThread=CreateThread(nullptr,0,AnimWndThreadProc,hEvent.get(),0,nullptr); if(!g_hAnimWndThread) return false;
    g_dwAnimThreadId=GetThreadId(g_hAnimWndThread);
    if(WaitForSingleObject(hEvent.get(),1000)!=WAIT_OBJECT_0){ hEvent.release(); g_fDisabled.store(true); return false; }
...
static bool IsAnimateCandidate(HWND hwnd){
    ...
    if(pid!=GetCurrentProcessId()){ Wh_Log(L"skipped: window of another process"); return false; }
...
static constexpr wchar_t kClosingProp[]=L"win7-window-animations-restorer.Closing";
static bool CloseIsProgressSafe(HWND hwnd){   // never delay a close that could cost work
    if(!hwnd||!IsWindow(hwnd)) return false;
    if(GetWindowThreadProcessId(hwnd,nullptr)!=GetCurrentThreadId()) return false;
    if(GetSystemMetrics(SM_SHUTTINGDOWN)) return false;
    if(pIsHungAppWindow&&pIsHungAppWindow(hwnd)) return false;
    if(!IsWindowEnabled(hwnd)) return false;
    HWND popup=GetLastActivePopup(hwnd);
    if(popup&&popup!=hwnd&&IsWindow(popup)&&IsWindowVisible(popup)) return false;
    return true;
}
...
struct PendingClose{ RECT rc{}; CaptureBits cap; ULONGLONG tick=0; bool ghost=false; };
static std::mutex g_pendingCloseMutex; static PendingClose g_pendingClose;
...
static void HideGhostOverlay(){
    ...
    if(ha&&IsWindow(ha)&&!g_fAnimating.load()) HideOverlayWindow(ha);
}
static void ForgetShellPreCapture(HWND hwnd){
    if(g_pendingCloseHwnd.load()!=hwnd) return;
    bool ghost=false;
    try{ std::lock_guard<std::mutex> lock(g_pendingCloseMutex); ghost=g_pendingClose.ghost; g_pendingClose=PendingClose{}; g_pendingCloseHwnd.store(nullptr); }
    catch(...){ return; }
    if(ghost) HideGhostOverlay();
}
static void GhostWatchdog(){
    HWND hwnd=g_pendingCloseHwnd.load();
    if(!hwnd) return;
    bool expired=false, ghost=false;
    try{ std::lock_guard<std::mutex> lock(g_pendingCloseMutex);
        expired=GetTickCount64()-g_pendingClose.tick>=kShellHideToCloseMs;
        ghost=g_pendingClose.ghost;
        if(expired){ g_pendingClose=PendingClose{}; g_pendingCloseHwnd.store(nullptr); } }
    catch(...){ return; }
    if(expired&&ghost) HideGhostOverlay();
}
...
static bool GetShellFrameBounds(HWND hwnd,RECT* rc){
    if(!hwnd||!rc) return false;
    RECT ext{},win{};
    if(FAILED(DwmGetWindowAttribute(hwnd,DWMWA_EXTENDED_FRAME_BOUNDS,&ext,sizeof(ext)))) return false;
    if(!IsRectUsable(ext)||RECTW(ext)<32||RECTH(ext)<32) return false;
    if(RECTW(ext)>kMaxCaptureSide||RECTH(ext)>kMaxCaptureSide) return false;
    if(!GetWindowRectPhysical(hwnd,&win)||!IsRectUsable(win)) return false;
    if(ext.left<win.left-2||ext.top<win.top-2||ext.right>win.right+2||ext.bottom>win.bottom+2) return false;
    const double area=double(RECTW(ext))*double(RECTH(ext)), full=double(RECTW(win))*double(RECTH(win));
    if(full<=0.0||area<full*0.5) return false;
    *rc=ext; return true;
}
static bool CaptureLooksComposed(const CaptureBits& cap){
    if(cap.empty()||cap.width<32||cap.height<32) return false;
    const int w=cap.width,h=cap.height; const uint32_t* p=cap.pixels.data();
    const int stepX=std::max(1,w/64), stepY=std::max(1,h/64);
    size_t samples=0,lit=0;
    for(int x=0;x<w;x+=stepX){ samples+=2; if(p[x]&0x00FFFFFFu) ++lit; if(p[size_t(h-1)*size_t(w)+size_t(x)]&0x00FFFFFFu) ++lit; }
    for(int y=0;y<h;y+=stepY){ const uint32_t* row=p+size_t(y)*size_t(w); samples+=2; if(row[0]&0x00FFFFFFu) ++lit; if(row[w-1]&0x00FFFFFFu) ++lit; }
    return samples>0&&lit*16>=samples;
}
static void ShellPreCaptureForClose(HWND hwnd){
  try{
    if(!g_animateClose||g_fDisabled.load()||g_fAnimating.load()||g_stopping.load()) return;
    if(IsSnippingTool()||!IsShellBrowserWindow(hwnd)) return;
    if(g_pendingCloseHwnd.load()==hwnd){
        bool fresh=false;
        try{ std::lock_guard<std::mutex> lock(g_pendingCloseMutex); fresh=(GetTickCount64()-g_pendingClose.tick)<100; }catch(...){}
        if(fresh) return;
    }
    if(!IsWindowVisible(hwnd)||IsIconic(hwnd)) return;
    if(!IsTopLevelCloseCandidate(hwnd)||!CloseIsProgressSafe(hwnd)) return;
    RECT rc{}; CaptureBits cap;
    { ScopedDpiAware dpi; if(!GetShellFrameBounds(hwnd,&rc)) return; if(!CaptureWindowForClose(hwnd,cap,true)) return; }
    if(cap.empty()||!IsRectUsable(rc)) return;
    if(cap.width!=RECTW(rc)||cap.height!=RECTH(rc)) return;
    if(!CaptureLooksComposed(cap)){ Wh_Log(L"shell: capture does not look composed, skipping"); return; }
    ...
    ScopedExit hideGhost([ghost](){ if(ghost) HideGhostOverlay(); });
    if(cap.empty()) return;
    try{ std::lock_guard<std::mutex> lock(g_pendingCloseMutex);
        g_pendingClose.rc=rc; g_pendingClose.cap=std::move(cap); g_pendingClose.tick=GetTickCount64(); g_pendingClose.ghost=ghost;
        g_pendingCloseHwnd.store(hwnd); }
    catch(...){ return; }
    hideGhost.Dismiss();
    if(ghost){ HWND ha=g_hwndAnim.load(); UINT msg=g_msgAnim.load();
        if(ha&&msg&&IsWindow(ha)) PostMessageW(ha,msg,WPARAM(AnimMsg::GhostArm),LPARAM(kShellHideToCloseMs)); }
  }catch(...){
    try{ ForgetShellPreCapture(hwnd); }catch(...){}
  }
}
static bool TakeShellPreCapture(HWND hwnd,CaptureBits& cap,RECT& rc){
  try{
    if(g_pendingCloseHwnd.load()!=hwnd) return false;
    ULONGLONG tick=0; bool ghost=false;
    try{ std::lock_guard<std::mutex> lock(g_pendingCloseMutex);
        cap=std::move(g_pendingClose.cap); rc=g_pendingClose.rc; tick=g_pendingClose.tick; ghost=g_pendingClose.ghost;
        g_pendingClose=PendingClose{}; g_pendingCloseHwnd.store(nullptr); }
    catch(...){ return false; }
    if(cap.empty()||!IsRectUsable(rc)||GetTickCount64()-tick>kShellHideToCloseMs){ cap={}; if(ghost) HideGhostOverlay(); return false; }
    if(!CaptureLooksComposed(cap)){ cap={}; if(ghost) HideGhostOverlay(); return false; }
    return true;
  }catch(...){ cap={}; return false; }
}
static bool PlayClose(HWND hwnd){
    if(!g_animateClose) return false;
    if(IsSnippingTool()) return false;
    if(!CloseIsProgressSafe(hwnd)){ Wh_Log(L"close: skipped, animating could cost the user work"); return false; }
    CaptureBits pre; RECT preRc{}; bool usePre=false;
    if(!ShouldAnimateClose(hwnd)){
        if(g_fDisabled.load()||g_fAnimating.load()) return false;
        if(IsWindowVisible(hwnd)||IsIconic(hwnd)) return false;
        if(!IsTopLevelCloseCandidate(hwnd)) return false;
...
static UINT CmdFromShow(int c){
    switch(c){ case SW_MINIMIZE: case SW_SHOWMINIMIZED: case SW_FORCEMINIMIZE: return SC_MINIMIZE; case SW_RESTORE: return SC_RESTORE; default: return 0; }
}
...
BOOL WINAPI DestroyWindow_hook(HWND hWnd){
    bool animated=false;
    if(g_animateClose&&!GetPropW(hWnd,kClosingProp)&&CloseIsProgressSafe(hWnd)){
        ScopedProp closing(hWnd,kClosingProp);
        // nothing thrown by the animation may ever reach the application's close path
        try{ if(closing.ok()) animated=PlayClose(hWnd); }catch(...){ animated=false; }
    }
    ForgetCapture(hWnd); ForgetShellPreCapture(hWnd);
...
BOOL Wh_ModInit(){
    InitExeName(); g_hinst=GetCurrentModule(); LoadSettings();
    HMODULE user32=GetModuleHandleW(L"user32.dll"); if(!user32) return FALSE;
    pGetWindowMinimizeRect=GetWindowMinimizeRect_t(GetProcAddress(user32,"GetWindowMinimizeRect"));
    pIsHungAppWindow=IsHungAppWindow_t(GetProcAddress(user32,"IsHungAppWindow"));
    InterlockedExchange(&g_isUninitializing, 0);
#define HOOK_WIDE2(s) L##s
#define HOOK_WIDE(s) HOOK_WIDE2(s)
#define HOOK(f) if(!WindhawkUtils::SetFunctionHook(f,f##_hook,&f##_orig)){ Wh_Log(L"hook failed: %s", HOOK_WIDE(#f)); return FALSE; }
    HOOK(DefWindowProcA) HOOK(DefWindowProcW) HOOK(DefFrameProcA) HOOK(DefFrameProcW) HOOK(DefMDIChildProcA) HOOK(DefMDIChildProcW) HOOK(DefDlgProcA) HOOK(DefDlgProcW) HOOK(ShowWindow) HOOK(ShowWindowAsync) HOOK(DestroyWindow)
#undef HOOK
#undef HOOK_WIDE
#undef HOOK_WIDE2
    return TRUE;
...
void Wh_ModUninit(){
    SafeCleanup();
    if(g_hinst) UnregisterClassW(kAnimClassName,g_hinst);
}
