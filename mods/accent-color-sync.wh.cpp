...
### ⚠️ Requirements
**To use this mod, you will need to have [kfh83](https://github.com/kfh83)'s OpenGlass-legacy fork already installed.** This also requires the GlassType value in the registry to be set to 0x01 (Aero), and it will be automatically changed as such if it isn't.
...
* When using [Explorer7](https://github.com/Erizur/explorer7-releases/), the Start menu does not immediately change in opacity and requires a restart of explorer.exe.
*/
// ==/WindhawkModReadme==
...
- ex7: FALSE
  $name: Bypass Windows system compatibility check
  $description: Forces the mod to skip checking compatibility for Windows 10 and above. This can be particularly useful for using ports of the Windows 7 Explorer to newer versions (Explorer7 / ex7forw8).
*/
// ==/WindhawkModSettings==
