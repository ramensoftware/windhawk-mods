// ==WindhawkMod==
// @id              win7-network-flyout-sab-fix
// @name            Windows 7 Network Flyout (StartAllBack / Win11 Fix)
// @description     Restores the Windows 7 flyout on Windows 11 with StartAllBack support.
// @version         1.0.0
// @author          awnaise
...
// @compilerOptions -DWIN32_LEAN_AND_MEAN -lgdi32 -ldwmapi -luxtheme -lole32 -lshell32 -luser32 -lcomctl32 -liphlpapi -lwlanapi -luuid -lshlwapi -loleaut32 -lversion
// ==/WindhawkMod==
...
HINSTANCE WINAPI ShellExecuteW_Hook(
    ...
    if ((lpFile && wcsstr(lpFile, L"ms-availablenetworks:")) ||
        (lpParameters && wcsstr(lpParameters, L"ms-availablenetworks:"))) {
        ToggleFlyoutWindow();
        return (HINSTANCE)33; // Suppress Win11 flyout
    }
    return ShellExecuteW_Original(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd);
}

BOOL WINAPI ShellExecuteExW_Hook(SHELLEXECUTEINFOW *pExecInfo) {
    if (pExecInfo) {
        if ((pExecInfo->lpFile && wcsstr(pExecInfo->lpFile, L"ms-availablenetworks:")) ||
            (pExecInfo->lpParameters && wcsstr(pExecInfo->lpParameters, L"ms-availablenetworks:"))) {
            ToggleFlyoutWindow();
            return TRUE; // Suppress Win11 flyout
        }
    }
...
static BOOL GetNetworkIconScreenRect(RECT* outRect) {
    ...
    // Fallback for StartAllBack or custom taskbars where tray lookup fails
    POINT pt;
    GetCursorPos(&pt);
    if (outRect) {
        *outRect = RECT{ pt.x - 12, pt.y - 12, pt.x + 12, pt.y + 12 };
    }
    return TRUE;
}
...
BOOL Wh_ModInit() {
    // --- Hook Shell Execution for ms-availablenetworks: ---
    HMODULE hShell32 = GetModuleHandleW(L"shell32.dll");
    if (hShell32) {
        void* pShellExec = (void*)GetProcAddress(hShell32, "ShellExecuteW");
        if (pShellExec) {
            Wh_SetFunctionHook(pShellExec, (void*)ShellExecuteW_Hook, (void**)&ShellExecuteW_Original);
        }
        void* pShellExecEx = (void*)GetProcAddress(hShell32, "ShellExecuteExW");
        if (pShellExecEx) {
            Wh_SetFunctionHook(pShellExecEx, (void*)ShellExecuteExW_Hook, (void**)&ShellExecuteExW_Original);
        }
    }
