// ==WindhawkMod==
// @id              taskbar-hardware-telemetry-clock-customisation
// @name            Taskbar Hardware Telemetry and Clock Customisation
// @description     Minimal CPU, GPU, RAM, VRAM, temperature, time and date telemetry with NVIDIA GPU temperature support. Forked from Taskbar Clock Customization by m417z.
// @version         1.0
// @author          Connor
// @github          https://github.com/connorstx
// @homepage        https://github.com/connorstx/taskbar-hardware-telemetry-clock-customisation
// @license         Apache-2.0
...
// Original contributions by Connor are licensed under the Apache License 2.0.
// Portions derived from Taskbar Clock Customization by m417z retain their
// original GNU General Public License v3.0 terms and attribution.
// Upstream source: https://github.com/ramensoftware/windhawk-mods
// Upstream development: https://github.com/m417z/my-windhawk-mods

// ==WindhawkModReadme==
/*
# Taskbar Hardware Telemetry and Clock Customisation

A focused fork of **Taskbar Clock Customization** by m417z. It keeps the
battle-tested taskbar compatibility layer while narrowing the user-facing
feature set to clock and hardware telemetry.
...
* `%cpu%` - CPU usage.
* `%cpu_temp%` - CPU temperature in °C from Windows ACPI thermal zones.
* `%gpu%` - GPU usage.
* `%gpu_temp%` - NVIDIA GPU temperature in °C through NVAPI.
* `%ram%` - RAM usage.
* `%vram%` - dedicated VRAM usage percentage.

`%gpu_temp%` is NVIDIA-only. On unsupported hardware or when NVAPI cannot
return a temperature, the pattern displays `-`.

The default format is `%time% %date%`. Hardware telemetry refreshes according
to the configured update interval.

## Credits and licensing

This project is forked from Taskbar Clock Customization by m417z. Upstream
copyright, attribution and licence notices are retained. Original additions by
Connor are licensed under Apache License 2.0; inherited portions retain their
upstream GPL-3.0 terms.
*/
// ==/WindhawkModReadme==
...
- Format: '%time% %date%'
  $name: Format
  $description: >-
    Taskbar text format. Supported patterns: %time%, %date%, %cpu%, %cpu_temp%,
    %gpu%, %gpu_temp%, %ram%, and %vram%.
- TimeFormat: ""
  $name: Time format
  $description: >-
    Format used by %time%. Leave empty to use the Windows default time format.
- DateFormat: ""
  $name: Date format
  $description: >-
    Format used by %date%. Leave empty to use the Windows default date format.
- UpdateInterval: 1
  $name: Update interval
  $description: Hardware telemetry refresh interval in seconds.
- GpuAdapterName: ""
  $name: GPU adapter name
  $description: >-
    GPU name substring used for GPU and VRAM selection. Leave empty to
    auto-select the adapter with the most dedicated VRAM. GPU temperature uses
    NVAPI and is available on NVIDIA GPUs only.
*/
// ==/WindhawkModSettings==
...
bool IsDynamicTelemetryPatternUsed()
{
    return IsStrInDateTimePatternSettings(L"%cpu%") ||
...
// NVIDIA NVAPI thermal support. The API is loaded dynamically so this fork
// continues to work on systems without NVIDIA hardware.
constexpr unsigned int kNvApiMaxPhysicalGpus = 64;
constexpr unsigned int kNvApiMaxThermalSensors = 3;
constexpr unsigned int kNvApiThermalTargetGpu = 1;
constexpr unsigned int kNvApiThermalTargetAll = 15;
...
using NvApiQueryInterface_t = void* (__cdecl*)(unsigned int id);
using NvApiInitialize_t = int (__cdecl*)();
using NvApiEnumPhysicalGpus_t = int (__cdecl*)(void** handles,
                                               unsigned int* count);
using NvApiGpuGetFullName_t = int (__cdecl*)(void* handle, char* name);
using NvApiGpuGetThermalSettings_t = int (__cdecl*)(
    void* handle,
    unsigned int sensorIndex,
    NvApiThermalSettings* settings);

std::optional<int> QueryNvApiGpuTemperatureC()
{
    static HMODULE module = nullptr;
    static NvApiEnumPhysicalGpus_t enumPhysicalGpus = nullptr;
    static NvApiGpuGetFullName_t gpuGetFullName = nullptr;
    static NvApiGpuGetThermalSettings_t gpuGetThermalSettings = nullptr;
    ...
    if (!initializationAttempted)
    {
        ...
        module = LoadLibraryExW(L"nvapi64.dll", nullptr,
        ...
        NvApiQueryInterface_t queryInterface =
            reinterpret_cast<NvApiQueryInterface_t>(
        ...
        NvApiInitialize_t initialize =
            reinterpret_cast<NvApiInitialize_t>(queryInterface(0x0150E828));
        enumPhysicalGpus = reinterpret_cast<NvApiEnumPhysicalGpus_t>(
            queryInterface(0xE5AC921F));
        gpuGetFullName = reinterpret_cast<NvApiGpuGetFullName_t>(
            queryInterface(0xCEEE8E9F));
        gpuGetThermalSettings =
            reinterpret_cast<NvApiGpuGetThermalSettings_t>(
                queryInterface(0xE3640A56));

        if (!initialize || !enumPhysicalGpus || !gpuGetFullName ||
            !gpuGetThermalSettings || initialize() != kNvApiOk)
        {
            enumPhysicalGpus = nullptr;
            gpuGetFullName = nullptr;
            gpuGetThermalSettings = nullptr;
            return std::nullopt;
        }
    }

    if (!enumPhysicalGpus || !gpuGetFullName || !gpuGetThermalSettings)
    {
        return std::nullopt;
    }

    void* handles[kNvApiMaxPhysicalGpus]{};
    unsigned int count = 0;
    if (enumPhysicalGpus(handles, &count) != kNvApiOk || count == 0)
    {
        return std::nullopt;
    }

    void* selectedHandle = nullptr;
    PCWSTR configuredName = g_settings.dataCollection.gpuAdapterName.get();

    for (unsigned int i = 0; i < count; i++)
    {
        char nameA[64]{};
        if (gpuGetFullName(handles[i], nameA) != kNvApiOk)
        {
            continue;
        }
        ...
        WCHAR nameW[64]{};
        if (MultiByteToWideChar(CP_ACP, 0, nameA, -1, nameW,
                                ARRAYSIZE(nameW)) > 0 &&
            StrStrIW(nameW, configuredName))
        {
            selectedHandle = handles[i];
            break;
        }
    ...
    NvApiThermalSettings settings{};
    settings.version = sizeof(settings) | (2u << 16);

    if (gpuGetThermalSettings(selectedHandle, kNvApiThermalTargetAll,
                              &settings) != kNvApiOk ||
        settings.count == 0)
    {
        return std::nullopt;
    ...
    unsigned int sensorCount =
        std::min(settings.count, kNvApiMaxThermalSensors);

    for (unsigned int i = 0; i < sensorCount; i++)
    {
        if (settings.sensor[i].target ==
            static_cast<int>(kNvApiThermalTargetGpu))
        {
...
std::optional<int> GetGpuTemperatureC()
{
    static DWORD lastFormatIndex = 0xFFFFFFFF;
    static std::optional<int> cachedTemperature;

    DWORD formatIndex = GetDataCollectionFormatIndex();
    if (lastFormatIndex != formatIndex)
    {
        cachedTemperature = QueryNvApiGpuTemperatureC();
...
{
    DWORD formatIndex = GetDataCollectionFormatIndex();
    if (g_gpuTempFormatted.formatIndex != formatIndex)
    {
        std::optional<int> temperature = GetGpuTemperatureC();
        if (temperature)
        {
            swprintf_s(g_gpuTempFormatted.buffer,
                       ARRAYSIZE(g_gpuTempFormatted.buffer), L"%d\u00B0C",
                       *temperature);
        }
        else
        {
            wcscpy_s(g_gpuTempFormatted.buffer, L"-");
        }

        g_gpuTempFormatted.formatIndex = formatIndex;
    }

    return g_gpuTempFormatted.buffer;
}
    ...
    std::function<void(PCWSTR resolvedStr)> resolvedCallback)
{
    ...
    for (const FormatToken& formatToken : formatTokens)
    {
...
void ClockSystemTrayIconDataModel_RefreshIcon_Hook_Impl(
    ...
    g_refreshIconNeedToAdjustTimer =
        g_settings.showSeconds || g_dataCollectionSession ||
        IsDynamicTelemetryPatternUsed() || webContentPending;
...
void LoadSettings() {
    ...
    g_settings.topLine = StringSetting::make(L"Format");
    ...
    g_settings.dataCollection.updateInterval =
        Wh_GetIntSetting(L"UpdateInterval");
    ...
    g_settings.dataCollection.gpuAdapterName =
        StringSetting::make(L"GpuAdapterName");
