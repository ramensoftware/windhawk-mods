// ==WindhawkMod==
// @id              taskbar-rearrange-left-mouse-button
// @name            Taskbar rearrange windows of the same application (LEFT mouse button)
...
//
// This mod was forked from https://github.com/m417z/my-windhawk-mods/blob/main/mods/taskbar-reorder-right-drag.wh.cpp
...
std::atomic<bool> g_leftDragJustCompleted;
...
// Left-drag state tracking for taskbar buttons.
struct {
    ...
    bool newGroup = false;
    winrt::Windows::Foundation::Point pressPosition;
...
HRESULT WINAPI CTaskListWnd_HandleClick_Hook(void* pThis,
    ...
    if (g_leftDragJustCompleted.exchange(false)) {
        Wh_Log(L"Suppressing click after left-drag");
        return S_OK;
    }
...
void ClearRightDragState() {
    ...
    g_rightDragState.dragThresholdMet = false;
}
...
int WINAPI TaskListButton_OnPointerMoved_Hook(void* pThis, void* pArgs) {
    ...
    if (!g_rightDragState.dragThresholdMet) {
        auto pos = args.GetCurrentPoint(nullptr).Position();
        float dx = pos.X - g_rightDragState.pressPosition.X;
        float dy = pos.Y - g_rightDragState.pressPosition.Y;
        if (dx * dx + dy * dy < 25.0f) {
            // Suppress native handler even before threshold to prevent the
            // OS group-drag from starting before we take control.
            return 0;
        }
        g_rightDragState.dragThresholdMet = true;
    }
    ...
    // Do not call original() — suppressing the native handler prevents
    // the OS from initiating its own group-drag while ours is active.
    return 0;
}
...
int WINAPI TaskListButton_OnPointerPressed_Hook(void* pThis, void* pArgs) {
    ...
    // Initiate left-drag if left button pressed without right button, and the
    // element is a valid task item.
    if (isLeftButtonPressed && !isRightButtonPressed &&
        GetTaskGroupFromElement(element)) {
        Wh_Log(L"Starting left-drag");
        g_rightDragState.sourceElement = element;
        g_rightDragState.pressPosition =
            args.GetCurrentPoint(nullptr).Position();
        g_rightDragState.dragThresholdMet = false;
    }
...
int WINAPI TaskListButton_OnPointerReleased_Hook(void* pThis, void* pArgs) {
    ...
    if (sourceTaskItem && targetTaskItem) {
        if (sourceTaskGroup == targetTaskGroup) {
            ...
            Wh_Log(
                L"Completing left-drag: moving %p to %p (insertAfter=%d) in "
                L"group %p",
    ...
    // Suppress the left-click activation that fires after pointer release if
    // an actual drag was performed (target was reached or new group created).
    if (sourceTaskItem && (targetTaskItem || g_rightDragState.newGroup)) {
        g_leftDragJustCompleted = true;
    }
...
HMODULE GetTaskbarViewModuleHandle() {
    ...
    if (!module) {
        module = GetModuleHandle(L"twinui.pcshell.dll");
    }
...
using LdrLoadDll_t = NTSTATUS(NTAPI*)(PWSTR SearchPath,
...
NTSTATUS NTAPI LdrLoadDll_Hook(PWSTR SearchPath,
                                ...
                                PVOID* BaseAddress) {
    NTSTATUS status =
        LdrLoadDll_Original(SearchPath, DllCharacteristics, DllName,
                             BaseAddress);
    if (NT_SUCCESS(status) && BaseAddress && !g_taskbarViewDllLoaded) {
        HMODULE module = (HMODULE)*BaseAddress;
        if (module) {
            HandleLoadedModuleIfTaskbarView(module, L"twinui.pcshell.dll");
        }
    }
    return status;
}
...
BOOL Wh_ModInit() {
    ...
    if (HMODULE taskbarViewModule = GetTaskbarViewModuleHandle()) {
        ...
        auto pLdrLoadDll =
            (LdrLoadDll_t)GetProcAddress(ntdllModule, "LdrLoadDll");
        if (pLdrLoadDll) {
            WindhawkUtils::SetFunctionHook(pLdrLoadDll, LdrLoadDll_Hook,
                                           &LdrLoadDll_Original);
        }
    }
