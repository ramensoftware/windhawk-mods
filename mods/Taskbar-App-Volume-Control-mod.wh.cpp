...
// @description     Control the system volume and apps volume (SOME APPS) by
// scrolling .
// @version         1.0
// @author          tz39
// @github          https://github.com/tz39/Taskbar-APP-Volume-Control
// @include         explorer.exe
// @compilerOptions -DWINVER=0x0A00 -lcomctl32 -ldwmapi -lgdi32 -lole32
// -lversion -luiautomationcore -loleaut32 -luuid
// ==/WindhawkMod==
// ==WindhawkModReadme==
/*Control the system volume and apps volume (SOME APPS) by scrolling over the
 * taskbar for system volume and at icon to change the app volume, Optimized for
 * Windows 10 and 11 taskbars. Supports popular apps like Chrome, Firefox,
 * Spotify, Discord,...*/
// ==/WindhawkModReadme==
...
void ClearPidCache() { g_pidCache.clear(); }

// Simple case-insensitive substring helper
bool HasSubstringCaseInsensitive(const std::wstring &str,
...
bool SmartFuzzyMatch(const std::wstring &sessionName,
  ...
  if (nSession.empty() || nTarget.empty())
    return false;
...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam) {
  ...
  // Check if Window Title contains the Button Name (e.g. "YouTube - Chrome"
  // contains "Chrome" logic? No, usually "Chrome" is Button Name) Actually, on
  // Win11 grouped, Button Name = "Google Chrome". Window Title = "Tab - Google
  // Chrome". So Window Title contains Button Name.
...
BOOL SetAppVolume(DWORD pid, int nDelta,
                  const std::wstring &processNameOverride = L"",
                  bool isPartialMatch = false, bool useSmartMatch = false) {
  HRESULT hr = S_OK;
  ...
  if (g_audioCache.pid == pid && !g_audioCache.pVolumes.empty() &&
      processNameOverride.empty()) {
    BOOL anySuccess = FALSE;
    for (auto *pVol : g_audioCache.pVolumes) {
      float level = 0.0f;
      if (SUCCEEDED(pVol->GetMasterVolume(&level))) {
        float step = g_settings.volumeChangeStep
                         ? (float)g_settings.volumeChangeStep / 100.0f
                         : 0.02f;
  ...
  for (int i = 0; i < sessionCount; i++) {
    ...
    if (SUCCEEDED(pSessionEnumerator->GetSession(i, &pSessionControl))) {
      if (SUCCEEDED(pSessionControl->QueryInterface(
        ...
        if (SUCCEEDED(pSessionControl2->GetProcessId(&sessionPid))) {
          ...
          // Check if PID matches OR if Process Name matches (handles
          // Chrome/Spotify helper processes) If isPartialMatch is true, we
          // check if sessionProcessName contains targetProcessName If
          // useSmartMatch is true, we use the aggressive normalization check.
          // Otherwise exact match.
          bool nameMatch = false;
          if (useSmartMatch) {
            nameMatch = SmartFuzzyMatch(sessionProcessName, targetProcessName);
          } else if (isPartialMatch) {
            nameMatch = HasSubstringCaseInsensitive(sessionProcessName,
                                                    targetProcessName);
          } else {
            nameMatch = (_wcsicmp(targetProcessName.c_str(),
                                  sessionProcessName.c_str()) == 0);
          }

          if (sessionPid == pid || nameMatch) {
            // Wh_Log(L"FOUND MATCH: %s (PID: %d) for Target: %s",
            // sessionProcessName.c_str(), sessionPid,
            // targetProcessName.c_str());

            ISimpleAudioVolume *pVol = NULL;
            if (SUCCEEDED(pSessionControl2->QueryInterface(
                    __uuidof(ISimpleAudioVolume), (void **)&pVol))) {
              float level = 0.0f;
              if (SUCCEEDED(pVol->GetMasterVolume(&level))) {
                float step = g_settings.volumeChangeStep
                                 ? (float)g_settings.volumeChangeStep / 100.0f
                ...
                if (SUCCEEDED(pVol->SetMasterVolume(level, NULL))) {
                  Wh_Log(L"Adjusted Volume for: %s (PID: %d)",
                         sessionProcessName.c_str(), sessionPid);
                  bFound = TRUE;

                  // --- Update Cache ---
                  if (processNameOverride
                          .empty()) { // Only cache pure PID lookups
    ...
    // REMOVED break; to allow finding multiple sessions (e.g. Discord Voice +
    // Sounds) if (bFound) break;
  }
...
DWORD GetPidFromTaskbarPoint(POINT pt, BOOL *pIsItem, std::wstring *pNameOut) {
  *pIsItem = FALSE;
  ...
  if (FAILED(g_pAutomation->ElementFromPoint(pt, &pElement)) || !pElement)
    return 0;
  ...
  // Check if it's a button (50000) or list item (50007) - typical for taskbar
  // apps Also Header (50034) or Pane (50033) sometimes wrap the icon in Win11?
  ...
  if (SUCCEEDED(pElement->get_CurrentNativeWindowHandle(&uiaHwnd)) && uiaHwnd) {
    ...
    // If the PID belongs to Explorer (which owns the taskbar), it's not the
    // target app. We should ignore it and fallback to name matching.
    DWORD explorerPid = 0;
  ...
  if (!targetPid && pElement) {
    ...
    if (!nameBstr || wcslen(nameBstr) == 0) {
      ...
      if (SUCCEEDED(pElement->GetCurrentPattern(UIA_LegacyIAccessiblePatternId,
                                                (IUnknown **)&pLegacy)) &&
          pLegacy) {
    ...
    if (nameBstr) {
      ...
      // --- PID Cache Check ---
      // If we have seen this name ("Spotify", "Chrome") before, allow fast
      // lookup.
      if (g_pidCache.find(foundName) != g_pidCache.end()) {
        ...
        HANDLE hProc =
            OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, cachedPid);
        if (hProc) {
          CloseHandle(hProc);
          targetPid = cachedPid;
      ...
      if (!targetPid) {
        ...
        if (!searchData.foundPid) {
          ...
          if (dashPos != std::wstring::npos) {
            ...
            if (pNameOut)
              *pNameOut = cleanedName; // Return the cleaned name if we used it

            // If clean name worked, maybe cache that?
            // Complex because key was foundName.
            // Let's just update the cache with the ORIGINAL key mapping to the
            // FOUND pid.
          }
        }
...
bool GetNotificationAreaRect(HWND hMMTaskbarWnd, RECT *rcResult) {
  ...
  if (g_nExplorerVersion >= WIN_VERSION_11_21H2) {
    ...
    while (hBridgeWnd) {
      ...
      if (rcBridge.left != rcTaskbar.left || rcBridge.top != rcTaskbar.top ||
...
bool IsPointInsideScrollArea(HWND hMMTaskbarWnd, POINT pt) {
  ...
  // But for robustness, we always check against the known global taskbar
  // handle. This allows child windows (like the Comet button) to pass the check
  // even if they haven't been passed in as the root.
  ...
  // TODO: support secondary taskbars if needed, iterating
  // g_secondaryTaskbarWindows For now, checking the primary global taskbar
  // handle should fix the Comet issue if it lives on the main bar.
...
BOOL AddMasterVolumeLevelScalar(float fMasterVolumeAdd) {
  ...
  if (g_pDeviceEnumerator) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        if (SUCCEEDED(
                endpointVolume->GetMasterVolumeLevelScalar(&fMasterVolume))) {
...
void SndVolInit() {
  HRESULT hr = CoCreateInstance(XIID_MMDeviceEnumerator, NULL,
                                CLSCTX_INPROC_SERVER, XIID_IMMDeviceEnumerator,
                                (LPVOID *)&g_pDeviceEnumerator);
  if (FAILED(hr))
...
static BOOL OpenScrollSndVolInternal(WPARAM wParam, LPARAM lMousePosParam,
                                     HWND hVolumeAppWnd, BOOL *pbOpened);
static BOOL ValidateSndVolProcess();
static BOOL ValidateSndVolWnd();
static void CALLBACK CloseSndVolTimerProc(HWND hWnd, UINT uMsg,
...
BOOL OpenScrollSndVol(WPARAM wParam, LPARAM lMousePosParam) {
  ...
  if (g_settings.volumeIndicator == VolumeIndicator::None) {
    return AdjustVolumeLevelWithMouseWheel(GET_WHEEL_DELTA_WPARAM(wParam), 0);
  }
      ...
      if (ValidateSndVolWnd()) {
        ...
        if (hVolumeAppWnd) {
          ...
          if (GetProcessId(hSndVolProcess) == dwProcessId) {
            BOOL bOpened;
            if (OpenScrollSndVolInternal(wParam, lMousePosParam, hVolumeAppWnd,
                                         &bOpened)) {
              if (bOpened)
                SetSndVolTimer();
  ...
  if (hMutex) {
    ...
    hVolumeAppWnd =
        FindWindow(L"Windows Volume App Window", L"Windows Volume App Window");
    if (hVolumeAppWnd) {
      GetWindowThreadProcessId(hVolumeAppWnd, &dwProcessId);

      hSndVolProcess = OpenProcess(
          PROCESS_QUERY_LIMITED_INFORMATION | SYNCHRONIZE, FALSE, dwProcessId);
      if (hSndVolProcess) {
        if (WaitForInputIdle(hSndVolProcess, 0) == 0) // if not initializing
        {
          if (ValidateSndVolWnd()) {
  ...
  if (!CreateProcess(NULL, szCommandLine, NULL, NULL, FALSE,
                     ABOVE_NORMAL_PRIORITY_CLASS | CREATE_SUSPENDED, NULL, NULL,
                     &si, &pi))
    return FALSE;
    ...
    if (ValidateSndVolProcess()) {
      ...
      if (ValidateSndVolWnd()) {
        ...
        if (hPointWnd != hSndVolWnd)
          PostMessage(hSndVolWnd, WM_ACTIVATE, MAKEWPARAM(WA_INACTIVE, FALSE),
                      (LPARAM)NULL);
      }
    }
  }
...
bool OnMouseWheel(HWND hWnd, WPARAM wParam, LPARAM lParam) {
  ...
  if (!IsPointInsideScrollArea(hWnd, pt)) {
    Wh_Log(L"OnMouseWheel: Rejected (Point %d,%d not inside scroll area of "
           L"HWND %p)",
           pt.x, pt.y, hWnd);
    return false;
  }
  ...
  // Allow if PID found OR if we identified it as an Item and have a Name (Smart
  // Match fallback)
  if (pid || (isItem && !appName.empty())) {
    ...
    if (SetAppVolume(pid, delta, processNameOverride, isPartial,
                     useSmartMatch)) {
    ...
    Wh_Log(L"No Target. PID: %d, Name: %s, isItem: %d", pid, appName.c_str(),
           isItem);
  }

  // If we are over a taskbar item (like an app icon), but failed to change its
  // volume (e.g. no audio session or pid lookup failed), we should STOP here
  // and NOT change the system master volume.
  if (isItem) {
  ...
  Wh_Log(
      L"OnMouseWheel: Rejected (No PID and no App Name, fallback to global)");
...
LRESULT CALLBACK TaskbarWindowSubclassProc(_In_ HWND hWnd, _In_ UINT uMsg,
                                           _In_ WPARAM wParam,
                                           _In_ LPARAM lParam,
...
LRESULT CALLBACK InputSiteWindowProc_Hook(HWND hWnd, UINT uMsg, WPARAM wParam,
                                          LPARAM lParam) {
  switch (uMsg) {
  case WM_POINTERWHEEL:
  case WM_MOUSEWHEEL:
  case WM_MOUSEHWHEEL:
    // Relaxed Check: We don't strictly require the root to be our known taskbar
    // window. If OnMouseWheel (which checks screen coordinates) handles it, we
    // eat it.
...
void WINAPI VolumeSystemTrayIconDataModel_OnIconClicked_Hook(
    void *pThis, void *iconClickedEventArgs) {
  Wh_Log(L">");
