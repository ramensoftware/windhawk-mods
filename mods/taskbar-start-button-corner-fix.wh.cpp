...
UI Automation is initialized lazily on a dedicated worker thread, never on the
taskbar input thread. If the taskbar's Start button can't be found or invoked,
no fallback input is generated.
...
winrt::com_ptr<IUIAutomationElement> GetTaskbarStartButton(
    IUIAutomation* automation,
    ...
    if (cachedStartButton != startButtonCache.end()) {
        return cachedStartButton->second;
    }
...
bool ActivateTaskbarStartButton(IUIAutomation* automation,
    ...
    winrt::com_ptr<IUIAutomationElement> startButton = GetTaskbarStartButton(
    ...
    Wh_Log(L"Refreshing the cached Start button for taskbar %p",
           taskbarWindow);
    startButton = GetTaskbarStartButton(
        automation, condition, startButtonCache, taskbarWindow);
    if (!startButton) {
        return false;
    }
