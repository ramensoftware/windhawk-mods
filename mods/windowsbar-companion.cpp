// ==WindhawkMod==
// @id              windowsbar-companion
// @name            WindowsBar Companion
// @description     Moves the Windows 11 Start, search and action center window to the top of the screen while keeping the taskbar in place.
// @version         1.0
// @author          Meti0X7CB
...
// ==WindhawkModReadme==
/*
# WindowsBar Companion

Moves the Windows 11 Start, search and action center window to the top of the screen while keeping the taskbar in place.

This is a modified version of Taskbar on top for Windows 11 by m417z. The goal is to run this along side WindowsBar to achieve the desired user experience.

For Win+A and Win+N to show on top as well, you need to install Windows 11 Notification Center Styler mod by m417z and put the following in the mod setting:
...
Animation for the new start menu doesnt flip.
...
HRESULT WINAPI DwmSetWindowAttribute_Hook(HWND hwnd,
    ...
    // If taskbar is bottom, keep existing behavior for Start menu (optional),
    // but STILL allow SearchHost (Win+S) to be moved to the top.
    if (GetTaskbarLocationForMonitor(monitor) == TaskbarLocation::bottom &&
        target != DwmTarget::SearchHost) {
        return original();
    }
...
void ApplyStyleClassicStartMenu(FrameworkElement content,
    ...
    if (g_settings.startMenuAnimationAdjust || g_startMenuAnimationAdjusted) {
        ...
        bool adjustAnimation = !g_unloading && g_settings.startMenuAnimationAdjust;
...
void ApplyStyle() {
    ...
    TaskbarLocation taskbarLocation = TaskbarLocation::top;  // force Start menu top always
