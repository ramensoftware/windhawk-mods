...
Glass, Sharp Split, Aurora Glow, Album Art Blur, and six gradient modes (fade,
left→right, right→left, top→bottom, bottom→top, radial glow)
...
based on your system light/dark mode
...
in any order with a 4-digit code (e.g. `4213`)
- **Fixed or flexible Info width** — lock the title/artist area to a set pixel
width or let it grow freely
- **Per-container padding** — independent left/right padding on every container
plus top/bottom padding on the main wrapper
...
- **Mini Mode** — compact view showing only Album Art and/or Visualizer; great
for small taskbars

### 🎵 Media Info
- **Scrolling title & artist** — text smoothly loops when it overflows the
container (3 speed settings)
- **Truncation fallback** — adds an ellipsis when scrolling is disabled and text
overflows
- **Album art** with configurable corner radius and a music-note placeholder
when no art is available
- **Supports 16 media sources**: Spotify, Apple Music, YouTube Music, Chrome,
Edge, Firefox, Opera, Brave, Discord, VLC, Foobar2000, MusicBee, iTunes, Tidal,
Amazon Music, or auto-detect all

### ⏯️ Playback Controls
- **Previous / Play-Pause / Next** buttons
- **Shuffle** and **Repeat** toggle buttons (optional)
- **4 icon themes**: Custom drawn, Segoe MDL2 Assets, Segoe Fluent Icons
Outlined, Segoe Fluent Icons Filled, Segoe Fluent Icons Outlined Alt
- **Configurable icon size** and **button spacing**

### 🔊 Volume
- **Speaker icon** with a hover-to-expand inline volume slider — click and drag
or scroll to set level
- **Mouse wheel volume control** — scroll anywhere on the panel to raise or
lower volume, no icon needed
- **Per-app volume** — controls the volume of the playing media app only, not
the system master
...
- **Configurable bar count** (1–20), **bar width**, **bar gap**, and **idle bar
height**
- **Full-width background mode** — renders the visualizer behind all other
containers as a full-panel backdrop
- **Sensitivity control** (0–100)

### 📏 Progress Bar
- **5 position modes**: Below Everything, Above Everything, Under Info &
Controls, Under Info Only, Under Controls Only
- **Dynamic color** (matches album art) or custom hex color
- **Configurable height** (1–6 px), padding, and vertical offset
- **Smooth live interpolation** — position updates every frame between GSMTC
polls so the bar never jumps

### 🙈 Behavior
- **Auto-hide on fullscreen** — panel hides when a fullscreen app or game is
detected, with optional fade animation
- **Idle timeout** — hide the panel after N seconds of no media
- **Idle screen mode** — shows placeholder content instead of disappearing
entirely
...
```
All sections are optional and reorderable. The Info container expands into any
...
- AppearanceGroup:
  ...
  - ShowContainerBorders: false
    ...
    $description: "Draws a colored outline around each active container so you
can see exact boundaries and spacing." $name: Appearance
...
- LayoutGroup:
  ...
  - PanelWidth: 360
    $name: Width (px)
    $description: Total panel width. Only applies when Info container is in
fixed width mode.
  - OffsetX: 0
  ...
  - ContainerOrder: 1234
    ...
    $description: "Four-digit code sets left-to-right order. 1=Media 2=Info
3=Controls 4=Visualizer. Example: 4123 puts Visualizer first." $name: Layout
...
- MiniModeGroup:
  - MiniMode: false
    ...
    $description: "Compact mode — only Album Art and/or Visualizer are shown.
Padding and theme follow the main Layout and Appearance settings."
  - MiniShowMedia: true
    $name: Show Album Art
...
- MediaContainerGroup:
  ...
  - MediaWidth: 0
    $name: Width (px)
    $description: "Custom width for the art area. 0 = auto (square, or matches
art aspect ratio when Auto-Fit is on). Set a fixed value to override."
  - MediaAutoSize: true
    $name: Auto-Fit Art Shape
    $description: "Automatically resizes the art area to match the image's
native aspect ratio — square for Spotify/Apple Music, wider for YouTube
thumbnails, etc. Uses the manual Width above as a cap when set."
  - MediaCornerRadius: 6
...
- InfoContainerGroup:
  ...
  - InfoFixedWidth: true
    $name: Fixed Width
    $description: When on, Info uses exactly the set pixel width. When off it
expands to fill remaining space.
  ...
  - ArtTextGap: 12
    ...
    $description: Horizontal gap between the album art and the start of the
text.
  - FontSize: 11
    $name: Font Size (px)
  - TextGap: 2
...
- ControlsContainerGroup:
  ...
  - ScrollVolume: true
    ...
    $description: "Scroll anywhere on the panel to adjust volume. You can hover
over the panel and scroll up/down to turn the volume up or down — no need to
click anything."
  - IconTheme: mdl2
    $name: Icon Theme
    $options:
...
- VisualizerContainerGroup:
  ...
  - VizAsBackground: false
    ...
    $description: Renders behind all containers with no width slot. Spans the
full panel width.
  - VizWidth: 0
...
// Buttons that are disabled are skipped - the text area then expands to fill
// the gap.
...
inline ControlLayout CalcLayout(
    int W,
    int H,
    int padRight,
    ...
    int ctrlPadRight = 0,
    int ctrlCellRight =
        -1)  // right edge of the controls cell (ctrlCellX + containerWidth)
{
    ...
    int btnCount = 0;
    if (showPlayback)
        btnCount += 3;  // prev, play/pause, next
    if (showSpeaker)
        btnCount += 1;
    if (showRepeat)
        btnCount += 1;
    ...
    if (btnCount > 0) {
        ...
        float filledStart =
            cellLeft + filledStep * 0.5f;  // first button centre

        float cursor =
            filledStart + filledStep * (btnCount - 1);  // rightmost first
        if (showPlayback) {
            nX = cursor;
            cursor -= filledStep;
            ppX = cursor;
...
struct ModSettings {
    ...
    bool mediaAutoSize =
        true;  // auto-detect art aspect ratio and resize accordingly
...
static int CalcInfoWidth() {
    if (g_S.wrapper.infoFixedWidth) {
        return g_S.containers[1].padLeft + g_S.wrapper.infoWidth +
               g_S.containers[1].padRight;
    }
    // Flexible: will be set during UpdateBoundsAndRegion after all other widths
    // are known. Returning 0 here signals "fill remaining space."
    return 0;
}
...
static int CalcControlsWidth() {
    ...
    if (g_S.showSpeakerIcon)
        btnCount += 1;
    if (g_S.showShuffleButton)
        btnCount += 1;
    if (g_S.showRepeatButton)
        btnCount += 1;
    if (btnCount == 0)
        return 0;
    float scale = g_S.height / 52.f;
    float step = g_S.buttonSpacing * scale;
    // Span = (btnCount-1) steps between centres, plus half-step margin on each
    // side
    int w = (int)((btnCount - 1) * step +
                  step);  // == btnCount * step, one step = left+right margins
...
static int CalcVisualizerWidth() {
    ...
    if (g_S.vizContainerWidth > 0)
        return g_S.containers[3].padLeft + g_S.vizContainerWidth +
               g_S.containers[3].padRight;
...
static void RecomputeLayout() {
    ...
    if (miniMode) {
        // Mini: only Media and/or Visualizer visible; Info & Controls always
        // hidden.
        ...
        // Total width: wrapper left+right + active container widths + gap
        // between them if both shown
        int total = g_S.wrapper.padLeft + g_S.wrapper.padRight;
        int activeCount = (showMedia ? 1 : 0) + (showViz ? 1 : 0);
        if (showMedia)
            total += g_S.containers[0].calculatedWidth;
        if (showViz)
            total += g_S.containers[3].calculatedWidth;
        if (activeCount > 1)
            total += g_S.wrapper.globalGap;
    ...
    g_S.containers[2].enabled =
        g_S.controlsEnabled &&
        (g_S.showPlaybackControls || g_S.showSpeakerIcon ||
         g_S.showShuffleButton || g_S.showRepeatButton);
    ...
    for (int id : g_S.wrapper.order) {
        if (id == 2)
            continue;  // Info handled separately
        if (!IsContainerEnabled(id))
            continue;
        if (id == 4 && g_S.wrapper.vizAsBackground)
            continue;  // bg layer, no width
    ...
    if (IsContainerEnabled(2))
        activeCount++;
    int totalGaps =
        (activeCount > 1) ? (activeCount - 1) * g_S.wrapper.globalGap : 0;
    ...
    if (g_S.wrapper.infoFixedWidth) {
        ...
        if (remaining < 40)
            remaining = 40;  // enforce a minimum readable info width
...
// ── Settings helpers
// ────────────────────────────────────────────────────────── Reads a hex color
// string setting and returns a full ARGB DWORD with FF alpha. Falls back to
// defaultRgb if the setting is empty or not found.
...
void LoadSettings() {
    ...
    if (g_S.height < 32)
        g_S.height = 52;
    if (g_S.wrapper.padTop < 0)
        g_S.wrapper.padTop = 0;
    if (g_S.wrapper.padBottom < 0)
        g_S.wrapper.padBottom = 0;
    if (g_S.wrapper.padLeft < 0)
        g_S.wrapper.padLeft = 0;
    if (g_S.wrapper.padRight < 0)
        g_S.wrapper.padRight = 0;
    if (g_S.wrapper.globalGap < 0)
        g_S.wrapper.globalGap = 0;
    ...
    if (g_S.wrapper.infoWidth < 40)
        g_S.wrapper.infoWidth = 40;
    ...
    if (g_S.artTextGap < 0)
        g_S.artTextGap = 0;
    if (g_S.artTextGap > 80)
        g_S.artTextGap = 80;
    if (g_S.fontSize < 7)
        g_S.fontSize = 11;

    g_S.textControlGap =
        Wh_GetIntSetting(L"ControlsContainerGroup.TextControlGap");
    if (g_S.textControlGap < 0)
        g_S.textControlGap = 0;
    if (g_S.textControlGap > 80)
        g_S.textControlGap = 80;

    // ── Container padding slots ─────────────────────────────────────────────
    g_S.containers[0].padLeft =
        Wh_GetIntSetting(L"MediaContainerGroup.MediaPadLeft");
    g_S.containers[0].padRight =
        Wh_GetIntSetting(L"MediaContainerGroup.MediaPadRight");
    g_S.containers[0].padTop = 0;
    g_S.containers[0].padBottom = 0;
    g_S.mediaCornerRadius =
        Wh_GetIntSetting(L"MediaContainerGroup.MediaCornerRadius");
    if (g_S.mediaCornerRadius < 0)
        g_S.mediaCornerRadius = 0;
    if (g_S.mediaCornerRadius > 32)
        g_S.mediaCornerRadius = 32;
    g_S.mediaWidth = Wh_GetIntSetting(L"MediaContainerGroup.MediaWidth");
    if (g_S.mediaWidth < 0)
        g_S.mediaWidth = 0;
    g_S.mediaAutoSize =
        Wh_GetIntSetting(L"MediaContainerGroup.MediaAutoSize") != 0;
    g_S.containers[1].padLeft =
        Wh_GetIntSetting(L"InfoContainerGroup.InfoPadLeft");
    g_S.containers[1].padRight =
        Wh_GetIntSetting(L"InfoContainerGroup.InfoPadRight");
    g_S.containers[1].padTop = 0;
    g_S.containers[1].padBottom = 0;
    g_S.containers[2].padLeft =
        Wh_GetIntSetting(L"ControlsContainerGroup.CtrlPadLeft");
    g_S.containers[2].padRight =
        Wh_GetIntSetting(L"ControlsContainerGroup.CtrlPadRight");
    g_S.containers[2].padTop = 0;
    g_S.containers[2].padBottom = 0;
    g_S.containers[3].padLeft =
        Wh_GetIntSetting(L"VisualizerContainerGroup.VizPadLeft");
    g_S.containers[3].padRight =
        Wh_GetIntSetting(L"VisualizerContainerGroup.VizPadRight");
    ...
    if (themeStr) {
        if (!wcscmp(themeStr, L"windows"))
            g_S.background = 0;
        else if (!wcscmp(themeStr, L"transparent"))
            g_S.background = 1;
        else if (!wcscmp(themeStr, L"blurred"))
            g_S.background = 2;
        else if (!wcscmp(themeStr, L"gradient"))
            g_S.background = 3;
        else if (!wcscmp(themeStr, L"gradient_lr"))
            g_S.background = 4;
        else if (!wcscmp(themeStr, L"gradient_rl"))
            g_S.background = 12;
        else if (!wcscmp(themeStr, L"gradient_vertical"))
            g_S.background = 5;
        else if (!wcscmp(themeStr, L"gradient_radial"))
            g_S.background = 6;
        else if (!wcscmp(themeStr, L"gradient_vertical_inv"))
            g_S.background = 7;
        else if (!wcscmp(themeStr, L"split"))
            g_S.background = 9;
        else if (!wcscmp(themeStr, L"neon"))
            g_S.background = 10;
        else if (!wcscmp(themeStr, L"aurora"))
            g_S.background = 11;
        else if (!wcscmp(themeStr, L"album_blur"))
            g_S.background = 13;
        else
            g_S.background = 4;
    ...
    g_S.vizEQ = 0;
    if (vizEQStr) {
        if (!wcscmp(vizEQStr, L"bass"))
            g_S.vizEQ = 1;
        else if (!wcscmp(vizEQStr, L"rock"))
            g_S.vizEQ = 2;
        else if (!wcscmp(vizEQStr, L"pop"))
            g_S.vizEQ = 3;
        else if (!wcscmp(vizEQStr, L"jazz"))
            g_S.vizEQ = 4;
        else if (!wcscmp(vizEQStr, L"electronic"))
            g_S.vizEQ = 5;
    ...
    // Shape
    PCWSTR vizShapeStr =
        Wh_GetStringSetting(L"VisualizerContainerGroup.VizShape");
    g_S.vizShape = 0;
    if (vizShapeStr) {
        if (!wcscmp(vizShapeStr, L"mountain"))
            g_S.vizShape = 1;
        else if (!wcscmp(vizShapeStr, L"mirror"))
            g_S.vizShape = 2;
        else if (!wcscmp(vizShapeStr, L"wave"))
            g_S.vizShape = 3;
        else if (!wcscmp(vizShapeStr, L"breathe"))
            g_S.vizShape = 4;
    ...
    // Mode
    PCWSTR vizModeStr =
        Wh_GetStringSetting(L"VisualizerContainerGroup.VizMode");
    g_S.vizMode = 0;
    if (vizModeStr) {
        if (!wcscmp(vizModeStr, L"dynamic_album"))
            g_S.vizMode = 1;
        else if (!wcscmp(vizModeStr, L"dynamic_gradient"))
            g_S.vizMode = 2;
        else if (!wcscmp(vizModeStr, L"custom_gradient"))
            g_S.vizMode = 3;
        else if (!wcscmp(vizModeStr, L"acrylic"))
            g_S.vizMode = 4;
    ...
    // Anchor
    PCWSTR vizAnchorStr =
        Wh_GetStringSetting(L"VisualizerContainerGroup.VizAnchor");
    g_S.vizAnchor = 1;
    if (vizAnchorStr) {
        if (!wcscmp(vizAnchorStr, L"top"))
            g_S.vizAnchor = 0;
        else if (!wcscmp(vizAnchorStr, L"bottom"))
            g_S.vizAnchor = 2;
        Wh_FreeStringSetting(vizAnchorStr);
    }

    g_S.vizColor = ReadHexColor(L"VisualizerContainerGroup.VizColor", 0xFFFFFF);
    g_S.vizColor1 =
        ReadHexColor(L"VisualizerContainerGroup.VizColor1", 0x1ED760);
    g_S.vizColor2 =
        ReadHexColor(L"VisualizerContainerGroup.VizColor2", 0x00B4FF);
    ...
    if (g_S.vizBarWidth < 0)
        g_S.vizBarWidth = 0;
    if (g_S.vizBarWidth > 64)
        g_S.vizBarWidth = 64;

    g_S.idleBarSize = Wh_GetIntSetting(L"VisualizerContainerGroup.IdleBarSize");
    if (g_S.idleBarSize < 0)
        g_S.idleBarSize = 0;
    if (g_S.idleBarSize > 100)
        g_S.idleBarSize = 100;

    g_S.vizSensitivity =
        Wh_GetIntSetting(L"VisualizerContainerGroup.VizSensitivity");
    if (g_S.vizSensitivity < 0)
        g_S.vizSensitivity = 0;
    if (g_S.vizSensitivity > 100)
        g_S.vizSensitivity = 100;

    // Style
    g_S.roundedCorners =
        Wh_GetIntSetting(L"AppearanceGroup.RoundedCorners") != 0;
    g_S.cornerRadius = Wh_GetIntSetting(L"AppearanceGroup.CornerRadius");
    if (g_S.cornerRadius < 2)
        g_S.cornerRadius = 2;
    if (g_S.cornerRadius > 32)
        g_S.cornerRadius = 32;
    g_S.showBorder = Wh_GetIntSetting(L"AppearanceGroup.ShowBorder") != 0;
    g_S.showContainerBorders =
    ...
    g_S.manualText = ReadHexColor(L"InfoContainerGroup.TextColor", 0xFFFFFF);

    g_S.hoverColor =
        ReadHexColor(L"ControlsContainerGroup.HoverColor", 0x1ED760);

    // Icon
    g_S.iconSize = Wh_GetIntSetting(L"ControlsContainerGroup.IconSize");
    g_S.buttonSpacing =
        Wh_GetIntSetting(L"ControlsContainerGroup.ButtonSpacing");
    if (g_S.iconSize < 6)
        g_S.iconSize = 6;
    if (g_S.iconSize > 28)
        g_S.iconSize = 28;
    if (g_S.buttonSpacing < 18)
        g_S.buttonSpacing = 18;
    if (g_S.buttonSpacing > 60)
        g_S.buttonSpacing = 60;

    PCWSTR iconThemeStr =
        Wh_GetStringSetting(L"ControlsContainerGroup.IconTheme");
    g_S.iconTheme = 0;
    if (iconThemeStr) {
        if (!wcscmp(iconThemeStr, L"mdl2"))
            g_S.iconTheme = 1;
        else if (!wcscmp(iconThemeStr, L"fluent"))
            g_S.iconTheme = 2;
        else if (!wcscmp(iconThemeStr, L"fluent_filled"))
            g_S.iconTheme = 3;
        else if (!wcscmp(iconThemeStr, L"fluent_outline2"))
            g_S.iconTheme = 4;
    ...
    if (g_S.maskOpacity < 0)
        g_S.maskOpacity = 0;
    if (g_S.maskOpacity > 255)
        g_S.maskOpacity = 255;
    g_S.themeOpacity = Wh_GetIntSetting(L"AppearanceGroup.ThemeOpacity");
    if (g_S.themeOpacity < 0)
        g_S.themeOpacity = 0;
    if (g_S.themeOpacity > 255)
        g_S.themeOpacity = 255;
    g_S.themeBlur = Wh_GetIntSetting(L"AppearanceGroup.ThemeBlur");
    if (g_S.themeBlur < 0)
        g_S.themeBlur = 0;
    if (g_S.themeBlur > 100)
        g_S.themeBlur = 100;
    ...
    if (g_S.scrollSpeed < 0)
        g_S.scrollSpeed = 0;
    if (g_S.scrollSpeed > 2)
        g_S.scrollSpeed = 2;

    // Visualizer container width override
    g_S.vizContainerWidth =
        Wh_GetIntSetting(L"VisualizerContainerGroup.VizWidth");
    if (g_S.vizContainerWidth < 0)
        g_S.vizContainerWidth = 0;

    // Focused app
    PCWSTR focusApp = Wh_GetStringSetting(L"BehaviorGroup.FocusedApp");
    if (focusApp && wcscmp(focusApp, L"all") != 0) {
        if (!wcscmp(focusApp, L"applemusic"))
            g_S.focusedApp = L"applemusic";
        else if (!wcscmp(focusApp, L"ytmusic"))
            g_S.focusedApp = L"ytmusic";
        else
            g_S.focusedApp = focusApp;
    } else {
        g_S.focusedApp = L"";
    }
    if (focusApp)
        Wh_FreeStringSetting(focusApp);
    ...
    g_S.progressBarHeight =
        Wh_GetIntSetting(L"ProgressBarGroup.ProgressBarHeight");
    if (g_S.progressBarHeight < 1)
        g_S.progressBarHeight = 1;
    if (g_S.progressBarHeight > 6)
        g_S.progressBarHeight = 6;
    g_S.progressBarDynamic =
        Wh_GetIntSetting(L"ProgressBarGroup.ProgressBarDynamic") != 0;

    g_S.progressBarColor =
        ReadHexColor(L"ProgressBarGroup.ProgressBarColor", 0x1ED760);

    g_S.progressBarPadLeft =
        Wh_GetIntSetting(L"ProgressBarGroup.ProgressBarPadLeft");
    g_S.progressBarPadRight =
        Wh_GetIntSetting(L"ProgressBarGroup.ProgressBarPadRight");
    g_S.progressBarOffsetY =
        Wh_GetIntSetting(L"ProgressBarGroup.ProgressBarOffsetY");
    if (g_S.progressBarPadLeft < 0)
        g_S.progressBarPadLeft = 0;
    if (g_S.progressBarPadLeft > 80)
        g_S.progressBarPadLeft = 80;
    if (g_S.progressBarPadRight < 0)
        g_S.progressBarPadRight = 0;
    if (g_S.progressBarPadRight > 80)
        g_S.progressBarPadRight = 80;
    if (g_S.progressBarOffsetY < -20)
        g_S.progressBarOffsetY = -20;
    if (g_S.progressBarOffsetY > 20)
        g_S.progressBarOffsetY = 20;

    PCWSTR pbLayerStr =
        Wh_GetStringSetting(L"ProgressBarGroup.ProgressBarLayer");
    g_S.progressBarLayer = 0;
    if (pbLayerStr) {
        if (!wcscmp(pbLayerStr, L"above_both"))
            g_S.progressBarLayer = 1;
        else if (!wcscmp(pbLayerStr, L"under_text"))
            g_S.progressBarLayer = 2;
        else if (!wcscmp(pbLayerStr, L"under_text_only"))
            g_S.progressBarLayer = 3;
        else if (!wcscmp(pbLayerStr, L"under_controls_only"))
            g_S.progressBarLayer = 4;
...
// OPT: moved up so twiddle/Hann arrays can use FFT_SIZE before
// CaptureThreadProc
...
// ──────────────────────────────────────── Avoids cosf/sinf inside the
// butterfly loop (~10240 trig calls per FFT pass).
...
// ─────────────────────────────────────────────── Avoids repeated RegGetValueW
// kernel trips inside DrawPanel. Re-checked every 5 seconds; negligible
// staleness for a theme toggle.
...
static bool IsLightCached() {
    ...
    if (now - g_CachedIsLightTick > 5000) {
        ...
        g_CachedIsLight = (RegGetValueW(HKEY_CURRENT_USER,
                                        ...
                                        L"Version\\Themes\\Personalize",
                                        L"SystemUsesLightTheme", RRF_RT_DWORD,
...
// ───────────────────────────────────────────────── CalcLayout was called in
// WM_MOUSEMOVE (every pixel), WM_LBUTTONUP, WM_MOUSEWHEEL, and DrawPanel.
// Results only change on settings/resize.
struct CachedLayout {
    ControlLayout ctrl;
    int W = 0, H = 0;
    int aX = 0, aY = 0, aS = 0, aW = 0,
        aR = 0;  // art: x, y, height, width, corner-radius
...
static void RebuildLayout(int W, int H) {
    ...
    if (g_S.mediaWidth > 0) {
        ...
        // Scale to native art aspect ratio, clamped to 3:1 max to prevent
        // absurdly wide panels
        float ratio = max(0.25f, min(3.0f, g_ArtAspectRatio));
    ...
    // Corner radius clamped to the shorter of width and height so it never
    // exceeds either side
    ...
    int vCellX = aX,
        vCellW =
            aW;  // default: same cell as art (width not necessarily square)
    ...
    // Responsive: if Controls container is disabled, Visualizer shifts
    // automatically because the order vector already skips disabled containers
    // - no extra logic needed.

    // Compute bar zone from the visualizer cell
    // padLeft/padRight for visualizer container accessed via containers[3] in
    // DrawPanel
    ...
    g_Layout.barW =
        max(1.5f, (barZoneW - gap * (barCount - 1)) / (float)barCount);
    g_Layout.barStep = g_Layout.barW + gap;
    g_Layout.startX =
        barZoneX + (barZoneW - (g_Layout.barStep * barCount - gap)) * 0.5f;
    ...
    // ─────────────────────────────────────────────── Find the Controls
    // container x-origin so CalcLayout can anchor correctly.
    int ctrlCellX = W - g_S.wrapper.padRight;  // fallback
    for (int i = 0; i < g_OriginCount; i++)
        if (g_Origins[i].id == 3) {
            ctrlCellX = g_Origins[i].cellX;
            break;
        }
    ...
    g_Layout.ctrl = CalcLayout(
        W, H, effectivePadRight, g_S.buttonSpacing, g_S.showSpeakerIcon,
        g_S.showPlaybackControls, g_S.showShuffleButton, g_S.showRepeatButton,
        g_S.wrapper.padTop, g_S.wrapper.padBottom, ctrlCellX,
        g_S.containers[2].padLeft, g_S.containers[2].padRight, ctrlCellRight);
...
// ────────────────────────────────────────────────── IDT_POLL slows to 4 s when
// paused; IDT_POS is killed entirely when paused.
...
// ───────────────────────────── OPT: butterfly now indexes pre-baked twiddle
// tables (g_TwiddleRe/Im) instead of calling cosf/sinf per stage - eliminates
// ~10 240 trig calls per FFT pass.
...
// Event handle for WASAPI event-driven capture (wakes thread only when data
// arrives)
...
static void BuildHannWindow() {
    for (int i = 0; i < FFT_SIZE; i++)
        g_HannWindow[i] =
            0.5f * (1.f - cosf(2.f * 3.14159265f * i / (FFT_SIZE - 1)));
}
...
static void CaptureThreadProc() {
...

    {
        ...
        if (SUCCEEDED(pDev->Activate(__uuidof(IAudioClient), CLSCTX_ALL,
            ...
            if (pwfx) {
                ...
                // Use event-callback flag - SetEventHandle must be called
                // before Initialize
                if (SUCCEEDED(
    ...
    // Per-band envelope - gravity-based decay: constant speed fall, instant
    // attack This matches how professional VU meters behave.
    float bandEnv[NUM_BANDS] = {};
    static const float GRAVITY[NUM_BANDS] = {0.018f, 0.020f, 0.022f, 0.025f,
    ...
    while (g_CaptureRunning.load(std::memory_order_relaxed)) {
        ...
        if (FAILED(pCapture->GetNextPacketSize(&packetSize)) ||
            packetSize == 0) {
            // No data this wake - apply gravity decay and publish zeros
            // movement
            for (int b = 0; b < NUM_BANDS; b++) {
        ...
        while (ringCount >= FFT_SIZE) {
            ...
            for (int b = 0; b < NUM_BANDS; b++) {
                ...
                // RMS across the band - smoother and more accurate than
                // peak-pick
                float sumSq = 0.f;
                int count = 0;
                ...
                float eqM = (BAND_EQ_ZONE[b] == 0)   ? lowM
                            : (BAND_EQ_ZONE[b] == 1) ? midM
                                                     : highM;
                float mag = max(
...
// ───────────────── Fully dynamic: remaps NUM_BANDS (7) FFT bands onto
// g_S.vizBars bars using a fractional interpolation so every bar count from
// 1-20 looks correct. EQ is applied per-bar based on its frequency position
// (0=low, 1=high).
static void UpdateVisualizerPeaks() {
    ...
    // Interpolate a value from the bands[] array at fractional position t in
    // [0,1] t=0 = sub-bass (band 0), t=1 = air (band 6)
    ...
    for (int i = 0; i < vizBars; i++) {
        ...
        switch (g_S.vizShape) {
            ...
            case 1: {  // Mountain: center=bass, outer=highs, tapered
                ...
                // sampleBands(dist): center samples t=0 (sub-bass), edges
                // sample t=1 (air)
            ...
            case 2: {  // Mirror: symmetric, edges=low, center=high
                float mirrorT = 1.f - fabsf((float)i - center) /
                                          max(1.f, center);  // 0=edge, 1=center
                float energy = sampleBands(mirrorT) * eqForT(mirrorT);
                target = max(
                    0.f,
                    min(1.f, (energy + masterPeak * (0.1f + mirrorT * 0.12f)) *
                                 1.3f));
                break;
            }
            ...
            case 4: {  // Breathe: all bars pulse in unison with per-bar
                       // variation
                ...
                float inhale =
                    0.5f + 0.5f * sinf(t * breathRate +
                                       VIZ_SEEDS[i % VIZ_BARS_MAX] * 1.2f);
                target =
                    max(0.f, min(1.f, inhale * (0.12f + s_breatheEnv * 0.88f)));
                break;
            }
...
// ─────────────────────────────────────────────── GSMTC only fires position
// updates ~once per second. We interpolate between polls using a
// high-resolution tick so the bar moves smoothly every frame.
...
DWORD g_PosUpdateTick =
    0;  // GetTickCount() when last raw position was captured
...
// Cached volume interface - avoids re-enumerating all audio sessions every
// tick. Invalidated when the AUMID changes or when a call on the pointer fails.
...
static com_ptr<ISimpleAudioVolume> FindAppVolumeInterface(
    ...
    if (FAILED(pEnumerator->GetDefaultAudioEndpoint(eRender, eConsole,
                                                    pDevice.put())))
        return nullptr;
    ...
    if (FAILED(pSessionManager->GetSessionEnumerator(pSessionEnumerator.put())))
        return nullptr;
    ...
    transform(lowerAumid.begin(), lowerAumid.end(), lowerAumid.begin(),
              ::towlower);

    for (int i = 0; i < sessionCount; i++) {
        com_ptr<IAudioSessionControl> pSessionControl;
        if (FAILED(pSessionEnumerator->GetSession(i, pSessionControl.put())))
            continue;

        com_ptr<IAudioSessionControl2> pSessionControl2 =
            pSessionControl.as<IAudioSessionControl2>();
        if (!pSessionControl2)
            continue;
        ...
        bool matched = false;
        if (QueryFullProcessImageNameW(hProcess, 0, processImagePath,
                                       &pathSize)) {
            wstring wPath = processImagePath;
            size_t lastSlash = wPath.find_last_of(L"\\");
            wstring exeName = (lastSlash == wstring::npos)
                                  ? wPath
            ...
            transform(exeName.begin(), exeName.end(), exeName.begin(),
                      ::towlower);
            matched = (lowerAumid.find(exeName) != wstring::npos ||
                       exeName.find(lowerAumid) != wstring::npos);
        }
...
// Get the cached volume pointer, re-enumerating if the app changed or the
// pointer went stale.
static com_ptr<ISimpleAudioVolume> GetVolume(bool forceRefresh = false) {
    ...
    if (forceRefresh || g_cachedVolumeAumid != g_currentMediaAppAumid ||
        !g_cachedVolume) {
...
void SetVolume(float level) {
    ...
    if (FAILED(pVol->SetMasterVolume(g_Volume, NULL))) {
        ...
        pVol = GetVolume(/*forceRefresh=*/true);
        if (pVol)
            pVol->SetMasterVolume(g_Volume, NULL);
    }
}
...
static void ApplyTimerRates(HWND hwnd) {
    ...
    SetTimer(hwnd, IDT_POLL, playing ? POLL_RATE_PLAYING : POLL_RATE_PAUSED,
             NULL);
    if (playing) {
...
static void UpdateAlbumBlurBg(Bitmap* art, int w, int h, int artVersion) {
    if (!art || w <= 0 || h <= 0) {
        ...
        g_S.blurBgW = g_S.blurBgH = 0;
        g_S.blurArtVersion = -1;
        return;
    }
    if (g_S.blurredBg && g_S.blurBgW == w && g_S.blurBgH == h &&
        g_S.blurArtVersion == artVersion)
        return;
...
void FetchMedia() {
    try {
        ...
        // 1. Priority: Find the session that is actually PLAYING (e.g. YouTube
        // while Spotify is paused)
        ...
        if (!g_S.focusedApp.empty()) {
            ...
            transform(lowerFocus.begin(), lowerFocus.end(), lowerFocus.begin(),
                      ::towlower);
            if (lowerFocus.size() > 4 &&
                lowerFocus.substr(lowerFocus.size() - 4) == L".exe")
                lowerFocus = lowerFocus.substr(0, lowerFocus.size() - 4);
            for (auto const& s : sessions) {
                try {
                    wstring aumid = s.SourceAppUserModelId().c_str();
                    transform(aumid.begin(), aumid.end(), aumid.begin(),
                              ::towlower);
                    if (aumid.find(lowerFocus) != wstring::npos) {
                        ses = s;
                        break;
                    }
                } catch (...) {
        ...
        // 2. Fallback: If nothing is playing, grab the last active session
        // (paused Spotify/browser)
        if (!ses)
            try {
        ...
        if (ses) {
            ...
            try {
                ...
                // If fetching properties fails (app closing), just set playing
                // to false and keep last info
                scoped_lock lk(g_M.mtx);
                g_M.playing = false;
                return;
            }
            ...
            if (nt != g_M.title || !g_M.art) {
                // Trigger text crossfade when track title changes (not on first
                // load)
                if (nt != g_M.title && g_M.title != L"No Media") {
                    ...
                    g_TextFadeOut = true;
                    if (g_hWnd)
                        SetTimer(g_hWnd, IDT_TEXT_FADE, 16, NULL);
                }
                g_M.art.reset();
                try {
                ...
                ++g_ArtVersion;  // OPT: bump version so blur cache knows art
                                 // changed
                ...
                if (g_M.art) {
                    ...
                    g_ArtAspectRatio =
                        (artNatH > 0) ? (float)artNatW / (float)artNatH : 1.0f;
                } else {
                    g_ArtAspectRatio = 1.0f;  // no art → square placeholder
                }
                // Layout must rebuild because aW may have changed (panel width
                // changes)
                g_LayoutDirty = true;
            ...
            try {
                ...
                g_M.playing =
                    (pbi.PlaybackStatus() ==
                     GlobalSystemMediaTransportControlsSessionPlaybackStatus::
                         Playing);
                // Keep live progress tracker in sync with play/pause
                // transitions
            ...
            try {
                ...
                if (newPos != g_PosLastRaw || newEnd != g_PosEndRaw ||
                    isNowPlaying != g_PosIsPlaying) {
                    g_PosLastRaw = newPos;
                    g_PosEndRaw = newEnd;
                    g_PosUpdateTick = GetTickCount();
            ...
            // No session: keep last track info visible (idle screen overrides
            // in DrawPanel if enabled)
            scoped_lock lk(g_M.mtx);
            g_M.playing = false;
            g_M.hasMedia = false;
...
// ──────────────────────────────────────────── Draws a quadrilateral with four
// independently sized rounded corners using GDI+ arcs. Each radius can be 0
// (sharp corner) up to 32px. rTL=top-left, rTR=top-right, rBR=bottom-right,
// rBL=bottom-left.
void CustomRoundedPath(GraphicsPath& p,
                       float x,
                       float y,
                       float w,
                       float h,
...
void ApplyAppearance(HWND hwnd) {
    DWM_WINDOW_CORNER_PREFERENCE cp =
        g_S.roundedCorners ? DWMWCP_ROUND : DWMWCP_DONOTROUND;
    DwmSetWindowAttribute(hwnd, DWMWA_WINDOW_CORNER_PREFERENCE, &cp,
                          sizeof(cp));

    COLORREF borderCol =
        g_S.showBorder ? DWMWA_COLOR_DEFAULT : DWMWA_COLOR_NONE;
    ...
    if (g_S.background == 1 || (g_S.background >= 3 && g_S.background <= 7) ||
        g_S.background == 9 || g_S.background == 11 || g_S.background == 12 ||
        g_S.background == 13) {
        ...
        if (g_S.background == 0) {
            ...
            // Unified acrylic-glass: themeBlur controls tint density,
            // themeOpacity scales overall alpha. Set ThemeBlur low +
            // ThemeOpacity high for a frosted-glass look; ThemeBlur high +
            // ThemeOpacity lower for a dense acrylic look.
            bool light = g_S.autoTheme && IsLightCached();
            BYTE blurAlpha =
                (BYTE)(max(8, min(180, (int)(g_S.themeBlur * 1.4f))));
...
bool IsTaskbarEffectivelyVisible(HWND hTaskbar) {
    ...
    // Autohidden taskbar leaves a 1-2px sliver; 4px threshold is safe at any
    // DPI
...
void UpdateBoundsAndRegion(HWND hwnd) {
    ...
    int panelW;
    if (g_S.miniMode) {
        // Mini: wrapper left+right + active container widths + gap if both
        // shown
        ...
        for (int id : {1, 4}) {  // Only Media and Visualizer in mini
            if (!IsContainerEnabled(id))
                continue;
            if (!first)
                totalWidth += g_S.wrapper.globalGap;
    ...
    if (g_S.roundedCorners)
        hRgn = CreateRoundRectRgn(0, 0, panelW + 1, panelH + 1, cr * 2, cr * 2);
    else
...
void DrawPanel(HDC hdc, int W, int H) {
    ...
    // (All existing background cases preserved verbatim - only showing the
    // switch here)
    if (g_S.background == 3) {
        Color left(255, bgPrimary.GetR(), bgPrimary.GetG(), bgPrimary.GetB());
        Color right(0, bgPrimary.GetR(), bgPrimary.GetG(), bgPrimary.GetB());
        LinearGradientBrush br(Rect(0, 0, W, H), left, right,
                               LinearGradientModeHorizontal);
        FillShape(&br);
    } else if (g_S.background == 4) {
        LinearGradientBrush br(Rect(0, 0, W, H), bgPrimary, bgSecondary,
                               LinearGradientModeHorizontal);
        FillShape(&br);
    } else if (g_S.background == 12) {
        LinearGradientBrush br(Rect(0, 0, W, H), bgSecondary, bgPrimary,
                               LinearGradientModeHorizontal);
        FillShape(&br);
        ...
        BYTE neonAlpha =
            (BYTE)(max(10, (int)(40.f * g_S.themeOpacity / 255.f)));
        Color glassOverlay = IsLightCached() ? Color(neonAlpha, 255, 255, 255)
        ...
        if (g_S.roundedCorners) {
            GraphicsPath bp;
            CustomRoundedPath(
                bp, 1.f, 1.f, (float)(W - 2), (float)(H - 2),
        ...
        LinearGradientBrush br(Rect(0, 0, W, H), bgPrimary, bgPrimary,
                               LinearGradientModeHorizontal);
        Color darkPrimary(255, max(0, (int)bgPrimary.GetR() - 50),
    ...
    if (g_S.background == 13) {
        ...
        if (g_S.enableMask) {
            ...
            if (g_S.roundedCorners) {
                ...
                g.FillRectangle(&maskBr, 0, 0, W, H);
        }
    }
    ...
    Color artistColor(min((int)panelAlpha, 153), artistBase, artistBase,
                      artistBase);
    ...
    auto LerpDWORD = [](DWORD c1, DWORD c2, float t) -> Color {
        ...
        return Color(255, (BYTE)(r1 + (r2 - r1) * t),
                     (BYTE)(g1 + (g2 - g1) * t), (BYTE)(b1 + (b2 - b1) * t));
    };
    ...
    if (g_S.dynamicHover)
        ...
        targetHoverColor =
            Color(255, (g_S.hoverColor >> 16) & 0xFF,
                  (g_S.hoverColor >> 8) & 0xFF, g_S.hoverColor & 0xFF);
    ...
    if (g_S.ctrlIconDynamic)
        ...
        ctrlBaseColor = Color(255, (BYTE)((g_S.ctrlIconColor >> 16) & 0xFF),
    ...
    auto IB = [&](int btn) -> Color {
        ...
        if (t <= 0.f)
            return ctrlBaseColor;
        if (t >= 1.f)
            return targetHoverColor;
    ...
    if (g_S.showVisualizer && g_S.wrapper.vizAsBackground && !g_S.miniMode) {
        // Full-panel background layer: same geometry as art square but
        // stretched
        int barCount = max(1, g_S.vizBars);
        float idleH = g_S.idleBarSize / 100.f;
        Color baseBarCol;
        if (g_S.vizMode == 1)
            baseBarCol = Color(panelAlpha, bgPrimary.GetR(), bgPrimary.GetG(),
                               bgPrimary.GetB());
        else
            baseBarCol = Color(panelAlpha, (BYTE)((g_S.vizColor >> 16) & 0xFF),
        ...
        for (int i = 0; i < barCount; i++) {
            ...
            float bh = (bgZoneH * 0.035f) +
                       barFactor * (bgZoneH * 0.92f - bgZoneH * 0.035f);
            float bx = bgStartX + i * bgStep;
            float by;
            if (g_S.vizAnchor == 0)
                by = bgZoneY;
            else if (g_S.vizAnchor == 2)
                by = bgZoneY + bgZoneH - bh;
            else
                by = bgZoneY + (bgZoneH - bh) / 2.f;
            ...
            if (g_S.vizMode == 3) {
                ...
                loopBrush.SetColor(
                    Color(panelAlpha, gc.GetR(), gc.GetG(), gc.GetB()));
            } else {
    ...
    for (int ci = 0; ci < g_OriginCount; ci++) {
        ...
        if (co.id == 1) {
            GraphicsPath ap;
            CustomRoundedPath(ap, (float)aX, (float)aY, (float)aW, (float)aS,
                              aR, aR, aR, aR);
            ...
            auto DrawArtOrPlaceholder = [&]() {
                if (art) {
                    ...
                    Color phBg = _light ? Color(panelAlpha, 200, 200, 200)
                                        : Color(panelAlpha, 55, 55, 60);
                    Color phFg = _light ? Color(panelAlpha, 130, 130, 135)
                                        : Color(panelAlpha, 100, 100, 108);
                    SolidBrush bgBr(phBg);
                    g.FillPath(&bgBr, &ap);
                    int minDim = min(aW, aS);
                    if (minDim >= 14) {
                        FontFamily noteFF(ICON_FONT, nullptr);
                        Font noteF(&noteFF, (REAL)(minDim * 0.45f),
                                   FontStyleRegular, UnitPixel);
                        RectF noteBounds(0, 0, 4000, 200), noteMeas;
                        const wchar_t* noteGlyph = L"\uE8D6";
                        g.MeasureString(noteGlyph, -1, &noteF, noteBounds,
                                        &noteMeas);
                        float nX2 = aX + (aW - noteMeas.Width) / 2.f;
                        float nY2 = aY + (aS - noteMeas.Height) / 2.f;
                        SolidBrush fgBr(phFg);
                        g.DrawString(noteGlyph, -1, &noteF, PointF(nX2, nY2),
                                     &fgBr);
                    }
                }
            };
        ...
        if (co.id == 4) {
            ...
            // In normal mode, skip if it's set to the full-width background
            // layer.
            if (!g_S.showVisualizer)
                continue;
            if (!g_S.miniMode && g_S.wrapper.vizAsBackground)
                continue;
            ...
            Color baseBarCol;
            if (g_S.vizMode == 1)
                baseBarCol = Color(panelAlpha, bgPrimary.GetR(),
                                   bgPrimary.GetG(), bgPrimary.GetB());
            else
                baseBarCol =
                    Color(panelAlpha, (BYTE)((g_S.vizColor >> 16) & 0xFF),
                          (BYTE)((g_S.vizColor >> 8) & 0xFF),
                          (BYTE)(g_S.vizColor & 0xFF));
            ...
            for (int i = 0; i < barCount; i++) {
                ...
                if (g_S.vizAnchor == 0)
                    ...
                    by = barZoneY + barZoneH - bh;
                else
                    by = barZoneY + (barZoneH - bh) / 2.f;
                ...
                if (g_S.vizMode == 2) {
                    ...
                    Color gc(
                        panelAlpha,
                        (BYTE)(bgPrimary.GetR() +
                               (int)(bgSecondary.GetR() - bgPrimary.GetR()) *
                                   t),
                        (BYTE)(bgPrimary.GetG() +
                               (int)(bgSecondary.GetG() - bgPrimary.GetG()) *
                                   t),
                        (BYTE)(bgPrimary.GetB() +
                               (int)(bgSecondary.GetB() - bgPrimary.GetB()) *
                                   t));
                    ...
                    BYTE acrAlpha = (BYTE)(max(
                        30,
                        min(180, (int)(panelAlpha * 0.55f * barFactor + 30))));
                    Color ac(acrAlpha, bgPrimary.GetR(), bgPrimary.GetG(),
                             bgPrimary.GetB());
        ...
        if (co.id == 2 && !g_S.miniMode) {
            ...
            // artTextGap shrinks tW from the left so the gap between art and
            // text is respected without shifting the text's starting position.
            ...
            Font titleF(&ff, (REAL)(g_S.fontSize + 1), FontStyleBold,
                        UnitPixel);
            Font artF(&artFF, (REAL)(g_S.fontSize - 1), FontStyleRegular,
                      UnitPixel);

            RectF lay(0, 0, 4000, 200), tb2, ab2;
            g.MeasureString(title.c_str(), -1, &titleF, lay, &tb2);
            if (!artist.empty())
                g.MeasureString(artist.c_str(), -1, &artF, lay, &ab2);

            float gap = (float)g_S.textGap;
            float totalH =
                tb2.Height + (artist.empty() ? 0.f : ab2.Height + gap);

            // Reserve vertical space for progress bar only when it draws
            // inside the info container area (under_text=2, under_text_only=3).
            // under_both(0), above_both(1), under_controls_only(4) don't affect
            // text position.
            float pbReserve = 0.f;
            ...
            float sY = g_S.wrapper.padTop +
                       (H - g_S.wrapper.padTop - g_S.wrapper.padBottom -
                        totalH - pbReserve) /
                           2.f;
            ...
            wstring displayTitle = (g_S.disableScroll && g_ScTxtW > tW)
                                       ? TruncateToFit(title, titleF, tW)
                                       : title;
            RectF ab2_artist;  // re-measure artist for layout
            if (!artist.empty())
                g.MeasureString(artist.c_str(), -1, &artF,
                                RectF(0, 0, 4000, 200), &ab2_artist);
            wstring displayArtist = (g_S.disableScroll && !artist.empty() &&
                                     (int)ab2_artist.Width > tW)
                                        ? TruncateToFit(artist, artF, tW)
                                        : artist;

            Color tcFaded(max(0, (int)(tc.GetA() * g_TextFadeAlpha)), tc.GetR(),
                          tc.GetG(), tc.GetB());
            Color acFaded(max(0, (int)(artistColor.GetA() * g_TextFadeAlpha)),
                          artistColor.GetR(), artistColor.GetG(),
                          artistColor.GetB());
            ...
            if (g_S.showProgressBar) {
                ...
                if (g_S.progressBarDynamic) {
                    pbFillCol = Color(220, bgPrimary.GetR(), bgPrimary.GetG(),
                                      bgPrimary.GetB());
                } else {
                    pbFillCol =
                        Color(220, (BYTE)((g_S.progressBarColor >> 16) & 0xFF),
                ...
                switch (g_S.progressBarLayer) {
                    ...
                    case 3:  // under text box only
                        ...
                        pbW = tW - g_S.progressBarPadLeft -
                              g_S.progressBarPadRight;
                        pbY = H - pbH - 4 - g_S.progressBarOffsetY;
                        break;
                    case 4:  // under controls only
                        pbX = (int)(firstControlX - pnSmallR) +
                              g_S.progressBarPadLeft;
                        pbW = (int)(nX + pnSmallR) - pbX -
                              g_S.progressBarPadRight;
                        pbY = H - pbH - 4 - g_S.progressBarOffsetY;
                        break;
                    default:  // under_both
        ...
        if (co.id == 3 && !g_S.miniMode) {
            ...
            for (int i = 1; i <= 6; i++) {
                ...
                SolidBrush hb(Color(alpha, ctrlBaseColor.GetR(),
                                    ctrlBaseColor.GetG(),
                                    ctrlBaseColor.GetB()));
                float animR = hR * (0.8f + 0.2f * g_HoverProgress[i]);
                g.FillEllipse(&hb, hX - animR, cY - animR, animR * 2.f,
                              animR * 2.f);
            }

            // Icon drawing helpers (same lambdas as original)
            auto DrawIcon = [&](int type, float cx, float cy, float s,
                                Color col) {
                SolidBrush b(col);
                ...
                if (type == 1) {
                    ...
                    g.DrawArc(&p2, X(-5.5f), Y(-4.f), W_f(8.f), W_f(8.f), -50,
                              100);
                    g.DrawArc(&p2, X(-8.5f), Y(-7.f), W_f(14.f), W_f(14.f), -50,
                              100);
                }
            };
            ...
            if (g_S.iconTheme >= 1 && g_S.iconTheme <= 4) {
                ...
                if (g_Volume <= 0.f)
                    glyphVolume = L"\uE74F";
                else if (g_Volume < 0.34f)
                    glyphVolume = L"\uE993";
                else if (g_Volume < 0.67f)
                    glyphVolume = L"\uE994";
                else
                    glyphVolume = L"\uE995";
                ...
                if (g_S.showSpeakerIcon)
                    DrawGlyph(glyphVolume, vX, IB(4));

                // Shuffle / Repeat glyphs - codepoints per icon theme
                const wchar_t *glyphShuffle, *glyphRepeatNone,
                    *glyphRepeatTrack, *glyphRepeatList;
                if (g_S.iconTheme == 3) {
                    // Segoe Fluent Icons - Filled (different codepoints for
                    // filled variants)
                    ...
                    // MDL2 Assets (1), Fluent Outlined (2), Fluent Outlined Alt
                    // (4) - same codepoints
                ...
                if (g_S.showShuffleButton && shuffleX > 0.f) {
                    ...
                    if (shOn) {
                        ...
                        g.FillEllipse(&dotBr, shuffleX - dotR, dotY - dotR,
                                      dotR * 2.f, dotR * 2.f);
                    }
                }
                if (g_S.showRepeatButton && repeatX > 0.f) {
                    const wchar_t* rg = (repMode == 1)   ? glyphRepeatTrack
                                        : (repMode == 2) ? glyphRepeatList
                                                         : glyphRepeatNone;
                    ...
                    if (repMode > 0) {
                        ...
                        float dotY = cY + 10.f * iconScale;
                        SolidBrush dotBr(targetHoverColor);
                        g.FillEllipse(&dotBr, repeatX - dotR, dotY - dotR,
                                      dotR * 2.f, dotR * 2.f);
                    }
                }
            } else {
                ...
                // ─────────────────────────────────
                if (g_S.showPlaybackControls) {
                    DrawIcon(playing ? 3 : 2, ppX, cY, 1.15f * iconScale,
                             IB(2));
            ...
            if (g_S.showSpeakerIcon) {
                if (g_S.iconTheme == 0)
                    DrawIcon(5, vX, cY, 0.9f * iconScale, IB(4));
                ...
                if (g_VolumeHover) {
                    ...
                    GraphicsPath vp;
                    RoundRect(vp, sliderBgX, sliderBgY, sliderBgW, sliderBgH,
                              sliderBgH / 2.f);
                    Color popupBg = IsLightCached() ? Color(230, 245, 245, 245)
                                                    : Color(200, 30, 30, 30);
                    ...
                    SolidBrush track(
                        Color(60, tc.GetR(), tc.GetG(), tc.GetB()));
                    GraphicsPath trackP;
                    RoundRect(trackP, sliderX, sliderTop, sliderW, sliderH,
                              sliderH / 2.f);
                    ...
                    if (filledW2 > 0) {
                        GraphicsPath fillP;
                        RoundRect(fillP, sliderX, sliderTop, filledW2, sliderH,
                                  sliderH / 2.f);
                        g.FillPath(&fill, &fillP);
                    }
                    float thumbR = 4.f, thumbX = sliderX + filledW2 - thumbR;
                    g.FillEllipse(&fill, thumbX, cY - thumbR, thumbR * 2.f,
                                  thumbR * 2.f);
                }
            }
            ...
            if (g_S.iconTheme == 0) {
                ...
                if (g_S.showShuffleButton && shuffleX > 0.f) {
                    ...
                    g.DrawLine(&shPen, shuffleX - 5.f * ss, cY + 4.f * ss,
                               shuffleX + 5.f * ss, cY - 4.f * ss);
                    PointF at2[] = {{shuffleX + 3.f * ss, cY - 6.f * ss},
                                    {shuffleX + 5.f * ss, cY - 4.f * ss},
                                    {shuffleX + 5.f * ss, cY - 2.f * ss}};
                    g.DrawLines(&shPen, at2, 3);
                    PointF ab3[] = {{shuffleX + 3.f * ss, cY + 6.f * ss},
                                    {shuffleX + 5.f * ss, cY + 4.f * ss},
                                    {shuffleX + 5.f * ss, cY + 2.f * ss}};
                    ...
                    if (shOn2) {
                        ...
                        SolidBrush dotBr(targetHoverColor);
                        g.FillEllipse(&dotBr, shuffleX - dotR, dotY, dotR * 2.f,
                                      dotR * 2.f);
                    }
                }
                if (g_S.showRepeatButton && repeatX > 0.f) {
                    ...
                    g.DrawArc(&repPen, repeatX - rr, cY - rr, rr * 2.f,
                              rr * 2.f, 30, 300);
                    PointF ra2[] = {{repeatX + rr * 0.5f, cY - rr},
                    ...
                    if (repMode2 == 1) {
                        ...
                        Font f1(&ff1, 5.5f * iconScale, FontStyleBold,
                                UnitPixel);
                        SolidBrush rb(repCol);
                        g.DrawString(L"1", -1, &f1,
                                     PointF(repeatX - 2.5f * iconScale,
                                            cY - 3.5f * iconScale),
                                     &rb);
                    }
                    if (repMode2 > 0) {
                        ...
                        g.FillEllipse(&dotBr, repeatX - dotR, dotY, dotR * 2.f,
                                      dotR * 2.f);
                    }
                }
            }
    ...
    // exact boundaries and spacing. Toggle with ShowContainerBorders in Panel
    // settings.
    if (g_S.showContainerBorders) {
        ...
        for (int ci = 0; ci < g_OriginCount; ci++) {
            ...
            // Also draw the inner content area (after padLeft/padRight) as a
            // dashed line
...
// ── Cached back-buffer
// ──────────────────────────────────────────────────────── Recreated only when
// panel size changes, not every frame.
...
void CALLBACK TbEvt(HWINEVENTHOOK, DWORD, HWND h, LONG, LONG, DWORD, DWORD) {
    ...
    if (pid && IsWindowVisible(h) && GetWindow(h, GW_OWNER) == nullptr) {
        ...
        if (hProc) {
            ...
            if (QueryFullProcessImageNameW(hProc, 0, path, &sz)) {
                ...
                transform(lAumid.begin(), lAumid.end(), lAumid.begin(),
                          ::towlower);
                transform(lExe.begin(), lExe.end(), lExe.begin(), ::towlower);

                if (lAumid.find(lExe) != wstring::npos ||
                    lExe.find(lAumid) != wstring::npos) {
                    fd->found = h;
                    CloseHandle(hProc);
    ...
    switch (msg) {
        case WM_CREATE:
            ...
            if (g_S.hideFullscreen)
                SetTimer(hwnd, IDT_FULLSCREEN, 100, NULL);
            HookTaskbar(hwnd);
            FetchMedia();
            // OPT: set adaptive timer rates AFTER FetchMedia so we know whether
            // playback is active; avoids spinning IDT_POS at 250 ms when paused
            // at startup.
            ApplyTimerRates(hwnd);
            if (g_S.showVisualizer) {
                StartCaptureThread();
                SetTimer(hwnd, IDT_VIZ, 16, NULL);
        ...
        case WM_APPCLOSE:
            ...
            return 0;
        case WM_DESTROY:
            // OPT: kill ALL timers before anything else - stray WM_TIMER on a
            // destroyed window is UB and can crash explorer.exe.
            ...
            // OPT: bounded capture thread join - prevents explorer hang if
            // WASAPI deadlocks.
            if (g_CaptureRunning.load()) {
                g_CaptureRunning.store(false);
                if (g_hCaptureEvent)
                ...
                if (g_CaptureThread.joinable()) {
                    ...
                    if (WaitForSingleObject(hRaw, 500) == WAIT_OBJECT_0)
                        ...
                        g_CaptureThread.detach();  // abandon - thread will exit
                                                   // on next check
                }
                if (g_hCaptureEvent) {
                    CloseHandle(g_hCaptureEvent);
            ...
            // OPT: free blurred background bitmap (was leaking on uninit
            // without art change)
            if (g_S.blurredBg) {
                delete g_S.blurredBg;
                g_S.blurredBg = nullptr;
        ...
        case WM_SETTINGCHANGE:
            ...
            UpdateBoundsAndRegion(hwnd);
            g_LayoutDirty =
                true;  // OPT: invalidate cached layout on settings change
            g_CachedIsLightTick =
                0;  // OPT: force IsLight re-check after theme change
            DestroyBackBuffer();
        ...
        case WM_TIMER:
            if (wp == IDT_POLL) {
                ...
                FetchMedia();
                // When auto-size is on, a ratio change means the panel width
                // changed — resize the window immediately so it doesn't flash
                // at the old size.
                if (g_S.mediaAutoSize && g_S.mediaEnabled &&
                    g_ArtAspectRatio != prevRatio) {
                    RecomputeLayout();
                    UpdateBoundsAndRegion(hwnd);
                }
                // Track how long we've had zero media sessions, enter idle
                // after user-defined delay
                {
                    scoped_lock lk(g_M.mtx);
                    if (g_S.idleScreenEnabled && !g_M.hasMedia) {
                        g_NoMediaSecs++;
                        if (g_NoMediaSecs >= g_S.idleScreenDelay &&
                            !g_IdleState) {
                            g_IdleState = true;
                        }
                    } else {
                ...
                if (g_S.showVisualizer) {
                    ...
                    if (!panelVisible && g_CaptureRunning.load())
                        StopCaptureThread();
                    if (panelVisible && !g_CaptureRunning.load())
                        StartCaptureThread();
                }
                ...
                if (g_FullscreenHidden) {
                    // Fullscreen hide is managed entirely by IDT_FULLSCREEN --
                    // don't touch visibility here.
                ...
                ApplyTimerRates(hwnd);  // OPT: slow poll when paused, kill
                                        // IDT_POS when not needed

            } else if (wp == IDT_FULLSCREEN) {
                // Runs every 100ms when Hide on Fullscreen is enabled.
                // SHQueryUserNotificationState is the same API Windows uses
                // internally to suppress notifications during fullscreen apps
                // and games. QUNS_BUSY = regular fullscreen app,
                // QUNS_RUNNING_D3D_FULL_SCREEN = game.
                bool isFullscreen = false;
                QUERY_USER_NOTIFICATION_STATE qs;
                if (SUCCEEDED(SHQueryUserNotificationState(&qs)))
                    isFullscreen =
                        (qs == QUNS_BUSY || qs == QUNS_RUNNING_D3D_FULL_SCREEN);

                if (isFullscreen && !g_FullscreenHidden) {
                    g_FullscreenHidden = true;
                    if (g_S.showVisualizer && g_CaptureRunning.load())
                        StopCaptureThread();
                ...
                // Poll timeline position directly every 50ms for smooth live
                // progress bar. This mirrors what taskbar-media-bar does with
                // its IDT_LYRICS timer.
                if (g_S.showProgressBar && g_Mgr) {
                    try {
                        ...
                        if (!g_currentMediaAppAumid.empty()) {
                            for (auto const& s : sessions) {
                                if (wstring(s.SourceAppUserModelId().c_str()) ==
                        ...
                        if (ses) {
                            ...
                            // Update interpolation baseline whenever position
                            // or playing state changes
                            if (newPos != g_PosLastRaw ||
                                newEnd != g_PosEndRaw ||
                                nowPlaying != g_PosIsPlaying) {
                                g_PosLastRaw = newPos;
                                g_PosEndRaw = newEnd;
                                g_PosUpdateTick = GetTickCount();
                ...
                if (g_Scroll) {
                    if (g_ScWait > 0) {
                        ...
                        int scrollStep = g_S.scrollSpeed == 0   ? 1
                                         : g_S.scrollSpeed == 1 ? 2
                                                                : 3;
                        if (g_ScDir) {
                            g_ScOff += scrollStep;
                ...
                for (int i = 1; i <= 6; i++) {
                    float target = 0.f;
                    if (i == 4)
                        target = (g_Hover == 4 || g_VolumeHover) ? 1.f : 0.f;
                    else
                        target = (g_Hover == i && !g_VolumeHover) ? 1.f : 0.f;

                    if (g_HoverProgress[i] != target) {
                        float step = 0.15f;  // Animation speed
                        if (g_HoverProgress[i] < target)
                            g_HoverProgress[i] =
                                min(target, g_HoverProgress[i] + step);
                        else
                            g_HoverProgress[i] =
                                max(target, g_HoverProgress[i] - step);
                        needsAnim = true;
                    }
                }
                if (needsAnim)
                ...
                if (g_FadeIn) {
                    ...
                    SetLayeredWindowAttributes(
                        hwnd, 0, (BYTE)(g_FadeAlpha * 255), LWA_ALPHA);
                    if (g_FadeAlpha >= 1.f) {
                        KillTimer(hwnd, IDT_FADE);
                        g_FadeActive = false;
                    }
                ...
                if (g_TextFadeOut) {
                    ...
                    if (g_TextFadeAlpha <= 0.f) {
                        ...
                        {
                            scoped_lock lk(g_M.mtx);
                            g_M.title = g_PendingTitle;
                            g_M.artist = g_PendingArtist;
                        }
                    }
                } else {
                    g_TextFadeAlpha += 0.12f;
                ...
                if (!g_S.showVisualizer) {
                    ...
                    for (int i = 0; i < max(1, min(g_S.vizBars, VIZ_BARS_MAX));
                         i++) {
                        ...
                        if (g_S.vizShape == 1) {  // Mountain: per-bar physics
                            float dist = fabsf(
                                (float)i -
                                ((max(1, min(g_S.vizBars, VIZ_BARS_MAX)) - 1) *
                                 0.5f));
                            if (dist < 0.5f) {
                                a = 0.85f;
                                d = 0.24f;
        ...
        case WM_LBUTTONDOWN:
            if (g_S.showSpeakerIcon && g_Hover == 4 && g_VolumeHover) {
                ...
                if (g_LayoutDirty || g_Layout.W != _rc.right ||
                    g_Layout.H != _rc.bottom)
        ...
        case WM_MOUSEMOVE: {
            ...
            if (!g_S.miniMode) {
                if (g_S.showSpeakerIcon) {
                    ...
                    float volHitRight =
                        g_VolumeHover ? (vX + 95.f) : (vX + 14.f);
                    if (mx >= volHitLeft && mx <= volHitRight && my >= hitTop &&
                        my <= hitBot) {
                        newVolumeHover = true;
                        ns = 4;
                    } else
                        newVolumeHover = false;
                }
                ...
                if (ns != 4) {
                    if (g_S.showPlaybackControls) {
                        if (mx >= pX - (pnSmallR + 4) &&
                            ...
                            my <= cY + (pnSmallR + 4))
                            ns = 1;
                        else if (mx >= ppX - ppBigR && mx <= ppX + ppBigR &&
                                 my >= cY - ppBigR && my <= cY + ppBigR)
                            ns = 2;
                        else if (mx >= nX - (pnSmallR + 4) &&
                                 mx <= nX + (pnSmallR + 4) &&
                                 my >= cY - (pnSmallR + 4) &&
                                 my <= cY + (pnSmallR + 4))
                            ns = 3;
                    ...
                    if (ns == 0 && g_S.showShuffleButton && shuffleX > 0.f &&
                        mx >= shuffleX - (pnSmallR + 4) &&
                        mx <= shuffleX + (pnSmallR + 4) &&
                        my >= cY - (pnSmallR + 4) && my <= cY + (pnSmallR + 4))
                        ns = 5;
                    if (ns == 0 && g_S.showRepeatButton && repeatX > 0.f &&
                        mx >= repeatX - (pnSmallR + 4) &&
                        mx <= repeatX + (pnSmallR + 4) &&
                        my >= cY - (pnSmallR + 4) && my <= cY + (pnSmallR + 4))
                        ns = 6;
                }
            ...
            if (g_S.showSpeakerIcon && g_VolumeHover && (wp & MK_LBUTTON) &&
                GetCapture() == hwnd) {
            ...
            if (ns != g_Hover || newVolumeHover != g_VolumeHover) {
                g_Hover = ns;
                g_VolumeHover = newVolumeHover;
                if (!g_HoverAnimRunning) {
                    SetTimer(hwnd, IDT_HOVER_ANIM, 16, NULL);
                    g_HoverAnimRunning = true;
                }
                InvalidateRect(hwnd, NULL, FALSE);
            }
        ...
        case WM_LBUTTONUP:
            ...
            if (g_Hover > 0 && g_Hover <= 6 && g_Hover != 4) {
                ...
                // If user clicks the left side (art/title), try to focus the
                // app
                if (mx < (int)(firstControlX - 12) &&
        ...
        case WM_MOUSEWHEEL: {
            ...
            // scrollVolume: scroll anywhere on the panel to adjust volume
            // (speaker icon not required)
            if (g_S.scrollVolume) {
                short d = GET_WHEEL_DELTA_WPARAM(wp);
                SetVolume(g_Volume + (d > 0 ? 0.05f : -0.05f));
                InvalidateRect(hwnd, NULL, FALSE);
            } else if (g_S.showSpeakerIcon) {
                // Speaker icon visible but scrollVolume off: only scroll near
                // the icon
                RECT _rc;
                GetClientRect(hwnd, &_rc);
                if (g_LayoutDirty || g_Layout.W != _rc.right ||
        ...
        case WM_PAINT: {
            ...
            if (g_Scroll && !g_AnimTimerRunning) {
                SetTimer(hwnd, IDT_ANIM, 16, NULL);
                g_AnimTimerRunning = true;
            }
        ...
        default:
            ...
            // Forward media key AppCommands (mouse hotkeys, keyboard media
            // keys) to the shell. Without this, WM_APPCOMMAND dies here because
            // this window is parentless and topmost, breaking the normal bubble
            // chain that would reach the media app or shell handler.
...
void MediaThread() {
    ...
    if (CWB) {
        g_hWnd = CWB(WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST,
                     ...
                     WS_POPUP | WS_VISIBLE, 0, 0, g_S.width, g_S.height, NULL,
                     NULL, wc.hInstance, NULL, ZBID_IMMERSIVE_NOTIFICATION);
    }

    if (!g_hWnd) {
        g_hWnd = CreateWindowEx(
            WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST, wc.lpszClassName,
            L"Taskbar Media Player", WS_POPUP | WS_VISIBLE, 0, 0, g_S.width,
            g_S.height, NULL, NULL, wc.hInstance, NULL);
    }
...
BOOL WhTool_ModInit() {
    ...
    LoadSettings();
    g_pThread = std::make_unique<std::thread>(MediaThread);
    return TRUE;
}
