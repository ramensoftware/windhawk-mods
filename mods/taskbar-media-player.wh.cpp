...
// @description     Taskbar Media Player — is a Windhawk mod that integrates a modern media player directly into the Windows 11 taskbar. It allows you to control music and view track information seamlessly without interrupting your workflow.
// @version         1.0.0
// @author          Eliasilyz
...
**Taskbar Player —** is a Windhawk mod that integrates a modern media player directly into the Windows 11 taskbar. It allows you to control music and view track information seamlessly without interrupting your workflow.

![image](https://files.catbox.moe/a631rz.png)
### Key Features:
...
* **[Eliasilyz](https://github.com/Eliasilyz) —** Bug Fixing and refactoring code.

### Report a Bug
If you encounter any issues, bugs, or have suggestions for new features, please report them on the project's GitHub page:
👉 **[Report an Issue on GitHub](https://github.com/Eliasilyz/taskbar-media-player/issues)**
*/
// ==/WindhawkModReadme==
...
static std::shared_mutex g_mediaButtonsMutex;
...
static void LoadSettings() {
    ...
    try {
        std::unique_lock<std::shared_mutex> lock(g_mediaButtonsMutex);
...
static std::shared_mutex g_mediaMtx;
...
static std::shared_mutex g_sessionMtx;
...
static AlbumPalette ExtractAlbumPalette(const std::vector<BYTE>& thumbBytes) {
    ...
    try {
        // Use WIC synchronously — safe on any thread, no .get() blocking needed.
        winrt::com_ptr<IWICImagingFactory> wicFactory;
        HRESULT hr = CoCreateInstance(
            CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER,
            __uuidof(IWICImagingFactory), wicFactory.put_void());
        if (FAILED(hr) || !wicFactory) return {fallbackPrimary, fallbackSecondary};

        // Create a stream from raw bytes.
        winrt::com_ptr<IStream> stream;
        stream.attach(SHCreateMemStream(thumbBytes.data(), static_cast<UINT>(thumbBytes.size())));
        if (!stream) return {fallbackPrimary, fallbackSecondary};

        winrt::com_ptr<IWICBitmapDecoder> decoder;
        hr = wicFactory->CreateDecoderFromStream(stream.get(), nullptr,
            WICDecodeMetadataCacheOnDemand, decoder.put());
        if (FAILED(hr) || !decoder) return {fallbackPrimary, fallbackSecondary};

        winrt::com_ptr<IWICBitmapFrameDecode> frame;
        hr = decoder->GetFrame(0, frame.put());
        if (FAILED(hr) || !frame) return {fallbackPrimary, fallbackSecondary};

        // Convert to 32bpp BGRA.
        winrt::com_ptr<IWICFormatConverter> converter;
        hr = wicFactory->CreateFormatConverter(converter.put());
        if (FAILED(hr) || !converter) return {fallbackPrimary, fallbackSecondary};

        hr = converter->Initialize(frame.get(), GUID_WICPixelFormat32bppBGRA,
            WICBitmapDitherTypeNone, nullptr, 0.0, WICBitmapPaletteTypeCustom);
        if (FAILED(hr)) return {fallbackPrimary, fallbackSecondary};

        UINT w = 0, h = 0;
        converter->GetSize(&w, &h);
        if (w == 0 || h == 0) return {fallbackPrimary, fallbackSecondary};

        // Cap decode size to avoid allocating huge buffers.
        const UINT kMaxDim = 256;
        winrt::com_ptr<IWICBitmapScaler> scaler;
        if (w > kMaxDim || h > kMaxDim) {
            hr = wicFactory->CreateBitmapScaler(scaler.put());
            if (SUCCEEDED(hr) && scaler) {
                UINT sw = w, sh = h;
                if (sw > kMaxDim) { sh = sh * kMaxDim / sw; sw = kMaxDim; }
                if (sh > kMaxDim) { sw = sw * kMaxDim / sh; sh = kMaxDim; }
                if (sw == 0) sw = 1;
                if (sh == 0) sh = 1;
                hr = scaler->Initialize(converter.get(), sw, sh, WICBitmapInterpolationModeLinear);
                if (SUCCEEDED(hr)) {
                    scaler->GetSize(&w, &h);
                }
            }
        ...
        IWICBitmapSource* src = scaler ? static_cast<IWICBitmapSource*>(scaler.get())
                                       : static_cast<IWICBitmapSource*>(converter.get());
        hr = src->CopyPixels(nullptr, stride, static_cast<UINT>(pixels.size()), pixels.data());
        if (FAILED(hr)) return {fallbackPrimary, fallbackSecondary};

        // Quantize into 16x16x16 color buckets.
...
static winrt::fire_and_forget SendMediaCommandAsync(int cmd) {
    if (g_unloading) co_return;
    co_await winrt::resume_background();
    try {
        GlobalSystemMediaTransportControlsSession session{ nullptr };
        {
            std::shared_lock<std::shared_mutex> lk(g_sessionMtx);
            session = g_currentSession;
        }
        if (!session) co_return;

        switch (cmd) {
            case 1: co_await session.TrySkipPreviousAsync();    break;
            case 2: co_await session.TryTogglePlayPauseAsync(); break;
            case 3: co_await session.TrySkipNextAsync();        break;
            case 4: {
                // co_await is not allowed inside a catch block, so we use a bool flag
                // to run the fallback outside the try/catch handler.
                bool stopSucceeded = false;
                bool stopAttempted = false;
                try {
                    Wh_Log(L"SendMediaCommandAsync: Attempting to stop playback");
                    auto result = co_await session.TryStopAsync();
                    Wh_Log(L"SendMediaCommandAsync: TryStopAsync returned %d", result);
                    stopAttempted = true;
                    stopSucceeded = result;
                } catch (...) {
                    Wh_Log(L"SendMediaCommandAsync: TryStopAsync threw, will use fallback");
                    stopAttempted = true;
                    stopSucceeded = false;
                }
                if (stopAttempted && !stopSucceeded) {
                    Wh_Log(L"SendMediaCommandAsync: Stop not supported/failed, using pause + seek fallback");
                    try { co_await session.TryPauseAsync(); } catch (...) {}
                    try { co_await session.TryChangePlaybackPositionAsync(0); } catch (...) {
                        Wh_Log(L"SendMediaCommandAsync: Fallback seek also failed");
                    }
                }
                }
                break;
            case 5:
                try {
                    ...
                    if (timeline) {
                        ...
                        auto newPos = pos - std::chrono::seconds(5);
                        if (newPos.count() < 0) newPos = std::chrono::seconds(0);
                        co_await session.TryChangePlaybackPositionAsync(newPos.count());
                    }
                } catch (...) {}
                break;
            case 6:
            ...
            case 7:
                try {
                    ...
                    if (info) {
                        ...
                        if (shuffleRef) {
                            currentShuffle = shuffleRef.Value();
                        }
                    }
                    co_await session.TryChangeShuffleActiveAsync(!currentShuffle);
                } catch (...) {}
                break;
            case 8:
                try {
                    ...
                    if (co_await session.TryChangeAutoRepeatModeAsync(mode)) {
                        g_repeatMode = next;
                        DispatchMediaUpdate();
                    }
                } catch (...) {}
...
static winrt::fire_and_forget FetchMediaPropertiesAsync();
static winrt::fire_and_forget FetchPlaybackInfoAsync();
...
static void SwitchMediaSession() {
    ...
    {
        std::unique_lock<std::shared_mutex> lk(g_sessionMtx);
...
static void ExecuteMediaAction(const std::wstring& action, FrameworkElement const& sourceElement = nullptr) {
    if (action == L"none") {
        ...
        std::thread([]() {
            ...
            {
                std::shared_lock<std::shared_mutex> lk(g_mediaMtx);
                title = g_media.title;
            }
            {
                std::shared_lock<std::shared_mutex> lk(g_sessionMtx);
...
static winrt::fire_and_forget FetchMediaPropertiesAsync() {
    ...
    try {
        ...
        try {
                ...
                try {
                    ...
                    if (stream) {
                        ...
                        if (sz > 0 && sz < 4 * 1024 * 1024) {
                            ...
                            co_await reader.LoadAsync((UINT32)sz);
            ...
            {
                std::unique_lock<std::shared_mutex> lk(g_mediaMtx);
                try {
...
static winrt::fire_and_forget FetchPlaybackInfoAsync() {
    ...
    try {
        GlobalSystemMediaTransportControlsSession session{nullptr};
        { std::shared_lock<std::shared_mutex> lk(g_sessionMtx); session = g_currentSession; }
        if (session) {
            try {
                auto info = session.GetPlaybackInfo();
                if (!info) {
...
static void DetachCurrentSession() {
    ...
    {
        std::unique_lock<std::shared_mutex> lkm(g_mediaMtx);
...
static Grid BuildPlayerGrid() {
    try {
        ...
        if (g_settings.showMediaButtons) {
            ...
            try {
                std::shared_lock<std::shared_mutex> lock(g_mediaButtonsMutex);
                if (!g_mediaButtons.empty()) {
...
static void RefreshPlayerContents() {
    ...
    { std::shared_lock<std::shared_mutex> lk(g_sessionMtx); hasSession = (g_currentSession != nullptr); }
...
static void UpdateVisibility() {
    ...
    if (!hide && g_settings.idleHideSeconds > 0) {
        bool playing = false;
        { std::shared_lock<std::shared_mutex> lk(g_mediaMtx); playing = g_media.isPlaying; }
    ...
    if (!hide) {
        ...
        { std::shared_lock<std::shared_mutex> lk(g_mediaMtx); hasMedia = g_media.hasMedia; }
        { std::shared_lock<std::shared_mutex> lk(g_sessionMtx); hasSession = (g_currentSession != nullptr); }
