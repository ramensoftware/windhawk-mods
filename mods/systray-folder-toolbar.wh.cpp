// ==WindhawkMod==
// @id              systray-folder-toolbar
// @name            Systray Folder Toolbar
// @description     Adds one or more folders as tray icons to the Systray. Clicking opens the folder as a quick-launch menu—just like the Windows 10 toolbars.
// @version         2.0.0
// @author          Rene Mayer
...
static void ShowFolderMenu(FolderInstance* inst, bool rightClick) {
    if (inst->settings.folderPath.empty()) {
        MessageBoxW(nullptr,
            ...
            L"SYSTRAY Folder Toolbar", MB_OK | MB_ICONINFORMATION);
        return;
    }
...
// ---------------------------------------------------------------------------
BOOL Wh_ModInit() {
    Wh_Log(L"Sys-Tray Folder Toolbar v2: Init");
...
void Wh_ModUninit() {
    Wh_Log(L"Sys-Tray Folder Toolbar v2: Uninit");
    StopAllInstances();
}

void Wh_ModSettingsChanged() {
    Wh_Log(L"Sys-Tray Folder Toolbar v2: Settings changed");
    StopAllInstances();
    StartInstances(LoadAllSettings());
}