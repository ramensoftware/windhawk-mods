...
// Build full command line for classic editor (argv[0] replaced, rest preserved)
static std::wstring BuildNewCommandLineForClassic(const std::wstring& classicPath) {
  std::wstring cmd = GetCommandLineW();
  auto argv = ParseCommandLineW(cmd.c_str());
  std::wstring newCmd = L"\"" + classicPath + L"\"";
  for (size_t i = 1; i < argv.size(); ++i) {
    newCmd += L" ";
    newCmd += QuoteArgIfNeeded(argv[i]);
  }
  return newCmd;
}
...
static DWORD WINAPI HandoffThreadProc(LPVOID) {
  ...
  // If our own first real arg starts with /SESSION, exit quietly with 0
  {
    auto argv = ParseCommandLineW(GetCommandLineW());
    if (argv.size() >= 2 && CiStartsWith(TrimLeft(argv[1]), L"/SESSION")) {
