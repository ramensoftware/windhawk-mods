...
// @id              scrambled-eggs
// @name            Scrambled Eggs
// @description     Turn your computer's text into scrambled eggs.
// @version         1.0
...
/*
# Scrambled Eggs
This is a fork of the Replace Text mod that completely scrambles the text on your computer. Do not attempt to use your computer like this, unless you want a really bad time.
*/
// ==/WindhawkModReadme==
...
std::string upper("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 26);
std::string lower("abcdefghijklmnopqrstuvwxyz", 26);
std::string nums("0123456789", 10);
std::string syms("!@#$%^&*()-_+=[]{}|\\:'\"/?.>,<`~", 31);
...
std::string ReplaceStringA(PCSTR string, size_t len = -1)
{
    ...
    for (int i = 0; i < (int)len; i++) {
        if(upper.contains(result[i])) {
            found = upper.find(result[i]);
            if(found != std::string::npos) {
                result[i] = upper[(found + 2) % 26];
            }
        } else if(lower.contains(result[i])) {
            found = lower.find(result[i]);
            if(found != std::string::npos) {
                result[i] = lower[(found + 2) % 26];
            }
        } else if(nums.contains(result[i])) {
             found = nums.find(result[i]);
            if(found != std::string::npos) {
                result[i] = nums[(found + 2) % 10];
            }
        } else if(syms.contains(result[i])) {
            found = syms.find(result[i]);
            if(found != std::string::npos) {
                result[i] = syms[(found + 2) % 31];
            }
        }
    }
