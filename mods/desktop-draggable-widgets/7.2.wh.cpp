...
// 166.667 logical px * 0.75 = 125 screen px.
constexpr float kRoundMeterDiameter = 166.666667f;
constexpr float kRoundMeterThickness = 40.0f;       // 30 screen px
constexpr float kSliderShellThickness = 40.0f;      // 30 screen px
constexpr float kSliderIndicatorThickness = 13.333333f; // 10 screen px
...
void ApplyDefaultWidgetLayout() {
    ...
    const float roundD = kRoundMeterDiameter;
...
RectF RectForSliderOrientation(const RectF& current, SliderOrientation orientation,
    ...
    if (orientation == SliderOrientation::Round) {
        const float diameter = kRoundMeterDiameter;
...
RectF VolumeSliderRect() {
    ...
    float thickness = kSliderShellThickness;
    if (orientation == SliderOrientation::Vertical) {
...
RectF VolumeSliderIndicatorRect() {
    ...
    const float thickness = kSliderIndicatorThickness;
...
bool PtInVolumeSlider(PointF pt) {
    if (g_volumeWidgetOrientation == SliderOrientation::Round) {
        ...
        const float half = kRoundMeterThickness / 2.0f;
        const float shellCenterRadius = outerRadius - half;
        const float innerRadius = std::max(0.0f, shellCenterRadius - half);
        const float outerIndicatorRadius = shellCenterRadius + half;
        return distance >= innerRadius && distance <= outerIndicatorRadius;
    }
    return PtInRectF(VolumeSliderRect(), pt);
}

float VolumeLevelFromPoint(PointF pt, bool continuingDrag) {
    if (g_volumeWidgetOrientation == SliderOrientation::Round) {
        RectF rc = g_widgets[WidgetVolume].rc;
        const float cx = rc.X + rc.Width / 2.0f;
        const float cy = rc.Y + rc.Height / 2.0f;
        ...
        if (continuingDrag && std::fabs(value - g_volumeLevel) > 0.5f) {
            value = value > g_volumeLevel ? 0.0f : 1.0f;
        }
        return value;
    }
...
void DrawSlider(Graphics& g, const Widget& widget, const wchar_t* label,
    ...
    if (IsWidgetRoundMeter(widget.id)) {
        const float shellThickness = kRoundMeterThickness;
        const float indicatorThickness = kSliderIndicatorThickness;
    ...
    const float shellThickness = kSliderShellThickness;
    const float indicatorThickness = kSliderIndicatorThickness;
    const float shellPadding = 2.0f / RenderScale();
    ...
    switch (msg) {
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (hit == WidgetVolume && PtInVolumeSlider(pt)) {
                ...
                SetCapture(hwnd);
                SetSystemVolume(VolumeLevelFromPoint(pt, false));
                Render();
                return 0;
            }
            if (hit >= 0) {
        ...
        case WM_MOUSEMOVE: {
            ...
            if (g_draggingSlider) {
                ...
                SetSystemVolume(VolumeLevelFromPoint(pt, true));
                Render();
                return 0;
            }
