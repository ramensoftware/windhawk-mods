// ==WindhawkMod==
// @id              cursor-motion-ultra
// @name            Cursor Motion Ultra / 光标运动模糊·增强版
...
// ==WindhawkModReadme==
/*
# Cursor Motion Ultra / 光标运动模糊·增强版
...
- particle_attraction: 40
  ...
  $description: 粒子被吸向光标的强度（0=关闭吸附，0-100）。开启后粒子会缓慢飞回光标位置。How strongly particles are attracted back to the cursor (0=off, 0-100).
...
VOID CALLBACK SmearTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime) {
    ...
    if (!g_particles.empty()) {
        for (auto& p : g_particles) {
            ...
            } else if (p.shapeType == 3 && g_pHexagramGeom) {
                D2D1_MATRIX_3X2_F oldT;
                g_pDCRenderTarget->GetTransform(&oldT);
                g_pDCRenderTarget->SetTransform(D2D1::Matrix3x2F::Scale(p.size, p.size) * D2D1::Matrix3x2F::Translation(p.x, p.y));
...
DWORD WINAPI OverlayThreadProc(LPVOID) {
    ...
    const wchar_t CN[] = L"CursorMotionUltraClass";
    ...
    g_overlayHwnd = CreateWindowEx(
        ...
        CN, L"CursorMotionUltraOverlay", WS_POPUP, sx, sy, sw, sh, NULL, NULL, hi, NULL);
