...
This mod blocks suspicious intermediate white frames that appear during Explorer tab creation and redraw, making tab opening look cleaner and less distracting.

## What it does

- Reduces or removes the white flash when opening a new Explorer tab
- Works without depending on localized window titles
- Avoids affecting Control Panel explorer factory windows
- Supports small Explorer windows too
...
bool IsWindowClass(HWND hWnd, const wchar_t* clsName)
{
...
bool IsWhiteishBrush(HBRUSH hbr)
{
    if (!hbr)
        return false;

    LOGBRUSH lb = {};
    if (GetObjectW(hbr, sizeof(lb), &lb) != sizeof(lb))
        return false;

    if (lb.lbStyle != BS_SOLID)
        return false;

    return IsWhiteishColor(lb.lbColor);
}
...
bool IsNearWholeDirectUISurface(HWND hWndDest, int x, int y, int cx, int cy)
{
    ...
    long long blitArea   = 1LL * cx * cy;
    long long clientArea = 1LL * clientW * clientH;
...
void RememberWhiteFill(HDC hdc, int w, int h)
{
    ...
    for (int i = 0; i < static_cast<int>(ARRAYSIZE(g_recent)); i++)
    {
...
void ArmFollowupBlock(HWND hWndDest, HDC hdcSrc, int cx, int cy)
{
    ...
    for (int i = 0; i < static_cast<int>(ARRAYSIZE(g_followup)); i++)
    {
...
BOOL Wh_ModInit()
{
    ...
    ShouldAppsUseDarkMode =
        (ShouldAppsUseDarkMode_T)GetProcAddress(g_hUxTheme, MAKEINTRESOURCEA(132));
