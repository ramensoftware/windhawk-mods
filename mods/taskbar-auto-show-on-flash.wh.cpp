...
// @description     通知闪烁时任务栏自动弹出，延迟后自动隐藏，新通知重置计时器
// @version         1.0
...
BOOL Wh_ModInit() {
    ...
    if (!hModule) {
        Wh_Log(L"Failed to load taskbar module");
        return FALSE;
    }
    ...
    if (!HookSymbols(hModule, symbolHooks, ARRAYSIZE(symbolHooks))) {
...
BOOL Wh_ModSettingsChanged(BOOL* bReload) {
    LoadSettings();
    Wh_Log(L"Settings updated: delay=%lldms, enabled=%d",
           delayFiletime / (10 * 1000), g_enabled);
    return TRUE;
}
