...
// @id              BlackWindows
// @name            BlackWindows
// @description     Black Windows based on TranslucentWindows by Undisputed00x
// @version         0.1.0
// @author          mvdvicenze
// @github          https://github.com/mvdvicenze
...
{
    // Background roles only. Preserve text, selection and control-edge contrast.
    if (nIndex == COLOR_APPWORKSPACE || nIndex == COLOR_ACTIVECAPTION ||
        nIndex == COLOR_INACTIVECAPTION || nIndex == COLOR_GRADIENTACTIVECAPTION ||
        nIndex == COLOR_GRADIENTINACTIVECAPTION || nIndex == COLOR_ACTIVEBORDER)
        return RGB(0, 0, 0);
...
// Opaque fill, including alpha when the destination is a composition DIB.
static bool PaintOpaqueBlack(HDC hdc, LPCRECT rect, LPCRECT clip = nullptr)
{
    if (!hdc || !rect) return false;
    RECT area = *rect;
    if (clip && !IntersectRect(&area, rect, clip)) return true;
    if (IsRectEmpty(&area)) return true;
    BP_PAINTPARAMS params = {sizeof(params)};
    HDC bufferedDC = nullptr;
    HPAINTBUFFER buffer = BeginBufferedPaint(hdc, &area, BPBF_TOPDOWNDIB,
                                            &params, &bufferedDC);
    if (buffer && bufferedDC) {
        FillRect(bufferedDC, &area, (HBRUSH)GetStockObject(BLACK_BRUSH));
        BufferedPaintMakeOpaque(buffer, &area);
        if (SUCCEEDED(EndBufferedPaint(buffer, TRUE))) return true;
    } else if (buffer) {
        EndBufferedPaint(buffer, FALSE);
    }
    return FillRect(hdc, &area, (HBRUSH)GetStockObject(BLACK_BRUSH)) != 0;
}

static bool IsSolidBackgroundPart(const std::wstring& cls, int part, int state)
{
    if (cls == L"TreeView")
        return part == 0 || (part == TVP_TREEITEM && state == TREIS_NORMAL);
    if (cls == L"Status")
        return part == 0 || part == SP_PANE || part == SP_GRIPPERPANE;
    if (cls == L"Rebar") return part == RP_BACKGROUND || part == RP_BAND;
    if (cls == L"PreviewPane") return part == 1;
    if (cls == L"CommandModule") return part == 1 && state == 0;
    if (cls == L"CommonItemsDialog") return part == 1;
    if (cls == L"Tab") return part == TABP_PANE || part == TABP_BODY;
    if (cls == L"TaskDialog")
        return part == TDLG_PRIMARYPANEL || part == TDLG_CONTENTPANE ||
               part == TDLG_SECONDARYPANEL || part == TDLG_FOOTNOTEPANE;
    if (cls == L"AeroWizard")
        return part == AW_TITLEBAR || part == AW_HEADERAREA ||
               part == AW_CONTENTAREA || part == AW_COMMANDAREA;
    if (cls == L"ControlPanel")
        return part == CPANEL_CONTENTPANE || part == CPANEL_BANNERAREA ||
               part == CPANEL_LARGECOMMANDAREA || part == CPANEL_SMALLCOMMANDAREA;
    if (cls == L"Menu" && g_settings.FlyoutsEffects)
        return part == MENU_POPUPBACKGROUND || part == MENU_POPUPBORDERS ||
               part == MENU_POPUPGUTTER || part == MENU_BARBACKGROUND;
    return false;
}
...
{       
    std::wstring ThemeClassName = GetThemeClass(hTheme);
    if (IsSolidBackgroundPart(ThemeClassName, iPartId, iStateId) &&
        PaintOpaqueBlack(hdc, pRect, pClipRect)) return S_OK;
...
{    
    std::wstring ThemeClassName = GetThemeClass(hTheme);
    // Leave specialized omit/content-only drawing requests to the original.
    if ((!pOptions || !(pOptions->dwFlags & ~DTBG_CLIPRECT)) &&
        IsSolidBackgroundPart(ThemeClassName, iPartId, iStateId) &&
        PaintOpaqueBlack(hdc, pRect,
...
VOID HandleEffects(HWND hWnd)
{
    // Disable composition materials rather than tinting an acrylic backdrop.
    DWM_BLURBEHIND solidBlur = {};
    solidBlur.dwFlags = DWM_BB_ENABLE;
    solidBlur.fEnable = FALSE;
    DwmEnableBlurBehindWindow(hWnd, &solidBlur);
    ACCENT_POLICY solidAccent = {};
    solidAccent.AccentState = ACCENT_STATE_DISABLED;
    WINCOMPATTRDATA solidAttrib = {};
    solidAttrib.Attrib = WCA_ACCENT_POLICY;
    solidAttrib.pvData = &solidAccent;
    solidAttrib.cbData = sizeof(solidAccent);
    if (SetWindowCompositionAttribute)
        SetWindowCompositionAttribute(hWnd, &solidAttrib);
    UINT solidBackdrop = DWMSBT_NONE;
    DwmSetWindowAttribute(hWnd, DWMWA_SYSTEMBACKDROP_TYPE, &solidBackdrop, sizeof(solidBackdrop));
    MARGINS solidMargins = {};
    DwmExtendFrameIntoClientArea(hWnd, &solidMargins);
    if (g_settings.BlackTitleBars) {
...
VOID LoadSettings()
{
    g_settings.FillBg = TRUE;
    g_settings.AccentColorize = FALSE;
    g_settings.SetSystemColors = FALSE; // Keep changes inside included processes.
    g_settings.BgType = g_settings.Default;
    g_settings.BlackTitleBars = Wh_GetIntSetting(L"BlackTitleBars");
    g_settings.FlyoutsEffects = Wh_GetIntSetting(L"BlackPopupMenus");
    GenerateTextAlphaGammaLUT();
    ApplyHooks();
}
