...
// ==WindhawkModReadme==
/*
# MacMotion App Effects

MacMotion keeps Windows' native virtual desktop and Alt+Tab transitions, and
focuses on smooth maximize plus Genie-style app launch, minimize, and restore
motion.

This build is derived from the MIT-licensed Windows Animations and Genie
Animation Windhawk implementations. Close and hide-as-close remain disabled by
default. Disable MacMotion and restart the affected app (or Explorer) to roll
back immediately.
...
* **Toggles:** Individually turn on/off Restore, Close, and Launch animations to suit your workflow.
*/
// ==/WindhawkModReadme==
...
- maximize_animation: true
  ...
  $description: Smoothly scale a window between its normal and maximized bounds.
- maximize_duration_ms: 260
  $name: Maximize animation duration (ms)
  $description: Controls maximize and unmaximize speed. Clamped between 120 and 700.
...
- launch_genie: true
  $name: Genie animation for app launch
  $description: Raise an application window from its taskbar position when it first opens.
*/
// ==/WindhawkModSettings==
...
struct NativeAltTabCommitData { HWND source; LONG generation; };
bool StartAnimation(HWND hWnd, BOOL rising, LONG_PTR originalExStyle,
                    BOOL cloakHidden, BOOL isClosing, UINT closeMsg,
                    HANDLE hWaitFinish);
static bool IsAppMainWindow(HWND hWnd, bool forSwitch);
...
static bool PrepareVisibleLaunchAnim(HWND hWnd, LONG_PTR* origExOut);
static void CommitLaunchAnim(HWND hWnd, LONG_PTR originalExStyle);
DWORD WINAPI NativeAltTabCommitThread(LPVOID parameter);
...
namespace AnimConstants {
    constexpr float SwitchStartScale = 0.95f;
    ...
    constexpr int AltTabPollMs = 10;
    constexpr int AltTabSessionMs = 750;
...
static constexpr PCWSTR kPropCloseBypass = L"macmotion.CloseBypass";
static constexpr PCWSTR kPropClosed = L"macmotion.Closed";
...
std::atomic<HWINEVENTHOOK> g_hSystemSwitchHook{NULL};
std::atomic<HWINEVENTHOOK> g_hObjectShowHook{NULL};
using XamlAltTabViewHost_Show_t = HRESULT(WINAPI*)(void*, void*, int, void*);
XamlAltTabViewHost_Show_t g_XamlAltTabViewHost_Show_Original = nullptr;
std::atomic<bool> g_nativeAltTabHookInstalled{false};
volatile LONG g_nativeAltTabGeneration = 0;
volatile LONG g_nativeAltTabPending = 0;
std::atomic<HWND> g_nativeAltTabSource{NULL};
std::atomic<HWND> g_nativeAltTabPreCloakedTarget{NULL};
...
std::atomic<bool> g_closeAnimation{false};
std::atomic<bool> g_hideAsClose{false};
std::atomic<bool> g_launchAnimation{true};
std::atomic<bool> g_switchAnimation{false};
std::atomic<int> g_switchDurationMs{220};
...
static bool StartWorkerThread(LPTHREAD_START_ROUTINE proc, void* param) {
    ...
    // Compact completed workers in one pass. Repeated vector::erase calls move
    // the tail on every deletion and become unnecessarily expensive when many
    // short-lived animation/UIA workers have completed.
    size_t writeIndex = 0;
    for (HANDLE hThread : g_WorkerThreads) {
        if (WaitForSingleObject(hThread, 0) == WAIT_OBJECT_0) {
            CloseHandle(hThread);
        } else {
            g_WorkerThreads[writeIndex++] = hThread;
        }
    }
    g_WorkerThreads.resize(writeIndex);

    // Grow geometrically instead of reserve(size + 1), which can force a heap
    // reallocation on almost every worker creation.
    if (g_WorkerThreads.size() == g_WorkerThreads.capacity()) {
        const size_t oldCapacity = g_WorkerThreads.capacity();
        const size_t newCapacity = oldCapacity ? oldCapacity * 2 : 8;
        try {
            g_WorkerThreads.reserve(newCapacity);
    ...
    // Capacity was guaranteed above, so this push doesn't need to allocate.
...
static void ResetAltTabState() {
    ...
    InterlockedExchangePointer(&g_pSharedState->altTabSourceWindow, nullptr);
...
static void BeginAltTabSession(HWND source) {
    if (!g_pSharedState || !g_sharedStateWritable) return;
    InterlockedExchange(&g_pSharedState->lastAltTabTick, 0);
    InterlockedExchangePointer(&g_pSharedState->altTabSourceWindow, source);
...
static void ReleasePreCloakedAltTabTarget(HWND except = NULL) {
    HWND target = g_nativeAltTabPreCloakedTarget.exchange(
        NULL, std::memory_order_acq_rel);
    if (!target || target == except || !IsWindow(target)) return;

    SetWindowCloak(target, FALSE);
    UpdateDwmTransitions(target, TRUE);
    Wh_Log(L"Native Alt+Tab released pre-cloaked target hwnd=%p", target);
}

static void PreCloakNativeAltTabActivation(HWND target, PCWSTR apiName) {
    if (InterlockedCompareExchange(&g_nativeAltTabPending, 0, 0) == 0 ||
        g_unloading.load(std::memory_order_relaxed) || !target ||
        target == g_nativeAltTabSource.load(std::memory_order_acquire) ||
        IsIconic(target) || !IsAppMainWindow(target, true) ||
        IsAnimating(target)) {
        return;
    }

    HWND previous = g_nativeAltTabPreCloakedTarget.exchange(
        target, std::memory_order_acq_rel);
    if (previous && previous != target && IsWindow(previous)) {
        SetWindowCloak(previous, FALSE);
        UpdateDwmTransitions(previous, TRUE);
    }
    UpdateDwmTransitions(target, FALSE);
    SetWindowCloak(target, TRUE);
    Wh_Log(L"Native Alt+Tab activation pre-cloaked by %s hwnd=%p",
           apiName, target);
}

BOOL WINAPI SetForegroundWindow_Hook(HWND hWnd) {
    PreCloakNativeAltTabActivation(hWnd, L"SetForegroundWindow");
    BOOL result = SetForegroundWindow_Original(hWnd);
    if (!result &&
        g_nativeAltTabPreCloakedTarget.load(std::memory_order_acquire) == hWnd) {
        ReleasePreCloakedAltTabTarget();
    }
    return result;
}

void WINAPI SwitchToThisWindow_Hook(HWND hWnd, BOOL altTab) {
    PreCloakNativeAltTabActivation(hWnd, L"SwitchToThisWindow");
    SwitchToThisWindow_Original(hWnd, altTab);
}

HRESULT WINAPI XamlAltTabViewHost_Show_Hook(
    void* pThis,
    void* immersiveMonitor,
    int flags,
    void* applicationView) {
    HWND source = GetForegroundWindow();
    LONG generation = InterlockedIncrement(&g_nativeAltTabGeneration);
    g_nativeAltTabSource.store(source, std::memory_order_release);
    InterlockedExchange(&g_nativeAltTabPending, 1);
    ReleasePreCloakedAltTabTarget();
    Wh_Log(L"Native Windows 11 Alt+Tab switcher armed activation hooks");
    HRESULT result = g_XamlAltTabViewHost_Show_Original(
        pThis, immersiveMonitor, flags, applicationView);

    auto* request = new (std::nothrow) NativeAltTabCommitData{source, generation};
    if (!request || !StartWorkerThread(NativeAltTabCommitThread, request)) {
        delete request;
        Wh_Log(L"Native Alt+Tab commit watcher could not start");
    }
    return result;
}

bool InstallNativeAltTabHook() {
    HMODULE twinuiPcshell = GetModuleHandleW(L"twinui.pcshell.dll");
    if (!twinuiPcshell) {
        twinuiPcshell = LoadLibraryW(L"twinui.pcshell.dll");
    }
    if (!twinuiPcshell) {
        Wh_Log(L"Native Alt+Tab hook unavailable: twinui.pcshell.dll not loaded");
        return false;
    }
    ...
    g_nativeAltTabHookInstalled.store(true, std::memory_order_release);
    Wh_Log(L"Native Windows 11 Alt+Tab hook installed");
    return true;
}
static bool ConsumeAltTabIntent(HWND target, DWORD eventTime) {
    ...
    HWND source = reinterpret_cast<HWND>(
        InterlockedCompareExchangePointer(
            &g_pSharedState->altTabSourceWindow, nullptr, nullptr));
...
static void CleanupWindowData(HWND hWnd) {
    ...
    // Only invalidate state belonging to this window. Clearing the complete
    // taskbar/process caches here caused unrelated windows to repeat expensive
    // UI Automation lookups after any window was destroyed.
...
void LoadAnimSettings() {
    ...
    // A fresh key intentionally avoids inheriting the disabled value persisted
    // by v1.0, where launch animation defaulted to off.
    g_launchAnimation.store(Wh_GetIntSetting(L"launch_genie") != 0, std::memory_order_relaxed);
    g_maximizeAnimation.store(Wh_GetIntSetting(L"maximize_animation") != 0,
                              std::memory_order_relaxed);
    g_maximizeDurationMs.store(
        Clamp(Wh_GetIntSetting(L"maximize_duration_ms"), 120, 700),
        std::memory_order_relaxed);
    // Alt+Tab animation is intentionally disabled. Native Windows switching
    // stays untouched; old persisted settings cannot re-enable the hooks.
    g_switchAnimation.store(false, std::memory_order_relaxed);
...
static bool IsLaunchWindow(HWND hWnd) {
    ...
    const auto cls = GetClassNameStr(hWnd);
    if (ContainsClass(cls, kAlwaysExcludedClasses)) return false;
    ...
    // Avoid tiny helper/message windows while allowing modern custom-framed
    // applications which don't expose WS_CAPTION.
...
int GetTaskbarButtonX_Async(HWND hWndApp, const WCHAR* windowTitle, int fallbackX, HMONITOR hMon) {
    ...
    // A taskbar position normally stays stable for the lifetime of the window
    // (and the process/monitor cache covers recreated windows). Avoid spawning a
    // COM/UIA worker and synchronously waiting when we already have a good match.
    if (haveCache) return cachedX;
    ...
    if (!pending) return haveCache ? cachedX : fallbackX;
    ...
    auto* task = new (std::nothrow) UiaTask{
        hWndApp, titleLower, procNameLower, processKey, hMon, fallbackX, pending};
    if (!task || !StartWorkerThread(UiaWorkerThread, task)) {
        delete task;
        pending->Release();
        pending->Release();
        return haveCache ? cachedX : fallbackX;
...
DWORD WINAPI AltTabTrackerThread(LPVOID lpParam) {
    ...
    while (g_altTabThreadRunning.load(std::memory_order_relaxed) && !g_unloading.load(std::memory_order_relaxed)) {
        if (g_nativeAltTabHookInstalled.load(std::memory_order_acquire)) {
            Sleep(50);
            continue;
        }
...
void CALLBACK ForegroundEventProc(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hWnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
    ...
    if (IsExplorerProcess() &&
        g_nativeAltTabHookInstalled.load(std::memory_order_acquire)) {
        if (InterlockedCompareExchange(&g_nativeAltTabPending, 0, 0) == 0 ||
            !hWnd || hWnd == g_nativeAltTabSource.load(std::memory_order_acquire) ||
            IsIconic(hWnd) || !IsAppMainWindow(hWnd, true)) {
            return;
        }

        HWND preCloaked = g_nativeAltTabPreCloakedTarget.load(
            std::memory_order_acquire);
        if (preCloaked && preCloaked != hWnd) {
            ReleasePreCloakedAltTabTarget();
            preCloaked = NULL;
        }
        ...
        if (SUCCEEDED(DwmGetWindowAttribute(
            ...
            if (preCloaked == hWnd &&
                g_nativeAltTabPreCloakedTarget.compare_exchange_strong(
                    preCloaked, NULL, std::memory_order_acq_rel)) {
                InterlockedExchange(&g_nativeAltTabPending, 0);
                InterlockedIncrement(&g_nativeAltTabGeneration);
                Wh_Log(L"Native Alt+Tab committed pre-cloaked target hwnd=%p", hWnd);
                LONG_PTR originalExStyle = GetWindowLongPtrW(hWnd, GWL_EXSTYLE);
                if (!StartAnimation(hWnd, TRUE, originalExStyle, TRUE, FALSE, 0, NULL)) {
                    SetWindowCloak(hWnd, FALSE);
                    UpdateDwmTransitions(hWnd, TRUE);
                    Wh_Log(L"Native Alt+Tab pre-cloaked Genie could not start hwnd=%p", hWnd);
                }
                return;
            }
            InterlockedExchange(&g_nativeAltTabPending, 0);
            InterlockedIncrement(&g_nativeAltTabGeneration);
            Wh_Log(L"Native Alt+Tab target already covered by restore hwnd=%p", hWnd);
            return;
        }

        if (InterlockedExchange(&g_nativeAltTabPending, 0) == 0) {
            return;
        }
        InterlockedIncrement(&g_nativeAltTabGeneration);
        Wh_Log(L"Native Alt+Tab foreground committed; cloaking immediately hwnd=%p", hWnd);
        LONG_PTR originalExStyle = GetWindowLongPtrW(hWnd, GWL_EXSTYLE);
        UpdateDwmTransitions(hWnd, FALSE);
        SetWindowCloak(hWnd, TRUE);
        if (!StartAnimation(hWnd, TRUE, originalExStyle, TRUE, FALSE, 0, NULL)) {
            Wh_Log(L"Native Alt+Tab immediate Genie could not start hwnd=%p", hWnd);
        }
        return;
    }
    ...
    if (IsAnimating(hWnd)) {
        Wh_Log(L"Alt+Tab Genie already covered by an active restore hwnd=%p", hWnd);
        return;
    }

    Wh_Log(L"Alt+Tab Genie start hwnd=%p", hWnd);
    LONG_PTR originalExStyle = GetWindowLongPtrW(hWnd, GWL_EXSTYLE);
    UpdateDwmTransitions(hWnd, FALSE);
    SetWindowCloak(hWnd, TRUE);
    if (!StartAnimation(hWnd, TRUE, originalExStyle, TRUE, FALSE, 0, NULL)) {
        Wh_Log(L"Alt+Tab Genie could not start hwnd=%p", hWnd);
    }
}
DWORD WINAPI NativeAltTabCommitThread(LPVOID parameter) {
    auto* request = static_cast<NativeAltTabCommitData*>(parameter);
    HWND source = request->source;
    LONG generation = request->generation;
    delete request;

    DWORD deadline = GetTickCount() + 3000;
    while ((GetAsyncKeyState(VK_MENU) & 0x8000) != 0) {
        if (generation != g_nativeAltTabGeneration ||
    ...
    if (!target || target == source || IsIconic(target) ||
        !IsAppMainWindow(target, true) || IsAnimating(target)) {
        ReleasePreCloakedAltTabTarget();
        return 0;
    }
    ...
    HWND preCloaked = g_nativeAltTabPreCloakedTarget.load(
        std::memory_order_acquire);
    if (preCloaked && preCloaked != target) {
        ReleasePreCloakedAltTabTarget();
        preCloaked = NULL;
    }
    ...
    UINT cloaked = 0;
    if (SUCCEEDED(DwmGetWindowAttribute(
            target, DWMWA_CLOAKED, &cloaked, sizeof(cloaked))) && cloaked) {
        if (preCloaked == target &&
            g_nativeAltTabPreCloakedTarget.compare_exchange_strong(
                preCloaked, NULL, std::memory_order_acq_rel)) {
            Wh_Log(L"Native Alt+Tab fallback committed pre-cloaked target hwnd=%p", target);
            LONG_PTR originalExStyle = GetWindowLongPtrW(target, GWL_EXSTYLE);
            if (!StartAnimation(target, TRUE, originalExStyle, TRUE, FALSE, 0, NULL)) {
                SetWindowCloak(target, FALSE);
                UpdateDwmTransitions(target, TRUE);
            }
            return 0;
        }
        Wh_Log(L"Native Alt+Tab target already covered by restore hwnd=%p", target);
        return 0;
    }

    Wh_Log(L"Native Alt+Tab committed; starting Genie hwnd=%p", target);
    LONG_PTR originalExStyle = GetWindowLongPtrW(target, GWL_EXSTYLE);
    UpdateDwmTransitions(target, FALSE);
    SetWindowCloak(target, TRUE);
    if (!StartAnimation(target, TRUE, originalExStyle, TRUE, FALSE, 0, NULL)) {
        Wh_Log(L"Native Alt+Tab Genie could not start hwnd=%p", target);
    }
    return 0;
}
void CALLBACK LaunchShowEventProc(HWINEVENTHOOK hWinEventHook, DWORD event,
    ...
    if (PrepareVisibleLaunchAnim(hWnd, &originalStyle)) {
        Wh_Log(L"Launch fallback detected visible window hwnd=%p", hWnd);
        CommitLaunchAnim(hWnd, originalStyle);
    }
}

void CALLBACK SystemSwitchEventProc(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hWnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
    if (!g_switchAnimation.load(std::memory_order_relaxed) ||
        g_nativeAltTabHookInstalled.load(std::memory_order_acquire) ||
        g_unloading.load(std::memory_order_relaxed)) {
        return;
    ...
    if (event == EVENT_SYSTEM_SWITCHSTART) {
        BeginAltTabSession(GetForegroundWindow());
        Wh_Log(L"Alt+Tab switch-start event detected");
    } else if (event == EVENT_SYSTEM_SWITCHEND) {
        TouchAltTabSession();
        Wh_Log(L"Alt+Tab switch-end event detected");
    }
}
DWORD WINAPI WinEventHookThread(LPVOID lpParam) {
    HWINEVENTHOOK foregroundHook = NULL;

    // Alt+Tab animation is intentionally disabled in the current build, so
    // don't subscribe every injected process to foreground changes unless that
    // feature is explicitly active in a future build.
    if (g_switchAnimation.load(std::memory_order_relaxed)) {
        ...
        g_hForegroundHook.store(foregroundHook, std::memory_order_release);

        if (!foregroundHook) {
            Wh_Log(L"Foreground event listener unavailable error=%lu",
                   GetLastError());
        }
    }

    HWINEVENTHOOK objectShowHook = NULL;
    if (g_launchAnimation.load(std::memory_order_relaxed)) {
        objectShowHook = SetWinEventHook(
            EVENT_OBJECT_SHOW,
            EVENT_OBJECT_SHOW,
            NULL,
            LaunchShowEventProc,
            ...
            WINEVENT_OUTOFCONTEXT);
        g_hObjectShowHook.store(objectShowHook, std::memory_order_release);
        if (!objectShowHook) {
            Wh_Log(L"Launch show-event listener unavailable error=%lu", GetLastError());
        }
    }

    HWINEVENTHOOK systemSwitchHook = NULL;
    if (g_switchAnimation.load(std::memory_order_relaxed) &&
        IsExplorerProcess()) {
        systemSwitchHook = SetWinEventHook(
            EVENT_SYSTEM_SWITCHSTART,
            EVENT_SYSTEM_SWITCHEND,
            NULL,
            SystemSwitchEventProc,
            0, 0,
            WINEVENT_OUTOFCONTEXT);
        g_hSystemSwitchHook.store(systemSwitchHook, std::memory_order_release);
        if (systemSwitchHook) {
            Wh_Log(L"Alt+Tab system event listener installed");
        } else {
            Wh_Log(L"Alt+Tab system event listener unavailable; fast fallback active");
        }
    }
        
    MSG msg{};
    for (;;) {
        const BOOL getMessageResult = GetMessageW(&msg, NULL, 0, 0);
        if (getMessageResult == 0) break;
        if (getMessageResult == -1) {
            Wh_Log(L"WinEvent message loop failed error=%lu", GetLastError());
            break;
        }
        TranslateMessage(&msg);
        DispatchMessageW(&msg);
    ...
    if (HWINEVENTHOOK oldHook = g_hSystemSwitchHook.exchange(NULL, std::memory_order_acquire)) {
        UnhookWinEvent(oldHook);
    }
    if (HWINEVENTHOOK oldHook = g_hObjectShowHook.exchange(NULL, std::memory_order_acquire)) {
        UnhookWinEvent(oldHook);
    }
    return 0;
}
...
bool StartAnimation(HWND hWnd, BOOL rising, LONG_PTR originalExStyle, BOOL cloakHidden = FALSE, BOOL isClosing = FALSE, UINT closeMsg = 0, HANDLE hWaitFinish = NULL) {
    ...
    static std::atomic<unsigned> s_animCount{0};
    const unsigned animationNumber =
        s_animCount.fetch_add(1, std::memory_order_relaxed) + 1;
    if ((animationNumber & 0x0Fu) == 0) {
        SweepStaleData();
    }
    ...
    int learnedTargetX = mi.rcMonitor.left + monWidth / 2;

    // Only Genie needs a taskbar destination. Ink Splash and close effects don't
    // consume targetDockX, so skip registry/cursor/UIA work for those paths.
    if (!isClosing &&
        g_minRestoreEffectStyle.load(std::memory_order_relaxed) == 0) {
        DWORD alignVal = 1;
        ...
        if (!PtInRect(&workArea, pt)) {
            ...
            GetWindowTextW(
                hWnd, windowTitle, ARRAYSIZE(windowTitle));
            learnedTargetX = GetTaskbarButtonX_Async(
                hWnd, windowTitle, learnedTargetX, hMon);
        }
    }
    ...
    auto* data = new (std::nothrow) WindowAnimData{
        hWnd, nullptr, nullptr, rect, hMon, w, h, learnedTargetX, rising,
        originalExStyle, cloakHidden, nullptr, durationMs, isClosing, closeMsg,
        hWaitFinish
    };
    if (!data) {
        Wh_Log(L"Animation data allocation failed hwnd=%p", hWnd);
        {
            std::lock_guard<std::mutex> lock(g_StateMutex);
            g_AnimActive.erase(hWnd);
        }
    ...
    auto CaptureNow = [&]() -> bool {
        ...
        if (forceOpaque) {
            captured = BitBlt(
                tempDC, 0, 0, rawW, rawH,
                hScreenDC, winRect.left, winRect.top, SRCCOPY);
        } else {
            captured = PrintWindow(hWnd, tempDC, PW_RENDERFULLCONTENT);

            // Screen-copy fallback is valid only while the real window is still
            // visible. During launch/restore the real window can be alpha=0 or
            // DWM-cloaked, so BitBlt would capture whatever is behind it.
            if (!captured && !rising) {
                captured = BitBlt(
                    tempDC, 0, 0, rawW, rawH,
                    hScreenDC, winRect.left, winRect.top, SRCCOPY);
    ...
    if (rising) {
        ...
        if (snapshotReady && !isClosing) {
            ...
            HBITMAP hCacheBmp =
                CreateDib32(hScreenDC, w, h, &pCacheBits);

            if (hCacheBmp && pCacheBits) {
                memcpy(
                    pCacheBits, data->pBits,
                    (size_t)w * (size_t)h * 4);
                g_WndSnapshots[hWnd] =
                    {hCacheBmp, pCacheBits, w, h};
            } else if (hCacheBmp) {
    ...
    if (!snapshotReady) {
        Wh_Log(L"Animation snapshot unavailable hwnd=%p rising=%d closing=%d",
               hWnd, rising, isClosing);

        if (rising) {
            UndoRisingHide(
                hWnd, data->originalExStyle, data->hiddenByCloak);
        } else {
            UpdateDwmTransitions(hWnd, TRUE);
        }
...
static void ForgetLaunchSeen(HWND hWnd) {
    std::lock_guard<std::mutex> lock(g_StateMutex);
    g_LaunchSeen.erase(hWnd);
}

static void RestorePreparedLaunchWindow(HWND hWnd, LONG_PTR originalExStyle,
                                        bool forgetLaunch) {
    if (IsWindow(hWnd)) {
        SetLayeredWindowAttributes(hWnd, 0, 255, LWA_ALPHA);
        if (!(originalExStyle & WS_EX_LAYERED)) {
    ...
    if (forgetLaunch) ForgetLaunchSeen(hWnd);
}

static bool PrepareLaunchAnimCommon(HWND hWnd, LONG_PTR* origExOut,
                                    bool requireAlreadyVisible) {
    if (!origExOut || g_unloading.load(std::memory_order_relaxed)) return false;
    if (!g_launchAnimation.load(std::memory_order_relaxed)) return false;
    if (IsIconic(hWnd) || !IsLaunchWindow(hWnd)) return false;

    const bool visible = IsWindowVisible(hWnd) != FALSE;
    if (requireAlreadyVisible != visible) return false;
...
static bool PrepareLaunchAnim(HWND hWnd, int nCmdShow, LONG_PTR* origExOut) {
    ...
    return PrepareLaunchAnimCommon(hWnd, origExOut, false);
}

static bool PrepareVisibleLaunchAnim(HWND hWnd, LONG_PTR* origExOut) {
    return PrepareLaunchAnimCommon(hWnd, origExOut, true);
}

static void CommitLaunchAnim(HWND hWnd, LONG_PTR originalExStyle) {
    auto* ld = new (std::nothrow) LaunchAnimData{hWnd, originalExStyle};
    if (!ld || !StartWorkerThread(LaunchAnimThread, ld)) {
        delete ld;
        RestorePreparedLaunchWindow(hWnd, originalExStyle, true);
    }
}
...
static void CleanupResizeAnimation(ResizeAnimData* data) {
    if (!data) return;
    if (IsWindow(data->hWnd)) {
        SetWindowCloak(data->hWnd, FALSE);
        UpdateDwmTransitions(data->hWnd, TRUE);
    }
    if (data->hThumb) DwmUnregisterThumbnail(data->hThumb);
    if (data->hGhost) DestroyWindow_Original(data->hGhost);
    {
        std::lock_guard<std::mutex> lock(g_StateMutex);
        g_AnimActive.erase(data->hWnd);
    }
    delete data;
}
static ResizeAnimData* PrepareResizeAnimation(HWND hWnd) {
    if (!g_maximizeAnimation.load(std::memory_order_relaxed) ||
        g_unloading.load(std::memory_order_relaxed) || IsIconic(hWnd) ||
        !IsAppMainWindow(hWnd, true) || IsAnimating(hWnd)) {
        return nullptr;
    }

    RECT fromRect{};
    if (!GetWindowRect(hWnd, &fromRect)) return nullptr;
    ...
    if (virtualWidth <= 0 || virtualHeight <= 0) return nullptr;

    HWND hGhost = CreateWindowExW(
        WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_NOACTIVATE |
            WS_EX_NOREDIRECTIONBITMAP,
        L"STATIC", NULL, WS_POPUP, virtualRect.left, virtualRect.top,
        virtualWidth, virtualHeight, NULL, NULL, NULL, NULL);
    if (!hGhost) return nullptr;

    HTHUMBNAIL hThumb = NULL;
    HRESULT hr = DwmRegisterThumbnail(hGhost, hWnd, &hThumb);
    if (FAILED(hr)) {
        DestroyWindow_Original(hGhost);
        Wh_Log(L"Maximize thumbnail register failed hwnd=%p hr=0x%08X", hWnd, hr);
        return nullptr;
    }
    ...
    props.rcDestination = {
        fromRect.left - virtualRect.left,
        fromRect.top - virtualRect.top,
        fromRect.right - virtualRect.left,
        fromRect.bottom - virtualRect.top,
    };
    if (FAILED(DwmUpdateThumbnailProperties(hThumb, &props))) {
    ...
    auto* data = new (std::nothrow) ResizeAnimData{
        hWnd, hGhost, hThumb, fromRect, fromRect, virtualRect,
...
static void CommitResizeAnimation(ResizeAnimData* data, PCWSTR kind) {
    if (!data) return;
    if (!IsWindow(data->hWnd) || !GetWindowRect(data->hWnd, &data->toRect) ||
        EqualRect(&data->fromRect, &data->toRect)) {
        CleanupResizeAnimation(data);
        return;
    }

    Wh_Log(L"Window %s animation start hwnd=%p duration=%d",
           kind, data->hWnd, data->durationMs);
    ...
    for (;;) {
        ...
        const bool lastFrame = elapsedMs >= data->durationMs;
        const float progress = lastFrame
                                   ? 1.0f
                                   : static_cast<float>(elapsedMs / data->durationMs);
        const float ease = 1.0f - powf(1.0f - progress, 3.0f);
        auto lerp = [ease](LONG from, LONG to) {
            return static_cast<LONG>(from + (to - from) * ease);
        };

        RECT frame{
            lerp(data->fromRect.left, data->toRect.left) - data->virtualRect.left,
            lerp(data->fromRect.top, data->toRect.top) - data->virtualRect.top,
            lerp(data->fromRect.right, data->toRect.right) - data->virtualRect.left,
            lerp(data->fromRect.bottom, data->toRect.bottom) - data->virtualRect.top,
        };
        DWM_THUMBNAIL_PROPERTIES props{};
        props.dwFlags = DWM_TNP_VISIBLE | DWM_TNP_RECTDESTINATION |
                        DWM_TNP_OPACITY;
        props.fVisible = TRUE;
        props.opacity = 255;
        props.rcDestination = frame;
        DwmUpdateThumbnailProperties(data->hThumb, &props);
        if (lastFrame || g_unloading.load(std::memory_order_relaxed)) break;
        DwmFlush();
    }
...
BOOL WINAPI ShowWindow_Hook(HWND hWnd, int cmd) {
    ...
    if (IsMinimizeCommand(cmd)) {
        if (g_minimizeAnimation.load(std::memory_order_relaxed) &&
            ...
            UpdateDwmTransitions(hWnd, FALSE);
            StartAnimation(
                hWnd, FALSE, GetWindowLongPtrW(hWnd, GWL_EXSTYLE));
        }
        return ShowWindow_Original(hWnd, cmd);
    }
    ...
    if ((cmd == SW_RESTORE || cmd == SW_SHOWNORMAL) && IsIconic(hWnd)) {
        if (g_restoreAnimation.load(std::memory_order_relaxed) &&
            ...
            if (!StartAnimation(
                    hWnd, TRUE, GetWindowLongPtrW(hWnd, GWL_EXSTYLE), TRUE)) {
                SetWindowCloak(hWnd, FALSE);
                UpdateDwmTransitions(hWnd, TRUE);
            }
            return result;
    ...
    // IMPORTANT: Detect a hidden first-show before treating SW_SHOWMAXIMIZED as
    // a normal maximize. Previously a freshly launched maximized app skipped
    // the Genie path completely.
    LONG_PTR originalStyle = 0;
    ...
    const bool maximize =
        cmd == SW_SHOWMAXIMIZED && !IsZoomed(hWnd) && !IsIconic(hWnd);
    const bool unmaximize =
        (cmd == SW_RESTORE || cmd == SW_SHOWNORMAL) && IsZoomed(hWnd);

    if (maximize || unmaximize) {
        ResizeAnimData* resize = PrepareResizeAnimation(hWnd);
        BOOL result = ShowWindow_Original(hWnd, cmd);
        CommitResizeAnimation(
            resize, maximize ? L"maximize" : L"unmaximize");
        return result;
    }
...
LRESULT WINAPI DefWindowProcW_Hook(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (msg == WM_SYSCOMMAND) {
        ...
        if (cmd == SC_MAXIMIZE || (cmd == SC_RESTORE && IsZoomed(hWnd))) {
            ResizeAnimData* resize = PrepareResizeAnimation(hWnd);
            LRESULT result = DefWindowProcW_Original(hWnd, msg, wParam, lParam);
            CommitResizeAnimation(resize,
                                  cmd == SC_MAXIMIZE ? L"maximize" : L"unmaximize");
            return result;
        }
    }
    return DefWindowProcW_Original(hWnd, msg, wParam, lParam);
...
BOOL WINAPI SetWindowPlacement_Hook(
    ...
    if (IsMinimizeCommand(placement->showCmd)) {
    ...
    if ((placement->showCmd == SW_RESTORE ||
    ...
    // First-show launch has to be checked before maximize/unmaximize.
    LONG_PTR originalStyle = 0;
    if (PrepareLaunchAnim(hWnd, placement->showCmd, &originalStyle)) {
        BOOL result = SetWindowPlacement_Original(hWnd, placement);
        if (result) {
            CommitLaunchAnim(hWnd, originalStyle);
        } else {
            RestorePreparedLaunchWindow(hWnd, originalStyle, true);
        }
        return result;
    }
...
DWORD WINAPI LaunchAnimThread(LPVOID lpParam) {
    ...
    if (g_unloading.load(std::memory_order_relaxed) || !IsWindow(hWnd) ||
        ...
        const bool retryLater =
            IsWindow(hWnd) && !IsWindowVisible(hWnd) &&
            !g_unloading.load(std::memory_order_relaxed);
        RestorePreparedLaunchWindow(hWnd, originalExStyle, retryLater);
        return 0;
    }

    if (!StartAnimation(hWnd, TRUE, originalExStyle)) {
        RestorePreparedLaunchWindow(hWnd, originalExStyle, false);
    }
    return 0;
}
...
BOOL Wh_ModInit() {
    ...
    if (explorerProcess) {
        ResetAltTabState();
        if (g_switchAnimation.load(std::memory_order_relaxed)) {
            InstallNativeAltTabHook();
            Wh_SetFunctionHook((void*)SetForegroundWindow,
                               (void*)SetForegroundWindow_Hook,
                               (void**)&SetForegroundWindow_Original);
            Wh_SetFunctionHook((void*)SwitchToThisWindow,
                               (void*)SwitchToThisWindow_Hook,
    ...
    Wh_Log(L"Init ok minimize=%d restore=%d maximize=%d close=%d switch=%d launch=%d explorer=%d",
...
void Wh_ModSettingsChanged() { 
    LoadAnimSettings(); 
    Wh_Log(L"Settings changed; Alt+Tab animation remains disabled");
}
