...
// Unique GUID for this tray icon - prevents Windows from grouping with other processes
DEFINE_GUID(GUID_MicSwitcherTrayIcon, 
...
// Unique AppUserModelID for the tool process — must be different from Windhawk's AppUserModelID.
static const PCWSTR g_toolAppId = L"BlackPaw.MicSwitcher.ToolMod";
...
void UpdateTrayTip(HWND hWnd, BOOL isAdd) {
    ...
    nid.guidItem = GUID_MicSwitcherTrayIcon;
...
DWORD WINAPI TrayThreadProc(LPVOID) {
    ...
    if (SUCCEEDED(SHGetPropertyStoreForWindow(g_trayHwnd, IID_PPV_ARGS(&pps)))) {
        ...
        if (var.pwszVal) {
            lstrcpyW(var.pwszVal, g_toolAppId);
...
BOOL WhTool_ModInit() {
    ...
    // Set process AppUserModelID BEFORE any windows are created - critical!
    typedef HRESULT (WINAPI *SetAppIDFunc)(PCWSTR);
    HMODULE hShell = LoadLibraryW(L"shell32.dll");
    if (hShell) {
        SetAppIDFunc pSetAppID = (SetAppIDFunc)GetProcAddress(hShell, "SetCurrentProcessExplicitAppUserModelID");
        if (pSetAppID) {
            HRESULT hr = pSetAppID(g_toolAppId);
            Wh_Log(L"SetCurrentProcessExplicitAppUserModelID result: 0x%X", hr);
        }
        FreeLibrary(hShell);
    }
