...
- The genie targets the taskbar edge of the monitor where the window is located.
...
struct MacGenieAnimData {
    ...
    RECT monitorRect;     // bounds of the monitor where the animation runs
};
...
DWORD WINAPI MacGenieAnimThread(LPVOID lpParam) {
    ...
    const int monitorLeft = data->monitorRect.left;
    const int monitorRight = data->monitorRect.right;
    const int monitorTop = data->monitorRect.top;
    const int monitorBottom = data->monitorRect.bottom;
    ...
    if (dockX < monitorLeft) dockX = monitorLeft;
    if (dockX > monitorRight) dockX = monitorRight;
    const float dockXf = (float)dockX;
    const float dockY  = (float)monitorBottom;          // flow into the taskbar edge
    float neckW = W * 0.03f;
    ...
    int boundBottom = monitorBottom;
    if (boundLeft < monitorLeft) boundLeft = monitorLeft;
    if (boundRight > monitorRight) boundRight = monitorRight;
    if (boundTop < monitorTop) boundTop = monitorTop;
    if (boundBottom > monitorBottom) boundBottom = monitorBottom;
...
void StartMacGenieAnim(HWND hWnd, BOOL rising) {
    ...
    int learnedTargetX = mi.rcMonitor.left + (mi.rcMonitor.right - mi.rcMonitor.left) / 2; // Default to center of this monitor

    // If the mouse is outside the workspace (e.g. hovering over the taskbar)
