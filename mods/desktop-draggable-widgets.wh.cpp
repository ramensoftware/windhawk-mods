...
// FIX B: custom message used to re-show the window after the three-finger
// swipe gesture (or Win+D) has hidden it. Posted from WM_SHOWWINDOW so the
// re-show happens on the UI thread after the hide has fully committed.
constexpr UINT kRestoreWindowMessage   = WM_APP + 4;
...
void ApplyDefaultWidgetLayout() {
    ...
    if (!g_searchWidgetVertical && !g_personalizationWidgetVertical) {
        ...
        g_widgets[WidgetSearch].rc          = RectF(0,   y, kSearchVerticalWidth, kVerticalWidgetHeight);
        g_widgets[WidgetPersonalization].rc = RectF(93.333333f, y, 256.666667f, 30);
        y += kVerticalWidgetHeight + 20.0f;
    } else {
        g_widgets[WidgetSearch].rc          = RectF(0,   y, 256.666667f, 30);
...
void DrawRoundShortcut(Graphics& g, const Widget& widget, const wchar_t* label,
    ...
    Font font(&family,
              wcscmp(label, L"Accessibility") == 0 ? 8.4f : 10.0f,
...
LRESULT CALLBACK WidgetWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        // ── FIX C: Survive three-finger swipe / Win+D / Show Desktop ─────
        //
        // When the user performs the three-finger swipe-down gesture (or
        // presses Win+D / clicks Show Desktop), Windows iterates all
        // non-topmost top-level windows and hides them.  For a WS_POPUP
        // WS_EX_LAYERED window like ours the sequence is:
        //
        //   1. WM_WINDOWPOSCHANGING  with flags containing SWP_HIDEWINDOW
        //   2. WM_SHOWWINDOW         with wParam=FALSE, lParam=SW_PARENTCLOSING
        //   3. WM_WINDOWPOSCHANGED
        //
        // The three handlers below form a defense-in-depth triple guard:
        //
        //  Guard 1 — WM_WINDOWPOSCHANGING:
        //    Strip the SWP_HIDEWINDOW flag (0x0080) from WINDOWPOS.flags
        //    *before* the system commits the hide. This is the earliest and
        //    cleanest intercept point; on most Windows 11 builds it alone is
        //    enough to prevent the widget from disappearing.
        //
        //  Guard 2 — WM_SHOWWINDOW (wParam=FALSE, lParam=SW_PARENTCLOSING):
        //    Skip DefWindowProc so the window-state hide is not committed,
        //    then post kRestoreWindowMessage to re-show on the next message
        //    pump iteration.  This catches builds / code paths where
        //    WM_WINDOWPOSCHANGING alone does not fully suppress the hide
        //    (e.g. the WS_EX_TOOLWINDOW "minimize all" path on some builds).
        //
        //  Guard 3 — kRestoreWindowMessage / WM_SIZE (SIZE_MINIMIZED):
        //    Belt-and-braces restore for the edge case where the window
        //    actually did get hidden or minimized before we could intercept.
        //    ShowWindow + SetWindowPos + HWND_BOTTOM ensures it is visible
        //    and stays at the correct z-order (behind normal app windows).

        case WM_WINDOWPOSCHANGING: {
            // Guard 1 — strip the hide flag before the system acts on it.
        ...
        case WM_SHOWWINDOW: {
            // Guard 2 — intercept the hide-all sequence.
            // wParam==FALSE  → window is being hidden
            // lParam==SW_PARENTCLOSING (1) → owner/show-desktop-driven hide
            if (wParam == FALSE && lParam == SW_PARENTCLOSING) {
                // Skip DefWindowProc (which would commit the hide) and
                // schedule a re-show on the next pump iteration.
                PostMessageW(hwnd, kRestoreWindowMessage, 0, 0);
                return 0;
            }
            break;
        }

        case kRestoreWindowMessage: {
            // Guard 2 (deferred) — re-show and push back to HWND_BOTTOM.
            ShowWindow(hwnd, SW_SHOWNOACTIVATE);
            RECT wa = DesktopWorkArea();
        ...
        case WM_SIZE: {
            // Guard 3 — if somehow minimized, restore immediately.
