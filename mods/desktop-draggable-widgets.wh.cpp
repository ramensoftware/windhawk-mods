...
// @description     Draggable Windows desktop widgets with transparent, light, and dark themes for calendar, clock, music, volume, battery, search, personalization, WiFi, Bluetooth, hotspot, and accessibility. You can add a custom folder path in the mod settings to open your desired music folder.
// @version         2.1
...
![image](https://i.imgur.com/YZev8jU.png)
*Acrylic better for non white or black wallpapers*
...
ThemePalette g_palettes[] = {
    // Unlike the other palettes below (all fully opaque, kSolid alpha),
    // Acrylic/Translucent deliberately uses partial alpha on header/body/
    // accent so the wallpaper behind the widgets shows through — that's
    // what makes it read as frosted glass rather than a flat color panel.
    // Text colors stay fully opaque for legibility. header ≈ Windows'
    // classic #0078D7 accent blue (used for the ribbon, clock dial, and
    // slider fill); body/accent are a translucent dark charcoal.
...
void CopyTrimmedPathSetting(PCWSTR value, wchar_t* outBuffer, size_t outBufferSize) {
    outBuffer[0] = L'\0';
    if (!value || outBufferSize == 0) {
        return;
    }

    // Users often paste a path copied with surrounding quotes or extra
    // whitespace (e.g. from the address bar or a terminal). Strip that off
    // so ShellExecute gets a clean path.
    const wchar_t* start = value;
    while (*start == L' ' || *start == L'\t' || *start == L'"' || *start == L'\'') {
        ++start;
    }

    const wchar_t* end = start + wcslen(start);
    while (end > start &&
           (end[-1] == L' ' || end[-1] == L'\t' || end[-1] == L'"' || end[-1] == L'\'')) {
        --end;
    }

    size_t length = static_cast<size_t>(end - start);
    if (length >= outBufferSize) {
        length = outBufferSize - 1;
    }
...
// Saved positions live in a small INI file under %LOCALAPPDATA%, independent
// of Windhawk's own settings storage, so a drag is remembered the moment it
// happens — including across a shutdown/restart — without needing the mod's
// settings UI involved at all.
// ─────────────────────────────────────────────────────────────────────────────
constexpr wchar_t kPositionsFileName[] = L"widget-positions.ini";
constexpr wchar_t kPositionsSection[] = L"Widgets";

bool GetPositionsFilePath(wchar_t* outPath, size_t outPathSize) {
    ...
    if (swprintf_s(dir, L"%s\\WindhawkDesktopGlassWidgets", localAppData) < 0) {
        return false;
    }

    CreateDirectoryW(dir, nullptr);

    return swprintf_s(outPath, outPathSize, L"%s\\%s", dir, kPositionsFileName) >= 0;
}

void SaveWidgetPositions() {
    wchar_t path[MAX_PATH * 2];
    if (!GetPositionsFilePath(path, ARRAYSIZE(path))) {
        return;
    }

    for (const Widget& widget : g_widgets) {
        wchar_t key[16];
        swprintf_s(key, L"%d", widget.id);

        wchar_t value[128];
        swprintf_s(value, L"%.2f,%.2f,%.2f,%.2f",
                   widget.rc.X, widget.rc.Y, widget.rc.Width, widget.rc.Height);

        WritePrivateProfileStringW(kPositionsSection, key, value, path);
    }
}
...
void LoadWidgetPositions() {
    ...
    for (Widget& widget : g_widgets) {
        ...
        wchar_t value[128];
        DWORD read = GetPrivateProfileStringW(kPositionsSection, key, L"", value,
                                              ARRAYSIZE(value), path);
        if (read == 0) {
            continue;
        }

        // Only the top-left position (X/Y) is restored here. Width/height
        // always come from ApplyDefaultWidgetLayout() /
        // ApplyOrientationResizeIfNeeded(), which size each widget from the
        // current orientation settings — there's no manual resize feature.
        // Restoring a saved width/height too would let a stale size (e.g.
        // saved while Volume/Battery were horizontal) override the current
        // orientation's size on the next load, which throws off the
        // vertical slider math and can make the percentage indicator draw
        // off-widget or not render at all.
        float x, y, w, h;
        if (swscanf_s(value, L"%f,%f,%f,%f", &x, &y, &w, &h) == 4) {
            widget.rc.X = x;
            widget.rc.Y = y;
...
void OpenMusicFolder() {
    if (g_musicFolderPath[0] != L'\0' && TryOpenUri(g_musicFolderPath)) {
        return;
    }

    // No custom folder configured, or it failed to open (e.g. path no
    // longer exists) — fall back to the default Music folder.
...
// ── DrawAcrylicGrain ──────────────────────────────────────────────────────────
// Real Windows Acrylic layers a fine noise texture on top of the tinted
// blur — without it, a translucent tint just looks like a flat color wash
// instead of frosted glass. This window draws its own bitmap rather than
// live-blurring the desktop behind it, so a sparse speckle grain is used
// as a cheap stand-in for that texture. The pattern is derived from each
// pixel's coordinates (not time), so it stays put instead of shimmering on
// the once-a-second re-render. Only runs for the Acrylic/Translucent theme;
// every other theme is unaffected.
// ─────────────────────────────────────────────────────────────────────────────
void DrawAcrylicGrain(Graphics& g, const Widget& widget) {
    if (g_appearanceTheme != AppearanceTheme::AcrylicTranslucent) {
        return;
    }
    ...
    if (widget.round) {
        ...
        const float d = 36.0f;  // matches the 18px radius used by DrawRoundedRectangle
        RectF rc = widget.rc;
    ...
    for (int y = top; y < bottom; y += 3) {
        for (int x = left; x < right; x += 3) {
            // Cheap integer hash (no <random> dependency) — deterministic
            // per pixel coordinate, roughly uniform in the low byte.
            unsigned int h = static_cast<unsigned int>(x) * 374761393u +
                             static_cast<unsigned int>(y) * 668265263u;
            h = (h ^ (h >> 13)) * 1274126177u;
            h ^= (h >> 16);
            if ((h & 0xFF) < 16) {  // ~6% of sampled points get a speckle
                Brush* speckle = ((h >> 8) & 1) ? static_cast<Brush*>(&lightSpeckle)
                                                 : static_cast<Brush*>(&darkSpeckle);
                g.FillRectangle(speckle, static_cast<float>(x), static_cast<float>(y), 1.0f, 1.0f);
            }
        }
    }
...
LRESULT CALLBACK WidgetWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_LBUTTONUP: {
            ...
            if (wasMoved && releasedWidget >= 0) {
                // Save right away so the new position survives even if the
                // PC is shut down or restarted without a clean app exit.
                SaveWidgetPositions();
            }
            return 0;
        }
        ...
        case WM_QUERYENDSESSION:
            // Fires on shutdown/restart/logoff before the process is torn
            // down. Save here too, as a belt-and-braces backup to the
            // save-on-drag-release above.
            SaveWidgetPositions();
            return TRUE;
