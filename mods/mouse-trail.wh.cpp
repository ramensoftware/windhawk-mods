...
A highly customizable mouse cursor trail with particle effects, multiple color modes, custom function trails, cursor color extraction, and click effects. Direct2D hardware accelerated. Runs as a dedicated process with zero CPU usage when idle.
...
高度可定制的鼠标拖尾特效，支持粒子系统、多种颜色模式、自定义函数轨迹、光标取色和点击特效。Direct2D 硬件加速，独立进程运行，静止时零 CPU 占用。
