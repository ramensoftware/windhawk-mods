...
void LoadSettings() {
    ...
    if (g_particleInterval < 10)
        g_particleInterval = 10;
...
static void ExtractCursorColor(POINT pt, DWORD dwTime) {
    if (dwTime - s_lastColorExtract < 40)
        return;
...
static void RenderFrame() {
    ...
    if (g_colorMode == 10 || g_colorMode == 11)
        ExtractCursorColor(pt, dwTime);
    ...
    if (!g_particles.empty()) {
        ...
        for (auto &p : g_particles) {
            ...
            g_pD2DDC->FillEllipse(D2D1::Ellipse(pp, p.size * 2.5f, p.size * 2.5f), g_pSolidOuterBrush);
            ...
            if (!particleFastPath && p.shapeType == 2 && g_pStarGeom) {
                ...
                g_pD2DDC->SetTransform(D2D1::Matrix3x2F::Scale(p.size, p.size) *
                ...
                g_pD2DDC->FillEllipse(D2D1::Ellipse(pp, p.size, p.size), g_pSolidOuterBrush);
            }
        }
    ...
    if (tailVisible && havePath) {
        ...
        if (g_trailShape == 1) {
            ...
            if (g_enableTrailShadow && lo.size() >= 2) {
                ...
                g_pD2DFactory->CreatePathGeometry(&pg);
                pg->Open(&ps);
                ...
                for (size_t i = 1; i < lo.size(); ++i)
                    ps->AddLine(D2D1::Point2F(lo[i].x + SHADOW_DX, lo[i].y + SHADOW_DY));
                for (int i = (int)ro.size() - 1; i >= 0; --i)
                    ps->AddLine(D2D1::Point2F(ro[i].x + SHADOW_DX, ro[i].y + SHADOW_DY));
                ps->EndFigure(D2D1_FIGURE_END_CLOSED);
                ps->Close();
                ps->Release();
                g_pShadowBrush->SetOpacity(0.18f * fa);
                g_pD2DDC->FillGeometry(pg, g_pShadowBrush);
                pg->Release();
            }
            if (useEnhancedGlow && gl2.size() >= 2) {
                ...
                pg->Open(&ps);
                ps->SetFillMode(D2D1_FILL_MODE_WINDING);
                ps->BeginFigure(gl2[0], D2D1_FIGURE_BEGIN_FILLED);
                for (size_t i = 1; i < gl2.size(); ++i)
                    ps->AddLine(gl2[i]);
                for (int i = (int)gr2.size() - 1; i >= 0; --i)
                    ps->AddLine(gr2[i]);
                ps->EndFigure(D2D1_FIGURE_END_CLOSED);
                ps->Close();
                ps->Release();
                ...
                g_pD2DDC->FillGeometry(pg, g_pSolidOuterBrush);
                pg->Release();
            }
            if (glowR > 0.1f && gl.size() >= 2) {
                ...
                pg->Open(&ps);
                ps->SetFillMode(D2D1_FILL_MODE_WINDING);
                ps->BeginFigure(gl[0], D2D1_FIGURE_BEGIN_FILLED);
            ...
            ID2D1GeometrySink *ps = nullptr;
            g_pD2DFactory->CreatePathGeometry(&pog);
            pog->Open(&ps);
            ps->SetFillMode(D2D1_FILL_MODE_WINDING);
            ps->BeginFigure(lo[0], D2D1_FIGURE_BEGIN_FILLED);
            for (size_t i = 1; i < lo.size(); ++i)
                ps->AddLine(lo[i]);
            for (int i = (int)ro.size() - 1; i >= 0; --i)
                ps->AddLine(ro[i]);
            ...
            ps->Close();
            ps->Release();
            g_pD2DFactory->CreatePathGeometry(&pcg);
            pcg->Open(&ps);
            ps->SetFillMode(D2D1_FILL_MODE_WINDING);
            ps->BeginFigure(lc[0], D2D1_FIGURE_BEGIN_FILLED);
            ...
            g_pD2DDC->FillGeometry(pog_g, ob);
            g_pD2DDC->FillGeometry(pig_g, ib);
            g_pSolidOuterBrush->SetOpacity(1);
            g_pSolidInnerBrush->SetOpacity(1);
    ...
    if (g_pDCompDevice)
        g_pDCompDevice->Commit();
    if (!isSmearing && g_history.empty() && g_ripples.empty() && g_particles.empty())
        needsClear = false;
}
