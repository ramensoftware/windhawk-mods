...
// @description  Restores rectangular Windows Basic style caption buttons directly in DWM
// @version      10.2
// @author       AmbushSK
...
![Process Inclusion List](https://i.imgur.com/qjCYSSH.png)
...
* Enforces 16x16 pixelated (nearest-neighbor) icon scaling without smooth interpolation.
...
## How It Works
The mod hooks into DWM's `CTopLevelWindow::UpdateNCAreaPositionsAndSizes` and `CTopLevelWindow::UpdateNCAreaButton` functions to override standard button positioning, enforcing rectangular Basic-style dimensions, glyph alignment, and point-sampled visual scaling.
*/
// ==/WindhawkModReadme==
...
int (WINAPI *GetSystemMetricsForDpiOrig)(int nIndex, UINT dpi) = nullptr;
int WINAPI GetSystemMetricsForDpiHook(int nIndex, UINT dpi) {
    if (GetSystemMetricsForDpiOrig) {
...
CTopLevelWindow_UpdatePinnedParts_t CTopLevelWindow_UpdatePinnedParts_Original = nullptr;

typedef void (*CVisual_SetInsetFromParentTop_t)(void* pThis, int inset);
CVisual_SetInsetFromParentTop_t CVisual_SetInsetFromParentTop_Original = nullptr;

typedef void (*CVisual_SetInsetFromParentLeft_t)(void* pThis, int inset);
CVisual_SetInsetFromParentLeft_t CVisual_SetInsetFromParentLeft_Original = nullptr;

typedef void (*CVisual_SetDirtyFlags_t)(void* pThis, int flag);
CVisual_SetDirtyFlags_t CVisual_SetDirtyFlags_Original = nullptr;

typedef void (*CVisual_SetInsetFromParent_t)(void* pThis, struct _MARGINS const *);
CVisual_SetInsetFromParent_t CVisual_SetInsetFromParent_Original = nullptr;

typedef void (*CVisual_SetBitmapScalingMode_t)(void* pThis, int mode);
CVisual_SetBitmapScalingMode_t CVisual_SetBitmapScalingMode_Original = nullptr;

typedef void (*CVisual_SetEdgeMode_t)(void* pThis, int mode);
CVisual_SetEdgeMode_t CVisual_SetEdgeMode_Original = nullptr;

typedef void (*CText_SetSize_t)(void* pThis, struct tagSIZE const *);
CText_SetSize_t CText_SetSize_Original = nullptr;

typedef void (*CSolidColorLegacyMilBrushProxy_Update_t)(void* pThis, double alpha, struct _D3DCOLORVALUE const *);
CSolidColorLegacyMilBrushProxy_Update_t CSolidColorLegacyMilBrushProxy_Update_Original = nullptr;
...
MilInstrumentationCheckHR_MaybeFailFast_t MilInstrumentationCheckHR_MaybeFailFast_Original = nullptr;

typedef long (*CCompositor_CreateMilBrushProxy_t)(__int64, long **);
CCompositor_CreateMilBrushProxy_t CCompositor_CreateMilBrushProxy_Original = nullptr;

typedef long (*CCompositor_CreateMatrixTransformProxy_t)(__int64, char*);
CCompositor_CreateMatrixTransformProxy_t CCompositor_CreateMatrixTransformProxy_Original = nullptr;

typedef long (*CPushTransformInstruction_Create_t)(__int64*, volatile int**);
CPushTransformInstruction_Create_t CPushTransformInstruction_Create_Original = nullptr;

typedef long (*CRenderDataVisual_AddInstruction_t)(void* pThis, volatile signed __int32 *);
CRenderDataVisual_AddInstruction_t CRenderDataVisual_AddInstruction_Original = nullptr;

typedef long (*CBitmapSource_Create_t)(struct IWICBitmap*, const struct _MARGINS*, long long **);
CBitmapSource_Create_t CBitmapSource_Create_Original = nullptr;

typedef long (*CDrawImageInstruction_Create_t)(long long*, const struct tagRECT*, volatile int **);
CDrawImageInstruction_Create_t CDrawImageInstruction_Create_Original = nullptr;

typedef long (*CPopInstruction_Create_t)(volatile int **);
CPopInstruction_Create_t CPopInstruction_Create_Original = nullptr;

typedef void (*CTopLevelWindow_GetAccentBlurBehindBrush_t)(void*, long long**, long long**, void*);
CTopLevelWindow_GetAccentBlurBehindBrush_t CTopLevelWindow_GetAccentBlurBehindBrush_Original = nullptr;

typedef void (*CTopLevelWindow_OnAccentPolicyUpdated_t)(void*);
CTopLevelWindow_OnAccentPolicyUpdated_t CTopLevelWindow_OnAccentPolicyUpdated_Original = nullptr;

typedef void (*CText_ReleaseResources_t)(void* pThis);
CText_ReleaseResources_t CText_ReleaseResources_Original = nullptr;

typedef long (*CVisual_RemoveChild_t)(void* pParent, void* pChild);
CVisual_RemoveChild_t CVisual_RemoveChild_Original = nullptr;

typedef long (*CVisual_AddChild_t)(void* pParent, void* pChild);
CVisual_AddChild_t CVisual_AddChild_Original = nullptr;

long long* DesktopManagerInstance = nullptr;
thread_local bool g_inButtonDraw = false;

void EnsureButtonsOnTop(void* pThis) {
    if (!pThis) return;

    for (int i = 0; i < 4; i++) {
        void* pButton = *(void**)((char*)pThis + 488 + 8 * i);
        if (pButton) {
            DWORD width = *(DWORD*)((char*)pButton + 120);
            if (width == 0) continue;

            if (CVisual_RemoveChild_Original) {
                CVisual_RemoveChild_Original(pThis, pButton);
            }
            if (CVisual_AddChild_Original) {
                CVisual_AddChild_Original(pThis, pButton);
            }
            if (CVisual_SetDirtyFlags_Original) {
                CVisual_SetDirtyFlags_Original(pButton, 2);
            }
        }
    }
}

bool (*CButton__UpdateCurrentGlyphOpacity_Original)(void *, bool) = nullptr;

bool CButton__UpdateCurrentGlyphOpacity_Hook(void *pThis, bool b) {
    if (pThis) {
        *((float *)pThis + 101) = 1.0f;
    }
    return CButton__UpdateCurrentGlyphOpacity_Original ? CButton__UpdateCurrentGlyphOpacity_Original(pThis, b) : false;
}

long long (*CButton_DrawStateW_Original)(void* pThis, void* CAtlasButton, UINT ButtonState) = nullptr;

long long CButton_DrawStateW_Hook(void* pThis, void* CAtlasButton, UINT ButtonState) {
    g_inButtonDraw = true;
    long long ret = CButton_DrawStateW_Original ? CButton_DrawStateW_Original(pThis, CAtlasButton, ButtonState) : 0;
...
long (*CTopLevelWindow_UpdateMarginsDependentOnStyle_Original)(void* pThis) = nullptr;

long CTopLevelWindow_UpdateMarginsDependentOnStyle_Hook(void* pThis) {
    long retvalue = CTopLevelWindow_UpdateMarginsDependentOnStyle_Original ? CTopLevelWindow_UpdateMarginsDependentOnStyle_Original(pThis) : 0;
    if (pThis) {
        long long CVisualInstance = *((long long *)pThis + 91);
        if (CVisualInstance && DesktopManagerInstance && *DesktopManagerInstance) {
...
long (*CText_ValidateResources_Original)(void* pThis) = nullptr;
long CText_ValidateResources_Hook(void* pThis) {
    return CText_ValidateResources_Original ? CText_ValidateResources_Original(pThis) : 0;
}

long (*CTopLevelWindow_UpdateColorizationColor_Original)(void* pThis) = nullptr;
long CTopLevelWindow_UpdateColorizationColor_Hook(void* pThis) {
    return CTopLevelWindow_UpdateColorizationColor_Original ? CTopLevelWindow_UpdateColorizationColor_Original(pThis) : 0;
}

void (*CText_SetBackgroundColor_Original)(void* pThis, unsigned long color) = nullptr;
void CText_SetBackgroundColor_Hook(void* pThis, unsigned long color) {
    if (CText_SetBackgroundColor_Original) {
        CText_SetBackgroundColor_Original(pThis, color);
    }
}

long (*CTopLevelWindow_UpdateNCAreaBackground_Original)(void* pThis) = nullptr;
long CTopLevelWindow_UpdateNCAreaBackground_Hook(void* pThis) {
    long retvalue = CTopLevelWindow_UpdateNCAreaBackground_Original ? CTopLevelWindow_UpdateNCAreaBackground_Original(pThis) : 0;
    if (pThis) {
        ...
        EnsureButtonsOnTop(pThis);
    }
    return retvalue;
}

long (*CVisual_SetSize_Original)(PVOID pThis, struct tagSIZE const *) = nullptr;
long CVisual_SetSize_Hook(PVOID pThis, struct tagSIZE* size) {
    return CVisual_SetSize_Original ? CVisual_SetSize_Original(pThis, size) : 0;
}

long (*CTopLevelWindow_UpdateNCAreaButton_Original)(__int64, int, int, int, int*) = nullptr;
long CTopLevelWindow_UpdateNCAreaButton_Hook(__int64 pThis, int buttonId, int height, int offsetTop, int* offsetRight) {
    ...
    if (!pThis || !offsetRight || buttonId < 0 || buttonId > 3) {
        return 0;
    }

    CVisual *buttonData = *(CVisual**)(pThis + 8 * buttonId + 488);
    if (!buttonData) return 0;

    if (!CTopLevelWindow_UpdateNCAreaButton_Original) return 0;

    int origOffsetRight = *offsetRight;
    long retnvalue = CTopLevelWindow_UpdateNCAreaButton_Original(pThis, buttonId, height, offsetTop, offsetRight);

    if (*offsetRight == origOffsetRight) {
        return retnvalue;
    }

    *offsetRight = origOffsetRight;

    __int64 pDpiStruct = *(__int64 *)(pThis + 728);
    __int64 DPIValue = pDpiStruct ? *(unsigned int *)(pDpiStruct + 324) : 96;
    if (DPIValue == 0) DPIValue = 96;
    ...
    if ((*(BYTE *)(pThis + 592) & TYPE_TOOL) != 0) {
        isTool = TRUE;
        TBHeight = GetSystemMetricsForDpiHook(53, (UINT)DPIValue);
    } else {
        isTool = FALSE;
        TBHeight = GetSystemMetricsForDpiHook(31, (UINT)DPIValue);
    }
    ...
    if (CVisual_SetInsetFromParentTop_Original) {
        CVisual_SetInsetFromParentTop_Original(buttonData, offsetTop);
    }
    ...
    if (CVisual_SetSize_Original) {
        retnvalue = CVisual_SetSize_Original(buttonData, &size);
    }

    if (retnvalue < 0) {
        if (MilInstrumentationCheckHR_MaybeFailFast_Original) {
...
long (*CTopLevelWindow_UpdateNCAreaPositionsAndSizes_Original)(void* pThis) = nullptr;
long CTopLevelWindow_UpdateNCAreaPositionsAndSizes_Hook(void *pThis) {
    ...
    const int ICON_OFFSET_LEFT_NORMAL  = 2;
    const int ICON_OFFSET_TOP_NORMAL   = 4;
    const int TITLE_OFFSET_LEFT_NORMAL = 1;
    const int TITLE_OFFSET_TOP_NORMAL  = 4;

    const int ICON_OFFSET_LEFT_MAX     = 2;
    const int ICON_OFFSET_TOP_MAX      = 4;
    const int TITLE_OFFSET_LEFT_MAX    = 5;
    const int TITLE_OFFSET_TOP_MAX     = 8;

    long long visualInst = *((long long *)pThis + 91);
    ...
    if (*((long long *)pThis + 60)) {
        ...
        if (clientData) {
            ...
            if (clientData[32] != insetLeft) {
                ...
                if (CVisual_SetDirtyFlags_Original) CVisual_SetDirtyFlags_Original(clientData, 2);
            }
        ...
        if (unknown && CVisual_SetInsetFromParent_Original && *((long long *)pThis + 68)) {
    ...
    if (insetRightNormal <= 0 && visualInst) {
        insetRightNormal = *(long long *)(visualInst + 96);
    }
    if (insetRightNormal < 0) insetRightNormal = 0;

    long *borderSizeArray = isMaximized ? (long *)((char *)pThis + 644) : (long *)((char *)pThis + 628);

    int rightBorder = borderSizeArray ? borderSizeArray[1] : 0;
    int topBorder   = borderSizeArray ? borderSizeArray[2] : 0;

    int insetRight;
    if (insetRightNormal - 2 <= rightBorder + 2) {
        insetRight = rightBorder + 2;
    } else {
    ...
    __int64 DPIValue = visualInst ? *(unsigned int *)(visualInst + 324) : 96;
    if (DPIValue == 0) DPIValue = 96;

    int height;
    if (isTool) {
        height = GetSystemMetricsForDpiHook(53, (UINT)DPIValue);
    } else {
        height = floor((float)(GetSystemMetricsForDpiHook(31, (UINT)DPIValue)) * 0.95238096f) + 0.5f;
    }

    int insetTop = topBorder;
    if (isTool) {
    ...
    int iconOffsetLeft  = isMaximized ? ICON_OFFSET_LEFT_MAX  : ICON_OFFSET_LEFT_NORMAL;
    int iconOffsetTop   = isMaximized ? ICON_OFFSET_TOP_MAX   : ICON_OFFSET_TOP_NORMAL;
    int titleOffsetLeft = isMaximized ? TITLE_OFFSET_LEFT_MAX : TITLE_OFFSET_LEFT_NORMAL;
    int titleOffsetTop  = isMaximized ? TITLE_OFFSET_TOP_MAX  : TITLE_OFFSET_TOP_NORMAL;

    bool hasIcon = (__int64 *)*((long long *)pThis + 66);
    if (hasIcon) {
        int width = 0;
        int heightIcon = 0;
        if (visualInst && (*(long long *)(visualInst + 136) || (*((long *)pThis + 148) & 0x10000) == 0)) {
            width = 16;
            heightIcon = 16;
        }
        
        long long *iconData = (__int64 *)*((long long *)pThis + 66);
        ...
        long retnvalue = 0;
        if (CVisual_SetSize_Original) {
            retnvalue = CVisual_SetSize_Original(iconData, &size);
        }
        if (retnvalue < 0) {
            if (MilInstrumentationCheckHR_MaybeFailFast_Original) {
        ...
        if (CVisual_SetBitmapScalingMode_Original) CVisual_SetBitmapScalingMode_Original(iconData, 2);
        if (CVisual_SetEdgeMode_Original) CVisual_SetEdgeMode_Original(iconData, 1);

        int insetTopIcon;
        DWORD iconHeightVal = *(DWORD *)(*((long long *)pThis + 66) + 124);
        if (isMaximized) {
            insetTopIcon = (*((DWORD *)pThis + 151) - iconHeightVal) / 2;
        } else {
            insetTopIcon = topBorder + (*((DWORD *)pThis + 151) - iconHeightVal - topBorder) / 2;
        }

        if (CVisual_SetInsetFromParentTop_Original) CVisual_SetInsetFromParentTop_Original(iconData, insetTopIcon + iconOffsetTop);
        if (CVisual_SetInsetFromParentLeft_Original) CVisual_SetInsetFromParentLeft_Original(iconData, insetLeft + iconOffsetLeft);
    ...
    if (hasText) {
        DWORD *textData = (DWORD *)*((long long *)pThis + 65);

        int textTopInset = isMaximized ? titleOffsetTop : (topBorder + titleOffsetTop);

        if (CVisual_SetInsetFromParentTop_Original) CVisual_SetInsetFromParentTop_Original(textData, textTopInset);
        if (CVisual_SetInsetFromParentLeft_Original) CVisual_SetInsetFromParentLeft_Original(textData, insetLeft + titleOffsetLeft);

        if (textData[33] != insetRight) {
            textData[33] = insetRight;
            if (CVisual_SetDirtyFlags_Original) CVisual_SetDirtyFlags_Original(textData, 2);
        }
        struct tagSIZE size;
        size.cx = textData[30];
        size.cy = *((DWORD *)pThis + 151) - topBorder;
        if (CText_SetSize_Original) CText_SetSize_Original(textData, &size);
    }

    if (CTopLevelWindow_UpdatePinnedParts_Original) {
...
BOOL Wh_ModInit(void) {
    ...
    do {
        if (_wcsicmp(findSymbol.symbol, L"private: void __cdecl CButton::UpdateCurrentGlyphOpacity(bool)") == 0) {
            ...
            CTopLevelWindow_UpdateNCAreaButton_Original = (long(*)(__int64, int, int, int, int*))findSymbol.address;
        }
        ...
        else if (_wcsicmp(findSymbol.symbol, L"public: void __cdecl CVisual::SetBitmapScalingMode(enum MilBitmapScalingMode::Enum)") == 0 ||
                 wcsstr(findSymbol.symbol, L"CVisual::SetBitmapScalingMode") != nullptr) {
            CVisual_SetBitmapScalingMode_Original = (CVisual_SetBitmapScalingMode_t)findSymbol.address;
        }
        else if (_wcsicmp(findSymbol.symbol, L"public: void __cdecl CVisual::SetEdgeMode(enum MilEdgeMode::Enum)") == 0 ||
                 wcsstr(findSymbol.symbol, L"CVisual::SetEdgeMode") != nullptr) {
            CVisual_SetEdgeMode_Original = (CVisual_SetEdgeMode_t)findSymbol.address;
        }
        ...
        else if (_wcsicmp(findSymbol.symbol, L"protected: long __cdecl CMatrixTransformProxy::CreateProxy(class CMatrixTransformProxy * *)") == 0 ||
                 _wcsicmp(findSymbol.symbol, L"protected: long __cdecl CCompositor::CreateProxy<class CMatrixTransformProxy>(class CMatrixTransformProxy * *)") == 0) {
        ...
        else if (_wcsicmp(findSymbol.symbol, L"public: long __cdecl CVisual::RemoveChild(class CVisual *)") == 0) {
            CVisual_RemoveChild_Original = (CVisual_RemoveChild_t)findSymbol.address;
        }
        else if (_wcsicmp(findSymbol.symbol, L"public: long __cdecl CVisual::AddChild(class CVisual *)") == 0 ||
                 wcsstr(findSymbol.symbol, L"CVisual::AddChild") != nullptr) {
            if (!CVisual_AddChild_Original) {
                CVisual_AddChild_Original = (CVisual_AddChild_t)findSymbol.address;
            }
        }
    ...
    if (UpdateCurrentGlyphOpacityAddr) {
    ...
    if (UpdateColorizationColorAddr) {
    ...
    Wh_Log(L"Windows Basic Caption Buttons initialized successfully.");
    return TRUE;
}
