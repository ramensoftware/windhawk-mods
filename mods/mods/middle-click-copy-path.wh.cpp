...
// @description     Copy the file path of the program with a middle click on the taskbar instead of closing it
// @version         1.0.0
// @author          webber3242
    ...
    if (hProcess) {
        ...
        if (GetModuleFileNameEx(hProcess, NULL, filePath, MAX_PATH)) {
            ...
            if (OpenClipboard(NULL)) {
                EmptyClipboard();
                HGLOBAL hGlobal = GlobalAlloc(GMEM_MOVEABLE, (wcslen(filePath) + 1) * sizeof(WCHAR));
                if (hGlobal) {
                    void* pGlobal = GlobalLock(hGlobal);
                    if (pGlobal) {
                        wcscpy_s((WCHAR*)pGlobal, wcslen(filePath) + 1, filePath);
