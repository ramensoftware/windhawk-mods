// ==WindhawkMod==
// @id                taskbar-dock-animation-fork
// @name              Taskbar Dock Animation - Fork
// @name:zh-CN        任务栏 Dock 动画 - Fork
// @name:ja-JP        タスクバー Dock アニメーション - Fork
// @name:ko-KR        작업 표시줄 Dock 애니메이션 - Fork
// @name:pt-BR        Animação Dock da Barra de Tarefas - Fork
// @name:it-IT        Animazione Dock della Barra delle applicazioni - Fork
...
// ==WindhawkModReadme==
/*
# Taskbar Dock Animation - Fork

This mod adds a macOS-like taskbar animation.
It is an updated fork of the original "Taskbar Dock Animation" mod, extending compatibility to all taskbar positions.
...
void ApplyAnimation(double mouseX, DockAnimationContext& ctx, double intensity, double dtSec) {
    ...
    if (g_isBouncing && closestIconIndex != (size_t)-1 &&
        ...
        bounceTranslateOffset = normalizedWave * BOUNCE_TRANSLATE_Y;       
    ...
    auto SetupAndAddElement = [&](FrameworkElement const& element) {
        try {
            ...
            if (g_settings.taskbarLabelsMode) {
                ...
                if (ctx.isVertical) {
                    element.RenderTransformOrigin({ xOrigin, iconCenterProportion });
                } else {
                ...
                if (ctx.isVertical) {
                    element.RenderTransformOrigin({ xOrigin, yOrigin });
                } else {
                    element.RenderTransformOrigin({ 0.5f, yOrigin });
                }
                info.originalCenterX = ctx.isVertical ? (point.Y + (info.elementWidth / 2.0)) : (point.X + (info.elementWidth / 2.0));
            }
