...
// @description     Adds "Copy and Open" to File Explorer folder context menu and opens a duplicate Explorer window.
// @version         1.0
// @author          Neko_RTHsama
...
- enableContextMenuItem: true
  ...
  $description: Add "Copy and Open" to Explorer folder context menus.
*/
// ==/WindhawkModSettings==
...
std::wstring g_menuText = L"Copy and Open";
...
void InitializeLocalization() {
    ...
    switch (primary) {
        ...
        case LANG_JAPANESE:
            ...
            break;
        case LANG_KOREAN:
            g_menuText = L"복사하여 열기";
            break;
        case LANG_RUSSIAN:
            g_menuText = L"Копировать и открыть";
            break;
        case LANG_FRENCH:
            g_menuText = L"Copier et ouvrir";
            break;
        case LANG_GERMAN:
            g_menuText = L"Kopieren und oeffnen";
            break;
        case LANG_SPANISH:
            g_menuText = L"Copiar y abrir";
            break;
        case LANG_PORTUGUESE:
            g_menuText = L"Copiar e abrir";
            break;
        default:
            g_menuText = L"Copy and Open";
            break;
    }
}
...
void InsertMenuItemIfNeeded(HMENU menu) {
    ...
    if (propPos < 0) {
        propPos = findMenuPosByText(L"內容");
    }
    if (propPos < 0) {
        propPos = findMenuPosByText(L"プロパティ");
    }
    if (propPos < 0) {
        propPos = findMenuPosByText(L"속성");
    }
    if (propPos < 0) {
        propPos = findMenuPosByText(L"Свойства");
    }
    ...
    InsertMenuW(menu, pos, MF_BYPOSITION | MF_STRING | MF_ENABLED,
                kCopyAndOpenCommandId, g_menuText.c_str());
}
