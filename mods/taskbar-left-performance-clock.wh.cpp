// ==WindhawkMod==
// @id              taskbar-left-performance-clock
// @name            Taskbar Left Performance Clock
// @description     Move the customizable Windows 11 clock and performance block to the left side of the taskbar with real layout relocation
// @version         1.0
// @author          pleromyst
...
//
// Based on Taskbar Clock Customization by m417z:
// https://windhawk.net/mods/taskbar-clock-customization
//
// This fork adds real Windows 11 taskbar layout relocation for the native
// clock/performance block. Maintained by pleromyst, with development assistance
// from ChatGPT.

// ==WindhawkModReadme==
/*
# Taskbar Left Performance Clock

Moves the native Windows 11 clock/performance block to the left side of the
taskbar using real layout relocation, while keeping its customizable date/time
format, performance metrics, weather, media information, fonts, colors, and
other features.

The original Taskbar Clock Customization implementation, including formatting,
performance metrics, data collection, media support, weather/web content, and
most clock customization logic, is by **m417z**. This fork adds the Windows 11
left-side real-layout relocation and related startup/restoration handling.

Based on: https://windhawk.net/mods/taskbar-clock-customization

Maintained by **pleromyst**, with development assistance from **ChatGPT**.
Licensed under GPL-3.0, consistent with the original mod.

Only Windows 10 64-bit and Windows 11 are supported. The left-side relocation
feature is intended for the modern Windows 11 taskbar.
...
- MoveClockToLeft: false
  $name: Move clock/performance block to taskbar left
  $description: >-
    Moves the entire Windows 11 notification-center clock button, including the
    customized clock/performance content and its click area, from the system
    tray to the left side of the taskbar using real layout relocation.
...
// Real-layout relocation of the complete notification-center
// clock button. Unlike RenderTransform, this reparents the actual XAML element,
// so its layout slot and hit-test area move with it.
struct MovedClockElementData {
    winrt::weak_ref<FrameworkElement> clockButtonElement;
    winrt::weak_ref<Controls::Grid> originalParent;
    winrt::weak_ref<Controls::Grid> newParent;
...
std::atomic<bool> g_moveClockToLeftEnabled;
std::vector<MovedClockElementData> g_movedClockElementData;
...
FrameworkElement FindAncestorByClassAndName(FrameworkElement element,
                                                PCWSTR className,
                                                PCWSTR name) {
    auto current = element;
    while (current) {
        if ((!*name || wcscmp(current.Name().c_str(), name) == 0) &&
            wcscmp(winrt::get_class_name(current).c_str(), className) == 0) {
            return current;
        }
...
void CleanupMovedClockElementData() {
    for (auto it = g_movedClockElementData.begin();
         it != g_movedClockElementData.end();) {
        if (!it->clockButtonElement.get()) {
            it = g_movedClockElementData.erase(it);
        } else {
            ++it;
        }
    }
...
bool RestoreMovedClockElement(FrameworkElement clockButtonElement) {
    CleanupMovedClockElementData();

    for (auto it = g_movedClockElementData.begin();
         it != g_movedClockElementData.end(); ++it) {
        auto movedElement = it->clockButtonElement.get();
        if (!movedElement || movedElement != clockButtonElement) {
            continue;
        }

        auto originalParent = it->originalParent.get();
        if (!originalParent) {
            g_movedClockElementData.erase(it);
            return false;
        }

        auto element = movedElement.as<UIElement>();
        auto originalParentPanel = originalParent.as<Controls::Panel>();
        auto currentParent = Media::VisualTreeHelper::GetParent(movedElement)
                                 .try_as<Controls::Panel>();

        if (currentParent && currentParent != originalParentPanel) {
            uint32_t currentIndex = 0;
            if (currentParent.Children().IndexOf(element, currentIndex)) {
                currentParent.Children().RemoveAt(currentIndex);
            }
        }

        Controls::Grid::SetColumn(movedElement, it->originalColumn);
        Controls::Grid::SetColumnSpan(movedElement, it->originalColumnSpan);
        Controls::Grid::SetRow(movedElement, it->originalRow);
        Controls::Grid::SetRowSpan(movedElement, it->originalRowSpan);
        movedElement.HorizontalAlignment(it->originalHorizontalAlignment);
        movedElement.VerticalAlignment(it->originalVerticalAlignment);
        movedElement.Margin(it->originalMargin);

        currentParent = Media::VisualTreeHelper::GetParent(movedElement)
                            .try_as<Controls::Panel>();
        if (!currentParent) {
            ...
            if (insertIndex < children.Size()) {
                children.InsertAt(insertIndex, element);
            } else {
                children.Append(element);
            }
        }

        Wh_Log(L"Restored NotificationCenterButton to system tray");
        g_movedClockElementData.erase(it);
        return true;
    }
...
bool MoveClockElementToTaskbarLeft(FrameworkElement dateTimeIconContentElement) {
    CleanupMovedClockElementData();

    auto clockButtonElement = FindAncestorByClassAndName(
        dateTimeIconContentElement, L"SystemTray.OmniButton",
        L"NotificationCenterButton");
    if (!clockButtonElement) {
        Wh_Log(L"NotificationCenterButton ancestor not found");
        return false;
    }

    for (const auto& data : g_movedClockElementData) {
        auto movedElement = data.clockButtonElement.get();
        if (movedElement && movedElement == clockButtonElement) {
            return true;
        }
    }
    ...
    auto originalParent =
        Media::VisualTreeHelper::GetParent(clockButtonElement)
            .try_as<Controls::Grid>();
    if (!originalParent ||
        wcscmp(originalParent.Name().c_str(), L"SystemTrayFrameGrid") != 0) {
        Wh_Log(L"Unexpected NotificationCenterButton parent");
        return false;
    }
    ...
    auto systemTrayFrame = Media::VisualTreeHelper::GetParent(originalParent)
                               .try_as<FrameworkElement>();
    if (!systemTrayFrame ||
        wcscmp(winrt::get_class_name(systemTrayFrame).c_str(),
               L"SystemTray.SystemTrayFrame") != 0) {
        Wh_Log(L"SystemTrayFrame ancestor not found");
        return false;
    }

    // UWPSpy shows the Windows 11 taskbar hierarchy as:
    // Border > Grid > SystemTray.SystemTrayFrame > Grid#SystemTrayFrameGrid.
    // Reparent the complete NotificationCenterButton into that outer Grid.
    auto taskbarRootGrid = Media::VisualTreeHelper::GetParent(systemTrayFrame)
                               .try_as<Controls::Grid>();
    if (!taskbarRootGrid || taskbarRootGrid == originalParent) {
        Wh_Log(L"Taskbar root Grid not found");
        return false;
    }

    auto element = clockButtonElement.as<UIElement>();
    uint32_t originalIndex = 0;
    if (!originalParent.Children().IndexOf(element, originalIndex)) {
        Wh_Log(L"NotificationCenterButton isn't a child of SystemTrayFrameGrid");
        return false;
    }

    MovedClockElementData data{
        .clockButtonElement = clockButtonElement,
        .originalParent = originalParent,
        .newParent = taskbarRootGrid,
        .originalIndex = originalIndex,
        .originalColumn = Controls::Grid::GetColumn(clockButtonElement),
        .originalColumnSpan = Controls::Grid::GetColumnSpan(clockButtonElement),
        .originalRow = Controls::Grid::GetRow(clockButtonElement),
        .originalRowSpan = Controls::Grid::GetRowSpan(clockButtonElement),
        .originalHorizontalAlignment = clockButtonElement.HorizontalAlignment(),
        .originalVerticalAlignment = clockButtonElement.VerticalAlignment(),
        .originalMargin = clockButtonElement.Margin(),
    };

    originalParent.Children().RemoveAt(originalIndex);

    // Span the taskbar root's columns, then align the real control to the left.
    // This keeps the centered app area and the right-side system tray in their
    // existing layout while moving the clock's actual hit-test area with it.
    Controls::Grid::SetColumn(clockButtonElement, 0);
    uint32_t columnCount = taskbarRootGrid.ColumnDefinitions().Size();
    Controls::Grid::SetColumnSpan(clockButtonElement,
                                  std::max<int>(1, columnCount));
    Controls::Grid::SetRow(clockButtonElement, 0);
    uint32_t rowCount = taskbarRootGrid.RowDefinitions().Size();
    Controls::Grid::SetRowSpan(clockButtonElement, std::max<int>(1, rowCount));
    clockButtonElement.HorizontalAlignment(HorizontalAlignment::Left);
    clockButtonElement.VerticalAlignment(VerticalAlignment::Stretch);
    clockButtonElement.Margin(Thickness{0, 0, 0, 0});

    taskbarRootGrid.Children().Append(element);
    g_movedClockElementData.push_back(std::move(data));

    Wh_Log(L"Moved NotificationCenterButton to taskbar left");
    return true;
}

void ScheduleMoveClockElementToTaskbarLeft(
    FrameworkElement dateTimeIconContentElement) {
    auto dispatcher = dateTimeIconContentElement.Dispatcher();
    if (!dispatcher) {
        Wh_Log(L"DateTimeIconContent has no dispatcher");
        return;
    }

    auto weakElement = winrt::make_weak(dateTimeIconContentElement);
    dispatcher.TryRunAsync(
        winrt::Windows::UI::Core::CoreDispatcherPriority::Low,
        [weakElement]() {
            auto element = weakElement.get();
            if (!element || !g_moveClockToLeftEnabled) {
                return;
            }

            try {
                MoveClockElementToTaskbarLeft(element);
            } catch (...) {
                HRESULT hr = winrt::to_hresult();
                Wh_Log(L"Deferred clock move failed: %08X", hr);
            }
        });
}

void UpdateClockElementPlacement(FrameworkElement dateTimeIconContentElement) {
    auto clockButtonElement = FindAncestorByClassAndName(
        dateTimeIconContentElement, L"SystemTray.OmniButton",
        L"NotificationCenterButton");
    if (!clockButtonElement) {
    ...
    if (!g_moveClockToLeftEnabled) {
        RestoreMovedClockElement(clockButtonElement);
        return;
    }

    // Moving the button directly from OnApplyTemplate is too early during an
    // Explorer/taskbar cold start. Windows is still constructing and arranging
    // SystemTrayFrameGrid at that point. It can work when the setting is toggled
    // later, but on restart it may leave the tray partially arranged or cause
    // the moved clock element to be recreated/discarded.
    //
    // Wait until the DateTimeIconContent is Loaded, then post the actual move
    // back to the XAML dispatcher at low priority. This lets the current load /
    // template pass finish before we change the visual-tree parent.
    if (dateTimeIconContentElement.IsLoaded()) {
        ScheduleMoveClockElementToTaskbarLeft(dateTimeIconContentElement);
        return;
    }

    auto weakElement = winrt::make_weak(dateTimeIconContentElement);
    dateTimeIconContentElement.Loaded(
        [weakElement](winrt::Windows::Foundation::IInspectable const&,
            ...
            ScheduleMoveClockElementToTaskbarLeft(element);
        });
}
...
void ApplyDateTimeIconContentStyles(
    ...
    UpdateClockElementPlacement(dateTimeIconContentElement);
...
void LoadSettings() {
    ...
    g_settings.moveClockToLeft = Wh_GetIntSetting(L"MoveClockToLeft");
    g_moveClockToLeftEnabled = g_settings.moveClockToLeft;
...
void Wh_ModBeforeUninit() {
    ...
    bool hadClockElementStyles = g_clockElementStyleEnabled.exchange(false);
    bool hadMovedClock = g_moveClockToLeftEnabled.exchange(false);

    if (g_winVersion >= WinVersion::Win11 &&
        (hadClockElementStyles || hadMovedClock)) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (!g_movedClockElementData.empty()) {
                allRestored = false;
            }
