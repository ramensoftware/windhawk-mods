...
void Wh_ModAfterInit() {
    ...
    WCHAR cmd[MAX_PATH * 2]; swprintf_s(cmd, L"\"%s\" -tool-mod \"%s\"", path, WH_MOD_ID);
    
    HMODULE hKernel = GetModuleHandle(L"kernelbase.dll");
    if (!hKernel) hKernel = GetModuleHandle(L"kernel32.dll");
    ...
    CreateProcessInternalW_t pCP = (CreateProcessInternalW_t)GetProcAddress(hKernel, "CreateProcessInternalW");
    
    STARTUPINFO si = { sizeof(si) }; 
    si.dwFlags = STARTF_FORCEOFFFEEDBACK;
    PROCESS_INFORMATION pi;
    if (pCP && pCP(nullptr, path, cmd, nullptr, nullptr, FALSE, NORMAL_PRIORITY_CLASS, nullptr, nullptr, &si, &pi, nullptr)) {
