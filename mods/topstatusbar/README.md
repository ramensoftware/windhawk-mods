...
*   **Live Resource Monitor (Right):** Real-time tracking of CPU load, RAM usage, and SSD (C:) storage.
*   **Quick Actions (Right):** Direct toggles for Volume, Brightness, Bluetooth, Network, and a dedicated Shutdown dialog.
*   **Active Window Info:** Displays the icon and title of the currently focused application.

#### **⚠️ Important Usage Note:**
Due to how Windows handles focus and security layers, the **Task View** and **Alt-Tab** buttons may not trigger if the Windhawk settings window is the active foreground application. This is normal behavior due to Windhawk's UI elevation; the buttons work perfectly with all other apps and the desktop.
...
*   **Monitor de Recursos (Derecha):** Seguimiento en tiempo real de la carga de CPU, uso de RAM y almacenamiento en disco (SSD).
*   **Acciones Rápidas (Derecha):** Controles directos para Volumen, Brillo, Bluetooth, Red y un diálogo de Apagado.
*   **Información de Ventana Activa:** Muestra el icono y el título de la aplicación que estás usando en ese momento.

#### **⚠️ Nota importante de uso:**
Debido a la gestión de foco y seguridad de Windows, los botones de **Vista de Tareas** y **Alt-Tab** podrían no activarse si la ventana de ajustes de Windhawk es la aplicación activa. Este es un comportamiento normal por la elevación de la interfaz de Windhawk; los botones funcionan perfectamente con cualquier otra aplicación y con el escritorio.
