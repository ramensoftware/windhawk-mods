...
bool IsPathExcluded(PCWSTR path) {
    ...
    for (const auto& excludePath : g_excludePaths) {
        ...
        if (fullPath.rfind(excl, 0) == 0) {
            if (fullPath.length() == excl.length() || (fullPath.length() > excl.length() && fullPath[excl.length()] == L'\\')) {
                return true;
            }
        }
    }
...
public:
    ...
    STDMETHODIMP Next(ULONG celt, LPITEMIDLIST* rgelt, ULONG* pceltFetched) {
        ...
        while (fetched < celt) {
            ...
            if (SUCCEEDED(SHGetNameFromIDList(pidl, SIGDN_DESKTOPABSOLUTEPARSING, &pszFullPath))) {
...
void LoadSettings() {
    ...
    Wh_FreeStringSetting(const_cast<PWSTR>(excludePaths));
}
...
void Wh_ModUninit() {
    Wh_UnhookFunction((void*)IShellFolder_EnumObjects_Original);
}
