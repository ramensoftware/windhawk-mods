...
static BOOL CALLBACK CP_EnumStrings(HWND hChild, LPARAM) {
    ...
    if (_wcsicmp(cls, L"Static") == 0) {
        ...
        if (text[0] && wcslen(text) > 3) {
            // Strip & accelerator
            WCHAR clean[64]; WCHAR* s = text; WCHAR* d = clean;
            while (*s) { if (*s != L'&') *d++ = *s; s++; } *d = 0;
            // Remove trailing ':'
            size_t len = wcslen(clean);
            if (len > 0 && clean[len-1] == L':') clean[len-1] = 0;
...
static void CP_UploadBitmaps(ColorPickerData* d) {
    if (d->pSpecBmp) { D2D1_RECT_U r={0,0,(UINT32)CP_SPEC_W,(UINT32)CP_SPEC_H}; d->pSpecBmp->CopyFromMemory(&r, d->specPixels, CP_SPEC_W*4); }
    if (d->pVBarBmp) { D2D1_RECT_U r={0,0,(UINT32)CP_VBAR_W,(UINT32)CP_SPEC_H}; d->pVBarBmp->CopyFromMemory(&r, d->vbarPixels, CP_VBAR_W*4); }
}

static D2D1_COLOR_F CP_D2D(COLORREF c, float a=1.f) { return D2D1::ColorF(GetRValue(c)/255.f, GetGValue(c)/255.f, GetBValue(c)/255.f, a); }
...
static void CP_DestroyD2D(ColorPickerData* d) {
    if (d->pTextFmt) { d->pTextFmt->Release(); d->pTextFmt=nullptr; }
    if (d->pDWFactory) { d->pDWFactory->Release(); d->pDWFactory=nullptr; }
    if (d->pDashed) { d->pDashed->Release(); d->pDashed=nullptr; }
    if (d->pSpecBmp) { d->pSpecBmp->Release(); d->pSpecBmp=nullptr; }
    if (d->pVBarBmp) { d->pVBarBmp->Release(); d->pVBarBmp=nullptr; }
    if (d->pRT) { d->pRT->Release(); d->pRT=nullptr; }
    if (d->pFactory) { d->pFactory->Release(); d->pFactory=nullptr; }
}
...
static void CP_Paint(HWND hwnd, HDC hdc, ColorPickerData* d) {
    ...
    if (d->pSpecBmp) {
        ...
        if (clip) { d->pRT->PushLayer(D2D1::LayerParameters(D2D1::InfiniteRect(), clip), nullptr);
...
static LRESULT CALLBACK CP_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (mx>=CP_SPEC_X&&mx<CP_SPEC_X+CP_SPEC_W&&my>=CP_SPEC_Y&&my<CP_SPEC_Y+CP_SPEC_H) {
            ...
            d->hue=(float)(mx-CP_SPEC_X)*360.f/(CP_SPEC_W-1); d->sat=1.f-(float)(my-CP_SPEC_Y)/(CP_SPEC_H-1);
            d->hue=d->hue<0?0:d->hue>360?360:d->hue; d->sat=d->sat<0?0:d->sat>1?1:d->sat;
        ...
        if (mx>=CP_VBAR_X-4&&mx<CP_VBAR_X+CP_VBAR_W+12&&my>=CP_VBAR_Y&&my<CP_VBAR_Y+CP_SPEC_H) {
            ...
            d->val=1.f-(float)(my-CP_VBAR_Y)/(CP_SPEC_H-1); d->val=d->val<0?0:d->val>1?1:d->val;
            CP_UpdateControls(hwnd,d); return 0; }
        for (int i=0;i<48;i++) { int col=i%CP_BCOLS,row=i/CP_BCOLS;
            float cx=CP_SPEC_X+col*CP_CIRC_STEP+CP_CIRC_D/2.f, cy=(float)CP_GridY()+row*CP_CIRC_STEP+CP_CIRC_D/2.f;
            float dx=mx-cx,dy=my-cy;
            if(dx*dx+dy*dy<=(CP_CIRC_D/2.f)*(CP_CIRC_D/2.f)){CP_RGBtoHSV(g_basicColors[i],d->hue,d->sat,d->val);CP_UpdateControls(hwnd,d);return 0;}}
        for (int i=0;i<16;i++) { int col=i%8,row=i/8;
            float cx=CP_SPEC_X+col*CP_CIRC_STEP+CP_CIRC_D/2.f, cy=(float)CP_CustGridY()+row*CP_CIRC_STEP+CP_CIRC_D/2.f;
            float dx=mx-cx,dy=my-cy;
            if(dx*dx+dy*dy<=(CP_CIRC_D/2.f)*(CP_CIRC_D/2.f)){d->selectedCustom=i;
                if(d->lpCustColors){CP_RGBtoHSV(d->lpCustColors[i],d->hue,d->sat,d->val);CP_UpdateControls(hwnd,d);}return 0;}}
        return 0; }
    case WM_MOUSEMOVE: {
        if(!(wParam&MK_LBUTTON)) return 0;
        int mx=(short)LOWORD(lParam), my=(short)HIWORD(lParam);
        if(d->trackingSpec){d->hue=(float)(mx-CP_SPEC_X)*360.f/(CP_SPEC_W-1);d->sat=1.f-(float)(my-CP_SPEC_Y)/(CP_SPEC_H-1);
            d->hue=d->hue<0?0:d->hue>360?360:d->hue;d->sat=d->sat<0?0:d->sat>1?1:d->sat;CP_UpdateControls(hwnd,d);}
        else if(d->trackingVBar){d->val=1.f-(float)(my-CP_VBAR_Y)/(CP_SPEC_H-1);d->val=d->val<0?0:d->val>1?1:d->val;CP_UpdateControls(hwnd,d);}
        return 0; }
    case WM_LBUTTONUP: d->trackingSpec=d->trackingVBar=FALSE; ReleaseCapture(); return 0;
    case WM_COMMAND: {
        WORD id=LOWORD(wParam),notif=HIWORD(wParam);
        if(id==IDOK){d->accepted=TRUE;{ HWND o=GetWindow(hwnd,GW_OWNER); if(o) EnableWindow(o,TRUE); DestroyWindow(hwnd); return 0; }}
        if(id==IDCANCEL){d->accepted=FALSE;{ HWND o=GetWindow(hwnd,GW_OWNER); if(o) EnableWindow(o,TRUE); DestroyWindow(hwnd); return 0; }}
        if(id==4010&&d->lpCustColors){int idx=d->selectedCustom>=0?d->selectedCustom:0;d->lpCustColors[idx]=d->result;d->selectedCustom=idx;InvalidateRect(hwnd,NULL,FALSE);return 0;}
        if(notif==EN_CHANGE&&!d->updating){
            if(id==IDC_CP_HEX){WCHAR buf[16];GetDlgItemTextW(hwnd,IDC_CP_HEX,buf,16);LPCWSTR p=buf;if(*p==L'#')p++;unsigned int hex=0;
                if(swscanf(p,L"%x",&hex)==1){CP_RGBtoHSV(RGB((hex>>16)&0xFF,(hex>>8)&0xFF,hex&0xFF),d->hue,d->sat,d->val);
                    d->result=CP_HSVtoRGB(d->hue,d->sat,d->val);d->updating=TRUE;WCHAR b[8];
                    wsprintfW(b,L"%d",GetRValue(d->result));SetDlgItemTextW(hwnd,IDC_CP_R,b);
                    wsprintfW(b,L"%d",GetGValue(d->result));SetDlgItemTextW(hwnd,IDC_CP_G,b);
                    wsprintfW(b,L"%d",GetBValue(d->result));SetDlgItemTextW(hwnd,IDC_CP_B,b);
...
static BOOL CP_ShowDialog(HWND hwndOwner, ColorPickerData* data) {
    if (!g_cpClassAtom) { WNDCLASSEXW wc={}; wc.cbSize=sizeof(wc); wc.lpfnWndProc=CP_WndProc; wc.hInstance=GetModuleHandleW(NULL);
        wc.hCursor=LoadCursorW(NULL,IDC_ARROW); wc.hbrBackground=(HBRUSH)(COLOR_3DFACE+1); wc.lpszClassName=L"BetterDialogs_ColorPicker";
    ...
    if(hwndOwner){RECT ro;GetWindowRect(hwndOwner,&ro);x=ro.left+(ro.right-ro.left-ww)/2;y=ro.top+(ro.bottom-ro.top-wh)/2;}
    ...
    MSG msg; while(GetMessageW(&msg,NULL,0,0)){if(IsDialogMessageW(hwnd,&msg))continue;TranslateMessage(&msg);DispatchMessageW(&msg);}
    return data->accepted;
}

BOOL WINAPI ChooseColorW_Hook(LPCHOOSECOLORW lpcc) {
    if (!settings.betterColorPicker || FLAG(lpcc->Flags, CC_ENABLETEMPLATE))
    ...
    else { data.hue = 0; data.sat = 0; data.val = 1.0f; } // default white
...
static void FP_PaintPreview(HWND hwnd, HDC hdc, FontPickerData* d) {
    ...
    if (bgBr) { d->pRT->FillRoundedRectangle(prevRR, bgBr); bgBr->Release(); }
    // Border
    ID2D1SolidColorBrush* bdrBr = nullptr;
    d->pRT->CreateSolidColorBrush(D2D1::ColorF(0.4f,0.4f,0.4f,0.5f), &bdrBr);
    if (bdrBr) { d->pRT->DrawRoundedRectangle(prevRR, bdrBr); bdrBr->Release(); }
...
static LRESULT CALLBACK FP_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Size
        WCHAR szBuf[16]; wsprintfW(szBuf, L"%d", d->pointSize10 > 0 ? d->pointSize10/10 : 12);
        ...
        for (int i = 0; i < (int)(sizeof(g_fpSizes)/sizeof(g_fpSizes[0])); i++) {
            if (g_fpSizes[i] == curSz) { SendMessageW(hSizList, LB_SETCURSEL, i, 0); break; }
        }
    ...
    case WM_COMMAND: {
        WORD id = LOWORD(wParam), notif = HIWORD(wParam);
        if (id == IDOK) { FP_SyncFromControls(hwnd, d); d->accepted = TRUE; DestroyWindow(hwnd); return 0; }
        if (id == IDCANCEL) { d->accepted = FALSE; DestroyWindow(hwnd); return 0; }
        ...
        if (id == IDC_FP_STRIKE || id == IDC_FP_UNDER) FP_SyncFromControls(hwnd, d);
        return 0; }
...
BOOL WINAPI ChooseFontW_Hook(LPCHOOSEFONTW lpcf) {
    ...
    if (lpcf->Flags & CF_ENABLETEMPLATE) return ChooseFontW_Original(lpcf);
...
static BOOL ModernFileDialog_Impl(LPOPENFILENAMEW lpofn, BOOL bOpen) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (lpofn->Flags & OFN_READONLY) {
            pCustomize->AddCheckButton(IDC_MODERN_READONLY, L"Open as read-only",
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (lpofn->Flags & OFN_READONLY) {
            ...
            if (SUCCEEDED(pfd->QueryInterface(IID_PPV_ARGS(&pCust)))) {
                BOOL checked = FALSE;
                if (SUCCEEDED(pCust->GetCheckButtonState(IDC_MODERN_READONLY, &checked))) {
                    if (checked)
                        lpofn->Flags |= OFN_READONLY;
                    else
                        lpofn->Flags &= ~OFN_READONLY;
...
static LPWSTR FilterAtoW(LPCSTR filter) {
    ...
    if (buf) MultiByteToWideChar(CP_ACP, 0, filter, (int)totalBytes, buf, wlen);
    return buf;
}
