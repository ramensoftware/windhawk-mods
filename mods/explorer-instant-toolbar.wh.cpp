// ==WindhawkMod==
// @id              explorer-instant-toolbar
// @name            True Windows 11 Explorer
// @description     Мод полностью заменяет интерфейс проводника Windows 11 на созданный мною с нуля, который работает мгновенно, в отличие от стандартного. С возможностью кастомизации.
// @version         1.73
// @author          Yevhenii
...
- tabs:
  - enabled: true
    $name: Показывать вкладки
    $description: Выключите — вкладок не будет (и пункта «Открыть в новой вкладке», и открытия средней кнопкой), верх окна станет ниже. Открытые окна лучше закрыть и открыть заново.
  - width: 248
    $name: Ширина вкладки
    $description: В точках (у Windows 11 — 248). При динамической ширине — наибольшая ширина.
  - dynamicWidth: false
    $name: Динамическая ширина
    $description: Ширина каждой вкладки — по длине её названия, крестик появляется при наведении.
  - icons: true
    $name: Значок папки на вкладке
    $description: Выключите — вкладки станут уже.
  - closeButtons: all
    $name: Крестик на вкладках
    $description: Где показывать крестик закрытия (вкладку можно закрыть и средней кнопкой мыши).
    $options:
    - all: На всех вкладках (как в Windows 11)
    - active: Только на выбранной и под указателем
    - none: Не показывать
  - menuItem: true
    $name: Пункт «Открыть в новой вкладке» в меню папки
    $description: Выключите — пункта в меню правой кнопки мыши не будет (средняя кнопка мыши по-прежнему открывает папку во вкладке).
  - scroll: false
    $name: Прокрутка вкладок
    $description: Если вкладки не помещаются — не сужать их дальше, а показывать стрелки прокрутки (как у Windows 11). Листаются и колёсиком мыши.
  - remember: false
    $name: Запоминать вкладки
    $description: При закрытии окна запоминать его вкладки и открывать их в следующем новом окне. Хранится в настройках самого Windhawk, не в системе.
  - newTabFolder: computer
    $name: Папка новой вкладки
    $description: Что открывать по «+» и Ctrl+T.
    $options:
    - computer: Этот компьютер
    - downloads: Загрузки
    - custom: Своя папка (путь ниже)
  - newTabPath: ""
    $name: Путь своей папки
    $description: Скопируйте путь папки (например, D:\Programs), вставьте сюда и нажмите «Сохранить».
  $name: Вкладки
- address:
  - navGap: 8
    $name: Промежуток между кнопками «Назад / Вперёд / Вверх / Обновить»
    $description: В точках (у Windows 11 — 16). Чем меньше, тем шире адресная строка.
  - height: "32"
    $name: Высота адресной строки и поиска
    $description: В точках. Шрифт подстраивается, но не мельче читаемого. Не выше, чем позволяет высота строки ниже.
    $options:
    - "24": 24 (наименьшая)
    - "26": "26"
    - "28": "28"
    ...
    - "44": 44 (наибольшая)
  - areaHeight: "49"
    $name: Высота строки с адресной строкой
    $description: Вся полоса с кнопками «Назад / Вперёд / Вверх / Обновить», адресной строкой и поиском, в точках.
    $options:
    - "36": 36 (наименьшая)
    - "40": "40"
    - "44": "44"
  ...
  - search: true
    $name: Поле поиска
    $description: Выключите — поиска не будет, адресная строка станет во всю ширину.
  - everything: false
    $name: Мгновенный поиск через Everything
    $description: При вводе в поле поиска сразу показывать найденное программой Everything (она должна быть запущена).
  $name: Адресная строка и поиск
- toolbar:
  - enabled: true
    $name: Показывать панель кнопок
    $description: Выключите — панели «Создать / Вырезать / Копировать…» не будет, список файлов начнётся сразу под адресной строкой.
  - height: "47"
    $name: Высота панели кнопок
    $description: Полоса с кнопками «Создать», «Вырезать», «Копировать» и т. д., в точках.
    $options:
    - "36": 36 (наименьшая)
    - "38": "38"
    - "40": "40"
...
- window:
  - animations: false
    ...
    $description: Плавное появление подсветки при наведении, плавное открытие, закрытие и перестановка вкладок.
  - snapLayouts: true
    $name: Раскладки окна при наведении на «Развернуть»
    $description: Панель Windows с раскладками окна (и предложениями групп окон) при наведении на кнопку «Развернуть».
  $name: Окно
- navpane:
  - noIndent: false
    $name: Без отступа слева
    $description: Значки и названия — вплотную к левому краю. Вложенные папки тоже почти без сдвига вправо.
  - hidePins: false
    $name: Скрыть булавки у закреплённых папок
  $name: Область навигации
- debug:
  - info: false
    $name: Показывать отладку
    $description: Показывать в полосе вкладок, что мод видит в окне (щелчок по надписи — скопировать).
  $name: Отладка
*/
// ==/WindhawkModSettings==
...
Этот мод разрабатывался под версию 26h2 - Как будет работать в других версиях Windows? Я не знаю, нужны тесты.

Чистый слой: работает только пока включён, ничего не пишет в реестр и на диск.
Не даёт Проводнику загружать медленный верх Windows 11 — окно открывается быстро, как в Windows 10.
Вместо него рисует свой мгновенный верх (кнопки окна, «Назад/Вперёд/Вверх/Обновить», адресная строка, поиск)
и свою панель кнопок в стиле Windows 11 со значками Windows под тему.
*/
// ==/WindhawkModReadme==
...
static const wchar_t* PROP_INACTIVE = L"EIT_Inactive";   // окно не в фокусе (по сообщению Windows, как у встроенного верха)
...
static volatile LONG g_lightCache = -1;   // -1 — не прочитано (сбрасывается при смене настроек Windows)
static bool IsLight() {
    LONG c = g_lightCache;
    if (c >= 0) return c != 0;
...
static HICON MakeIcon(wchar_t g, Accent ac, int s, bool light, bool disabled) { return IconFromPx(RenderGlyph(g, ac, s, light, disabled), s); }
...
static RECT CapBtnRect(HWND top, int which);
static void UpdateSysButtons(HWND top) {   // спросить Windows, где её кнопки окна
    RECT b = {};
    bool ok = SUCCEEDED(DwmGetWindowAttribute(top, 5 /*DWMWA_CAPTION_BUTTON_BOUNDS*/, &b, sizeof(b))) && b.right - b.left > 0 && b.bottom - b.top > 0;
    SetPropW(top, L"EIT_SB", (HANDLE)(INT_PTR)(ok ? 1 : 2));
    if (ok) { SetPropW(top, L"EIT_SB1", (HANDLE)(INT_PTR)MAKELONG(b.left, b.top)); SetPropW(top, L"EIT_SB2", (HANDLE)(INT_PTR)MAKELONG(b.right, b.bottom)); }
}
// «Свернуть / Развернуть / Закрыть»: у окна со своим верхом Windows рисует их сама — поверх её кнопок наши давали двоение.
// Если Windows их рисует — пользуемся её кнопками (вид, подсветка, раскладки), свои не рисуем
static bool SysButtons(HWND top, RECT* screen = nullptr) {
    if (!top) return false;
    if (!GetPropW(top, L"EIT_SB")) UpdateSysButtons(top);
    if ((INT_PTR)GetPropW(top, L"EIT_SB") != 1) return false;
    DWORD a = (DWORD)(INT_PTR)GetPropW(top, L"EIT_SB1"), c = (DWORD)(INT_PTR)GetPropW(top, L"EIT_SB2");
    RECT b = {(short)LOWORD(a), (short)HIWORD(a), (short)LOWORD(c), (short)HIWORD(c)};
    if (screen) { RECT wr; GetWindowRect(top, &wr); OffsetRect(&b, wr.left, wr.top); *screen = b; }   // Windows даёт место от угла окна
    return true;
}
static bool IsRibbonPart(HWND h) {   // части ленты Windows 10 внутри окна Проводника под модом
    wchar_t c[40];
    if (!GetClassNameW(h, c, 40) || wcsncmp(c, L"UIRibbon", 8) != 0) return false;
    HWND p = GetParent(h);
    return p && GetPropW(p, L"EIT_OldProc");
}
...
static const wchar_t* NEW_TEXT = L"Создать";
...
static int NewTextWidth(UINT dpi) {
    int dw = TextWidthDW(NEW_TEXT, dpi);
    if (dw > 0) return dw;
    HDC dc = GetDC(nullptr);
    HFONT f = BarFont(dpi); HGDIOBJ of = SelectObject(dc, f);
    SIZE sz = {}; GetTextExtentPoint32W(dc, NEW_TEXT, (int)wcslen(NEW_TEXT), &sz);
...
static void PaintContent(HWND h, StripState* st, HDC dc, RECT cr) {
    ...
    for (auto& sl : Layout(h)) {
        ...
        if (a == A_VIEWMENU) {
            ...
            DrawTextDW(dc, tr, NEW_TEXT, light ? RGB(27, 27, 27) : RGB(255, 255, 255), dpi);
...
static void BuildShowItems(MList* m, int s, bool light, const std::remove_pointer_t<LPSHELLSTATE>& ss, bool nav) {
    ...
    AddItem(m, M_NAVPANE, L"Область навигации", nav ? 1 : 0, 0xE8A0, s, light, {"windowsnavigationpane", "navigationpane"});
    AddItem(m, M_STATUSBAR, L"Строка состояния", GetStatusBar() ? 1 : 0, 0xE9F9, s, light, {"windowsstatusbar", "statusbar"});
    AddSep(m);
    AddItem(m, M_COMPACT, L"Компактное представление", GetCompact() ? 1 : 0, 0xE8FD, s, light, {"windowscompactmode"});
    AddSep(m);
    AddItem(m, M_CHECKBOXES, L"Флажки элементов", ss.fAutoCheckSelect ? 1 : 0, 0xE73A, s, light, {"windowsselectioncheckboxes"});
    AddItem(m, M_EXTENSIONS, L"Расширения имен файлов", ss.fShowExtensions ? 1 : 0, 0xE8A5, s, light, {"windowsshowfileextensions"});
    AddItem(m, M_HIDDEN, L"Скрытые элементы", ss.fShowAllObjects ? 1 : 0, 0xE7B3, s, light, {"windowsshowhiddenfiles"});
}
...
static void ShowSortMenu(HWND strip, const RECT& btn) {
    ...
    struct Col { UINT id; const wchar_t* text; DWORD pid; } cols[] = {
        {S_NAME, L"Имя", 10}, {S_DATE, L"Дата изменения", 14}, {S_TYPE, L"Тип", 4}, {S_SIZE, L"Размер", 12}};
    UINT cmd = RunMenu(strip, btn, [&](MList* m, int s, bool light) {
        ...
        AddSep(m);
        AddItem(m, S_ASC, L"По возрастанию", cur.direction == SORT_ASCENDING ? 2 : 0, 0, s, light, {});
        AddItem(m, S_DESC, L"По убыванию", cur.direction == SORT_DESCENDING ? 2 : 0, 0, s, light, {});
    });
    SORTCOLUMN nc = cur;
...
static void ShowViewMenu(HWND strip, const RECT& btn) {
    ...
    struct V { UINT id; const wchar_t* text; wchar_t glyph; std::vector<const char*> keys; } vs[] = {
        {V_HUGE, L"Огромные значки", 0xE8A9, {"windowsiconsizeextralarge"}},
        {V_LARGE, L"Крупные значки", 0xE8A9, {"windowsiconsizelarge"}},
        {V_MEDIUM, L"Обычные значки", 0xE8A9, {"windowsiconsizemedium"}},
        {V_SMALL, L"Мелкие значки", 0xE8A9, {"windowsiconsizesmallicon"}},
        {V_LIST, L"Список", 0xE8FD, {"windowsiconsizelist"}},
        {V_DETAILS, L"Таблица", 0xE8FD, {"windowsiconsizedetails"}},
        {V_TILES, L"Плитка", 0xE8FD, {"windowsiconsizetile"}},
        {V_CONTENT, L"Содержимое", 0xE8FD, {"windowsiconsizecontent"}},
    };
    UINT cmd = RunMenu(strip, btn, [&](MList* m, int s, bool light) {
...
static const wchar_t* TipText(Act a) {
    switch (a) {
        case A_OPTIONS: return L"Параметры";
        case A_VIEWMENU: return L"Параметры представления";
        case A_NEW:     return L"Создать";
        case A_SORT:    return L"Сортировать";
        case A_PROPS:   return L"Свойства";
        case A_VIEW:    return L"Просмотреть";
        case A_UNDO:    return L"Отменить";
        case A_REDO:    return L"Повторить";
        case A_CUT:     return L"Вырезать";
        case A_COPY:    return L"Копировать";
        case A_PASTE:   return L"Вставить";
        case A_RENAME:  return L"Переименовать";
        case A_DELETE:  return L"Удалить";
        default:        return L"";
    }
}
...
static std::wstring ChildDebug(HWND top) {   // отладка: части окна (вид, видно ли, где по высоте)
    std::wstring r = L" · части окна:";
    int n = 0;
    for (HWND c = GetWindow(top, GW_CHILD); c && n < 14; c = GetWindow(c, GW_HWNDNEXT), ++n) {
        ...
        wchar_t b[100]; swprintf(b, 100, L" %ls%ls %d-%d", cls, IsWindowVisible(c) ? L"" : L"(скрыто)", (int)rc.top, (int)rc.bottom);
        r += b;
    }
    RECT wr, cr; GetWindowRect(top, &wr); GetClientRect(top, &cr); MapWindowPoints(top, nullptr, (POINT*)&cr, 2);
    wchar_t b[80]; swprintf(b, 80, L" · рамка сверху: %d", (int)(cr.top - wr.top));   // есть ли у окна полоса заголовка Windows
    r += b;
    return r;
}
static std::wstring NavDebug(HWND top) {   // область навигации для отладки
    HWND tv = FindChild(top, L"SysTreeView32", true);
    if (!tv) return L" · дерево слева: не найдено";
    wchar_t b[120];
    swprintf(b, 120, L" · дерево слева: отступ %d, под присмотром мода %ls", (int)SendMessageW(tv, TVM_GETINDENT, 0, 0), GetPropW(tv, L"EIT_NavOld") ? L"да" : L"нет");
    return b;
}
static std::wstring DebugText(HWND top, PCIDLIST_ABSOLUTE cur, bool cpanel) {
    ...
    return std::wstring(L"панель управления: ") + (cpanel ? L"да" : L"нет") + L" · адрес окна: " + a + L" · папка списка: " + PidlText(cur) +
           L" · список файлов: " + (has ? L"есть" : L"нет") + L" · панель кнопок: " + (strip && IsWindowVisible(strip) ? L"видна" : L"скрыта") +
           [top] {
               ...
               swprintf(b, 80, L" · фон панели: %u,%u,%u (%ls)", GetRValue(c), GetGValue(c), GetBValue(c),
                        fromView ? L"у списка файлов" : g_listBgTheme[l ? 1 : 0] ? L"из темы" : L"запасной");
        ...
        for (auto& e : layout) {
            ...
            switch (e.id) {
                case T_CLOSE: case T_MAX: case T_MIN: {
                    if (SysButtons(top)) {   // кнопки рисует Windows; у развёрнутого окна подсвечиваем сами, значки её
                        if (float k = IsZoomed(top) ? HA(st, e.id) : 0.f) {
                            cv.Fill(e.r, Ax(e.id == T_CLOSE ? Hex(down ? 0xE6C42B1C : 0xFFC42B1C) : (down ? sub2 : sub1), k));
                            if (e.id == T_CLOSE) cv.Glyph(e.r, 0xE8BB, Hex(0xFFFFFFFF), 10.f);   // на красном — белый крестик
                        }
                        break;
                    }
                ...
                case T_ADDR: case T_SEARCH: {
                    ...
                    if (e.id == T_ADDR) {
                        ...
                        if (!focus) {
                            std::wstring ph = L"Поиск в: " + (st->crumbs.empty() ? std::wstring() : st->crumbs.back().name);
...
static const wchar_t* TopTipText(HWND bar, TopBarState* st, int id) {
    ...
    if (id >= T_TABX && id < T_TABX + 100) return L"Закрыть вкладку (Ctrl+W)";
    switch (id) {
        case T_BACK: return L"Назад (Alt+Стрелка влево)";
        case T_FWD: return L"Вперёд (Alt+Стрелка вправо)";
        case T_UP: return L"Вверх (Alt+Стрелка вверх)";
        case T_REFRESH: return L"Обновить (F5)";
        case T_ADDTAB: return L"Новая вкладка (Ctrl+T)";
        case T_DBG: return L"Скопировать";
        case T_TABL: return st->canL ? L"Прокрутить вкладки влево" : L"";
        case T_TABR: return st->canR ? L"Прокрутить вкладки вправо" : L"";
        case T_MIN: return L"Свернуть";
        case T_MAX: { HWND top = GetAncestor(bar, GA_ROOT); return top && IsZoomed(top) ? L"Свернуть в окно" : L"Развернуть"; }
        case T_CLOSE: return L"Закрыть";
    }
    return L"";
}
...
struct EvCat { wchar_t glyph; const wchar_t* name; const wchar_t* macro; };
static const EvCat EV_CATS[] = {
    {0xE71D, L"Все файлы", L""},        {0xECAA, L"Программы", L"exe: "}, {0xE8B7, L"Папки", L"folder: "},
    {0xE8A5, L"Документы", L"doc: "},   {0xE91B, L"Изображения", L"pic: "}, {0xE714, L"Видео", L"video: "},
    {0xE8D6, L"Музыка", L"audio: "},    {0xF012, L"Архивы", L"zip: "}};
...
static void EvPaint(HWND h) {
    ...
    if (g_ev->items.empty()) {
        ...
        DrawTextDWSize(dc, tr, EverythingWindow() ? L"Ничего не найдено" : L"Everything не запущена", fg2, dpi, 14.f);
    }
...
static void SwQuery() {
    ...
    std::wstring t = L"Поиск" + (g_sw->scope.empty() ? std::wstring() : L" в " + g_sw->scope) + (g_sw->text.empty() ? std::wstring() : L": " + g_sw->text);
...
static void SwPaint(HWND h) {
    ...
    if (g_sw->total || !g_sw->scope.empty()) {   // где ищем и сколько нашли
        wchar_t cnt[600];
        if (g_sw->total) swprintf(cnt, 600, g_sw->scope.empty() ? L"Найдено: %lu%ls" : L"Найдено: %lu в %ls", (unsigned long)g_sw->total, g_sw->scope.c_str());
        else swprintf(cnt, 600, L"Поиск в %ls", g_sw->scope.c_str());
    ...
    DrawTextDWSize(dc, {c.name, hy, c.nameR, lt}, L"Имя", fg2, dpi, 12.f);
    DrawTextDWSize(dc, {c.folder, hy, c.folderR, lt}, L"Папка", fg2, dpi, 12.f);
    DrawTextDWSize(dc, {c.size, hy, c.sizeR, lt}, L"Размер", fg2, dpi, 12.f, 400, false, L"Segoe UI", 2);
    DrawTextDWSize(dc, {c.date, hy, c.dateR, lt}, L"Дата изменения", fg2, dpi, 12.f);
    ...
    const wchar_t* empty = g_sw->text.empty() ? L"Введите запрос" : !EverythingWindow() ? L"Everything не запущена" : n ? nullptr : L"Ничего не найдено";
...
static void SwShow(HWND owner, const std::wstring& text, int cat, const std::wstring& scope) {   // открыть окно поиска (или обновить уже открытое)
    ...
    if (!g_sw->h) {
        ...
        g_sw->h = CreateWindowExW(0, SW_CLASS, L"Поиск", WS_OVERLAPPEDWINDOW, x, y, w, hh, owner, nullptr, g_inst, nullptr);
        if (!g_sw->h) return;
...
static void EndEdit(HWND bar, TopBarState* st, bool apply) {
    ...
    if (kind == T_SEARCH) {   // поиск Windows в текущей папке
        ...
        target = L"search-ms:displayname=" + UrlPart(L"Результаты поиска") + L"&crumb=System.Generic.String%3A" + UrlPart(buf) +
...
LRESULT CALLBACK TopBarProc(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    switch (m) {
        ...
        case WM_TIMER:
            ...
            if (st && w == 8) {
                ...
                if (back || GetTickCount() - st->frozeAt > 1500) {   // вернулись (или на всякий случай — не дольше 1,5 с)
                    ...
                    if (st->locked) { LockWindowUpdate(nullptr); st->locked = false; }
        ...
        case WM_NCHITTEST: {
            ...
            {   // над кнопками окна Windows — указатель самому окну: Windows сама подсветит их и покажет раскладки
                RECT sb; POINT sp = {(short)LOWORD(l), (short)HIWORD(l)};
                HWND rt = GetAncestor(h, GA_ROOT);
                if (SysButtons(rt, &sb)) {
                    if (PtInRect(&sb, sp)) return HTTRANSPARENT;
                    for (int k = 0; k < 3; ++k) { RECT cb = CapBtnRect(rt, k); if (PtInRect(&cb, sp)) return HTTRANSPARENT; }   // развёрнутое окно: Windows сообщает место со сдвигом
                }
            }
        ...
        case WM_RBUTTONUP: {   // правая кнопка по вкладке — меню: закрепить / открепить, закрыть
            ...
            UINT cmd = RunMenu(h, r, [&](MList* ml, int s, bool light) {
                AddItem(ml, 1, pin ? L"Открепить вкладку" : L"Закрепить вкладку", 0, pin ? 0xE77A : 0xE718, s, light, {});
                AddItem(ml, 2, L"Закрыть вкладку", 0, 0xE711, s, light, {});
            });
            g_menuCompact = false;
        ...
        case WM_APP + 34:   // область навигации переключили: перейти в папку выше и сразу обратно, чтобы окно перестроилось
            if (st && st->cur) {
                ...
                st->locked = LockWindowUpdate(top) != FALSE;   // окно «замирает» на экране — перехода туда-обратно не видно (другое окно уже заморожено — просто без этого)
...
static void PlaceTopBar(HWND top) {
    ...
    SetWindowPos(bar, HWND_TOP, 0, 0, cr.right, p.top, SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOCOPYBITS);   // без копии старой картинки
    UpdateSysButtons(top);   // размер окна мог поменяться — место кнопок Windows тоже   // от верха окна до нашей панели кнопок
    ...
    for (HWND c = GetWindow(top, GW_CHILD); c; c = GetWindow(c, GW_HWNDNEXT))   // части ленты — спрятаны
        if (IsRibbonPart(c) && IsWindowVisible(c)) ShowWindow(c, SW_HIDE);
    InvalidateRect(bar, nullptr, FALSE);
...
static void UseOwnFrame(HWND h) {   // один раз, когда окно в быстром виде: убираем заголовок Windows 10 и просим скругление
    ...
    {   // окно Проводника не рисует поверх наших частей (иначе при развороте и смене фокуса поверх верха остаются куски его картинки)
        LONG stl = GetWindowLongW(h, GWL_STYLE);
        if (!(stl & WS_CLIPCHILDREN)) { SetWindowLongW(h, GWL_STYLE, stl | WS_CLIPCHILDREN); SetPropW(h, L"EIT_ClipAdded", (HANDLE)1); }
    }
...
static RECT CapBtnRect(HWND top, int which) {   // место кнопки окна по нашей разметке, на экране: 0 — «Свернуть», 1 — «Развернуть», 2 — «Закрыть»
    RECT r = {};
    HWND bar = (HWND)GetPropW(top, PROP_TOPBAR);
    ...
    UINT dpi = DpiOf(bar);
    double W = cr.right * 96.0 / dpi;
    double x0 = which == 0 ? W - 47.333 - 46.667 - 47.333 : which == 1 ? W - 47.333 - 46.667 : W - 47.333;
    double w = which == 1 ? 46.667 : 47.333;
    r = {P(x0, dpi), 0, P(x0 + w, dpi), P(30, dpi)};
    if (which == 2) r.right = cr.right;
...
static void PlaceStrip(HWND top) {
    ...
    SetWindowPos(strip, HWND_TOP, o.x, o.y, w, hh, SWP_NOACTIVATE | SWP_SHOWWINDOW | SWP_NOCOPYBITS);
...
static BOOL ShowFolderMenuWithTab(HMENU m, UINT f, int x, int y, HWND owner, LPTPMPARAMS pp, bool ex, int reserved, const RECT* rc) {
    ...
    InsertMenuW(m, at + 1, MF_BYPOSITION | MF_STRING, ID_OPEN_NEW_TAB, L"Открыть в новой вкладке");
...
LRESULT CALLBACK TopProc(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    if ((m == WM_NCHITTEST || (m >= WM_NCMOUSEMOVE && m <= WM_NCXBUTTONDBLCLK) || m == WM_NCMOUSELEAVE) &&
        GetPropW(h, PROP_OWNFRAME) && SysButtons(h)) {   // кнопки окна рисует Windows — она их и ведёт (только сообщения мыши над рамкой)
        LRESULT dr = 0;
        if (DwmDefWindowProc(h, m, w, l, &dr)) return dr;
        if (m == WM_NCHITTEST) {   // развёрнутое окно: Windows не узнаёт свою кнопку под указателем — называем её сами
            POINT pt = {(short)LOWORD(l), (short)HIWORD(l)};
            static const LRESULT codes[3] = {HTMINBUTTON, HTMAXBUTTON, HTCLOSE};
            for (int k = 0; k < 3; ++k) { RECT cb = CapBtnRect(h, k); if (PtInRect(&cb, pt)) return codes[k]; }
        }
    }
    if (GetPropW(h, PROP_OWNFRAME)) {
        ...
        if (m == WM_NCMOUSEMOVE) {
            int id = w == HTMAXBUTTON ? T_MAX : w == HTMINBUTTON ? T_MIN : w == HTCLOSE ? T_CLOSE : T_NONE;
            if (id != T_NONE) { SetBarHover(h, id, -2); TRACKMOUSEEVENT t = {sizeof(t), TME_LEAVE | TME_NONCLIENT, h, 0}; TrackMouseEvent(&t); }
    ...
    if (m == g_msgDestroy || m == WM_NCDESTROY) {
        ...
        if (GetPropW(h, PROP_OWNFRAME)) {
            ...
            if (RemovePropW(h, L"EIT_ClipAdded")) SetWindowLongW(h, GWL_STYLE, GetWindowLongW(h, GWL_STYLE) & ~WS_CLIPCHILDREN);   // как было
    ...
    if (m == WM_SIZE || m == WM_MOVE || m == WM_DPICHANGED || m == WM_WINDOWPOSCHANGED || m == WM_SHOWWINDOW)
        PlaceStrip(h);
    if ((m == WM_NCPAINT || m == WM_NCACTIVATE || (m == WM_SIZE && (w == SIZE_MAXIMIZED || w == SIZE_RESTORED))) && GetPropW(h, PROP_OWNFRAME)) {
        // Windows только что перерисовала рамку окна (при развороте, возврате в окно, смене фокуса) — наш верх и панель сразу поверх, целиком
        if (HWND tb = (HWND)GetPropW(h, PROP_TOPBAR)) if (IsWindowVisible(tb)) RedrawWindow(tb, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
        if (HWND sp = (HWND)GetPropW(h, PROP_STRIP)) if (IsWindowVisible(sp)) RedrawWindow(sp, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
    }
    if (m == WM_WINDOWPOSCHANGED && l && (((WINDOWPOS*)l)->flags & SWP_SHOWWINDOW)) {   // окно появилось — наш верх и панель нарисовать сейчас же: иначе в первые мгновения сверху видна строка ленты
        if (HWND tb = (HWND)GetPropW(h, PROP_TOPBAR)) RedrawWindow(tb, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
        if (HWND sp = (HWND)GetPropW(h, PROP_STRIP)) RedrawWindow(sp, nullptr, nullptr, RDW_INVALIDATE | RDW_UPDATENOW);
    }
...
static int PosKind(HWND h) {   // 1 — часть ленты окна под модом, 2 — место списка файлов окна под модом, 0 — остальное
    wchar_t cls[40];
    if (!GetClassNameW(h, cls, 40)) return 0;
    bool rib = wcsncmp(cls, L"UIRibbon", 8) == 0, tab = !rib && wcscmp(cls, L"ShellTabWindowClass") == 0;
    if (!rib && !tab) return 0;
    HWND p = GetParent(h);
    if (!p || !GetPropW(p, L"EIT_OldProc")) return 0;
    return rib ? 1 : 2;
}
static int WantTopOfTab(HWND tab) {   // сколько сверху должен начинаться список файлов (0 — не трогаем)
    HWND par = GetParent(tab);
    if (!IsClassicTop(par)) return 0;
    return (int)lround(TopTotalFor(par) * DpiOf(par) / 96.0);
}
static void GuardFramePaint(HWND top, HDC dc, void* ret) {
    if (!dc || !top || !GetPropW(top, L"EIT_OldProc")) return;
    HMODULE m = nullptr;
    ...
    static HMODULE ribbon = nullptr;   // модуль ленты — узнаём один раз
    if (!ribbon) ribbon = GetModuleHandleW(L"UIRibbon.dll");
    if (m == ribbon) { IntersectClipRect(dc, 0, 0, 0, 0); return; }   // лента — ничего
    RECT wr; GetWindowRect(top, &wr);
    for (const wchar_t* pr : {L"EIT_TopBar", L"EIT_Strip"}) {   // остальные — не поверх наших частей
        HWND c = (HWND)GetPropW(top, pr);
        if (!c || !IsWindowVisible(c)) continue;
        RECT r; GetWindowRect(c, &r);
        ExcludeClipRect(dc, r.left - wr.left, r.top - wr.top, r.right - wr.left, r.bottom - wr.top);
    }
}
using GWDC_t = HDC (WINAPI*)(HWND);
static GWDC_t GWDC_orig;
HDC WINAPI GWDC_hook(HWND h) { void* ret = __builtin_return_address(0); HDC dc = GWDC_orig(h); GuardFramePaint(h, dc, ret); return dc; }
using GDCX_t = HDC (WINAPI*)(HWND, HRGN, DWORD);
static GDCX_t GDCX_orig;
HDC WINAPI GDCX_hook(HWND h, HRGN r, DWORD f) { void* ret = __builtin_return_address(0); HDC dc = GDCX_orig(h, r, f); if (f & DCX_WINDOW) GuardFramePaint(h, dc, ret); return dc; }
...
BOOL WINAPI SWP_hook(HWND h, HWND after, int x, int y, int cx, int cy, UINT f) {
    int k = PosKind(h);
    if (k == 1) { f &= ~SWP_SHOWWINDOW; f |= SWP_HIDEWINDOW; }   // лента не показывается
    if (k != 2) return SWP_orig(h, after, x, y, cx, cy, f);
    if (!(f & SWP_NOMOVE)) { int t = WantTopOfTab(h); if (t && y < t) { if (!(f & SWP_NOSIZE)) cy -= t - y; y = t; } }
...
HDWP WINAPI DWP_hook(HDWP d, HWND h, HWND after, int x, int y, int cx, int cy, UINT f) {
    int k = PosKind(h);
    if (k == 1) { f &= ~SWP_SHOWWINDOW; f |= SWP_HIDEWINDOW; }
    if (k != 2) return DWP_orig(d, h, after, x, y, cx, cy, f);
    if (!(f & SWP_NOMOVE)) { int t = WantTopOfTab(h); if (t && y < t) { if (!(f & SWP_NOSIZE)) cy -= t - y; y = t; } }
...
BOOL WINAPI MW_hook(HWND h, int x, int y, int cx, int cy, BOOL rp) {
    int k = PosKind(h);
    if (k == 1) cy = 0;
    if (k != 2) return MW_orig(h, x, y, cx, cy, rp);
    int t = WantTopOfTab(h); if (t && y < t) { cy -= t - y; y = t; }
...
BOOL Wh_ModInit() {
    ...
    {   // быстрый вид Проводника — перехват запроса нового верха
        ...
        HookFn(u32, "GetWindowDC", (void*)GWDC_hook, &GWDC_orig);   // лента не рисует на окне
        HookFn(u32, "GetDCEx", (void*)GDCX_hook, &GDCX_orig);
