...
struct ModSettings {
    ...
    int          vizPadLeft      = 0;
    int          vizPadRight     = 0;
    int          vizOverlayOpacity = 25;
    bool         lxEnabled              = true;
    std::wstring lxHost                 = L"127.0.0.1";
    int          lxPort                 = 23330;
    bool         lyricsReplaceArtist    = true;
    bool         lyricsScroll           = true;
    int          lyricsControlsUpOffset = 6;
    int          lyricFontSize          = 12;
    std::wstring lyricFont              = L"";
    int          lyricTopSpacing        = 4;
    std::wstring lyricNoLyricMode       = L"show_artist";
    bool         lyricDualLine          = true;
    int          lyricSecondLineSize    = 9;
    int          lyricSecondLineOpacity = 50;
    std::wstring lyricColor             = L"";
    bool         lyricShadow            = false;
    bool         enableMiniPlayerPopup  = true;
};
...
static void LoadSettings() {
    ...
    g_settings.vizSensitivity  = Int(L"MainSettings.VisualizerFunctionsSettings.vizSensitivity", 0, 300, 150);
    {
        ...
        g_settings.vizOverlayOpacity = Int(L"MainSettings.VisualizerFunctionsSettings.vizOverlayOpacity", 0, 100, 25);
    }
    {
    ...
    g_settings.lyricsControlsUpOffset = Int(L"LxMusicSettings.lyricsControlsUpOffset", 0, 40, 6);
    g_settings.lyricFontSize          = Int(L"LxMusicSettings.lyricFontSize", 6, 48, 12);
    g_settings.lyricFont               = Str(L"LxMusicSettings.lyricFont", L"");
    g_settings.lyricTopSpacing         = Int(L"LxMusicSettings.lyricTopSpacing", -20, 40, 4);
    g_settings.lyricNoLyricMode        = Str(L"LxMusicSettings.lyricNoLyricMode", L"show_artist");
    g_settings.lyricDualLine           = Wh_GetIntSetting(L"LxMusicSettings.lyricDualLine") != 0;
    g_settings.lyricSecondLineSize     = Int(L"LxMusicSettings.lyricSecondLineSize", 6, 20, 9);
    g_settings.lyricSecondLineOpacity  = Int(L"LxMusicSettings.lyricSecondLineOpacity", 0, 100, 50);
    g_settings.lyricColor              = Str(L"LxMusicSettings.lyricColor", L"");
    g_settings.lyricShadow             = Wh_GetIntSetting(L"LxMusicSettings.lyricShadow") != 0;
    g_settings.enableMiniPlayerPopup   = Wh_GetIntSetting(L"BehaviorSettings.enableMiniPlayerPopup") != 0;
...
static winrt::Windows::UI::Color LyricColor() {
    if (!g_settings.lyricColor.empty()) {
        std::wstring s = g_settings.lyricColor;
        // 去除首尾空白
        s.erase(0, s.find_first_not_of(L" \t"));
        s.erase(s.find_last_not_of(L" \t") + 1);
        // 支持十六进制格式：#RRGGBB 或 RRGGBB
        if (s.front() == L'#') s.erase(s.begin());
        if (s.length() == 6) {
            try {
                BYTE r = (BYTE)std::stoi(s.substr(0, 2), nullptr, 16);
                BYTE g = (BYTE)std::stoi(s.substr(2, 2), nullptr, 16);
                BYTE b = (BYTE)std::stoi(s.substr(4, 2), nullptr, 16);
        ...
        if (s.length() == 8) {
            try {
                BYTE a = (BYTE)std::stoi(s.substr(0, 2), nullptr, 16);
                BYTE r = (BYTE)std::stoi(s.substr(2, 2), nullptr, 16);
                BYTE g = (BYTE)std::stoi(s.substr(4, 2), nullptr, 16);
                BYTE b = (BYTE)std::stoi(s.substr(6, 2), nullptr, 16);
                return winrt::Windows::UI::Color{a, r, g, b};
            } catch (...) {}
        }
        // 回退到原生 R G B 空格格式
        return ParseColorWithSpecialValues(s, 255);
    }
    return ArtistColor();
}
...
static std::wstring g_scrollCachedLyric;
static double g_lyricsCtrlShift = -1.0;
...
static void ExecuteMediaAction(const std::wstring& action, FrameworkElement const& sourceElement = nullptr) {
    if (action == L"none") {
        ...
        if (!g_settings.enableMiniPlayerPopup) return;
        if (sourceElement) {
...
static constexpr wchar_t kLyricBlockName[]       = L"FluentMedia_Lyric";
static constexpr wchar_t kLyricSecondBlockName[] = L"FluentMedia_LyricSecond";
static constexpr wchar_t kLyricStackName[]       = L"FluentMedia_LyricStack";
static constexpr wchar_t kLyricShadowBlockName[] = L"FluentMedia_LyricShadow";
static constexpr wchar_t kControlPanelName[]     = L"FluentMedia_ControlPanel";
...
static double GetAvailableLyricWidth() {
    try {
        if (!g_playerGrid) return 0.0;
        auto panelFe = FindChildByName(g_playerGrid, kPanelGridName);
        if (!panelFe) return 0.0;
        ...
        if (total <= 0.0) return 0.0;
        auto cols = panelGrid.ColumnDefinitions();
        // 封面列：正常布局在列0，镜像布局在最后一列
        uint32_t artCol = g_settings.mirrorLayout ? (cols.Size() - 1) : 0;
        double used = 0.0;
        if (cols.Size() > artCol) used += cols.GetAt(artCol).ActualWidth();
        double available = total - used - 4.0;
        if (available < 0.0) available = 0.0;
        return available;
    } catch (...) {
        return 0.0;
...
static void UpdateScrollTransforms() {
    if (!g_playerGrid || (!g_settings.enableTitleScrolling && !g_settings.enableArtistScrolling && !g_settings.lyricsScroll)) return;
            ...
            if (auto fe = FindChildByName(g_playerGrid, kLyricScrollViewName)) {
                if (auto cv = fe.try_as<Canvas>()) {
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        if (auto tb = child.try_as<TextBlock>()) {
                            auto name = tb.Name();
                            if (name == kLyricBlockName) {
                                Canvas::SetLeft(tb, -g_lyricScroll.offset);
                            } else if (isLoop && name == kLyricCloneName) {
                                double gap = g_lyricScroll.textWidth + g_settings.loopGap;
                                Canvas::SetLeft(tb, gap - g_lyricScroll.offset);
                            }
                        }
                    }
                }
...
static void LxFindJsonString(const std::wstring& json, const std::wstring& key, std::wstring& out) {
    out.clear();
    ...
    for (; pos < json.size(); ++pos) {
        ...
        if (c == L'\\' && pos + 1 < json.size()) {
            ...
            wchar_t e = json[pos];
            switch (e) {
                case L'"': val.push_back(L'"'); break;
                case L'\\': val.push_back(L'\\'); break;
                case L'/': val.push_back(L'/'); break;
                case L'n': val.push_back(L'\n'); break;
                case L't': val.push_back(L'\t'); break;
                case L'r': val.push_back(L'\r'); break;
                case L'b': val.push_back(L'\b'); break;
                case L'f': val.push_back(L'\f'); break;
                case L'u': {
                        ...
                        if (ok) {
                            if (cp >= 0xD800 && cp <= 0xDBFF && pos + 10 < json.size() &&
                                ...
                                if (ok2 && lo >= 0xDC00 && lo <= 0xDFFF) {
                                    unsigned uc = 0x10000 + ((cp - 0xD800) << 10) + (lo - 0xDC00);
                                    val.push_back((wchar_t)(0xD800 + ((uc - 0x10000) >> 10)));
                                    val.push_back((wchar_t)(0xDC00 + ((uc - 0x10000) & 0x3FF)));
                                    pos += 10;
                                    consumedPair = true;
                                } else {
                                    val.push_back((wchar_t)cp);
                                }
                            } else {
                                val.push_back((wchar_t)cp);
                            }
...
static bool LxFindJsonNumber(const std::wstring& json, const std::wstring& key, double& out) {
    out = 0.0;
    std::wstring needle = L"\"" + key + L"\"";
    size_t pos = json.find(needle);
    ...
    while (pos < json.size()) {
        wchar_t c = json[pos];
        if ((c >= L'0' && c <= L'9') || c == L'.' || c == L'-' || c == L'e' || c == L'E' || c == L'+') {
            numStr.push_back(c);
            ++pos;
        } else {
            break;
    ...
    try { out = std::stod(numStr); return true; } catch (...) { return false; }
}
struct LrcLine {
    double time;
    std::wstring text;
};
static std::vector<LrcLine> ParseLrcLines(const std::wstring& lrcText) {
    std::vector<LrcLine> lines;
    size_t start = 0;
    while (start < lrcText.size()) {
        size_t end = lrcText.find(L'\n', start);
        std::wstring line = (end == std::wstring::npos)
            ? lrcText.substr(start) : lrcText.substr(start, end - start);
        start = (end == std::wstring::npos) ? lrcText.size() : end + 1;
        if (!line.empty() && line.back() == L'\r') line.pop_back();
        size_t pos = 0;
        std::vector<double> times;
        while (pos < line.size() && line[pos] == L'[') {
            size_t close = line.find(L']', pos);
            if (close == std::wstring::npos) break;
            std::wstring ts = line.substr(pos + 1, close - pos - 1);
            size_t colon = ts.find(L':');
            if (colon != std::wstring::npos) {
                try {
                    int min = std::stoi(ts.substr(0, colon));
                    double sec = std::stod(ts.substr(colon + 1));
                    times.push_back(min * 60.0 + sec);
                } catch (...) {}
            }
            pos = close + 1;
        }
        if (!times.empty()) {
            std::wstring text = line.substr(pos);
            size_t trimStart = text.find_first_not_of(L" \t");
            if (trimStart != std::wstring::npos) {
                text.erase(0, trimStart);
                size_t trimEnd = text.find_last_not_of(L" \t");
                if (trimEnd != std::wstring::npos) text.erase(trimEnd + 1);
            } else {
                text.clear();
            }
            if (!text.empty()) {
                for (double t : times) lines.push_back({t, text});
            }
        }
    }
...
static std::wstring FindNextLyricLine(const std::vector<LrcLine>& lines, double progress) {
    if (lines.empty()) return L"";
    int currentIdx = -1;
    for (int i = 0; i < (int)lines.size(); ++i) {
        if (lines[i].time <= progress + 0.05) currentIdx = i;
        else break;
    }
    if (currentIdx >= 0 && currentIdx + 1 < (int)lines.size()) {
        return lines[currentIdx + 1].text;
    }
    return L"";
}
static bool LxFetchStatus(std::wstring& lyricOut, std::wstring& statusOut,
                           std::wstring& fullLrcOut, double& progressOut) {
    lyricOut.clear();
    statusOut.clear();
    fullLrcOut.clear();
    progressOut = 0.0;
    HINTERNET hSession = WinHttpOpen(L"TaskbarFluentMediaPlayer-LX/1.0",
    ...
    if (hConnect) {
        HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET",
            L"/status?filter=status,lyricLineText,lyric,progress",
            ...
            if (WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
                ...
                if (WinHttpReceiveResponse(hRequest, nullptr)) {
                    ...
                    do {
                        ...
                        if (read > 0) body.append(buf, read);
                    } while (read > 0);
                    if (!body.empty()) {
                        std::wstring wbody = Utf8ToWide(body);
                        LxFindJsonString(wbody, L"status", statusOut);
                        LxFindJsonString(wbody, L"lyricLineText", lyricOut);
                        LxFindJsonString(wbody, L"lyric", fullLrcOut);
                        LxFindJsonNumber(wbody, L"progress", progressOut);
                        result = true;
                    }
                }
            }
...
static bool IsPureMusicLyric(const std::wstring& lyric) {
    ...
    return lyric.find(L"纯音乐") != std::wstring::npos;
}
static void LxPollThreadProc() {
    while (g_lxStopEvent && WaitForSingleObject(g_lxStopEvent, 1000) != WAIT_OBJECT_0) {
        std::wstring lyric, status, fullLrc;
        double progress = 0.0;
        std::wstring newLyric, newNextLyric;
        if (LxFetchStatus(lyric, status, fullLrc, progress)) {
            if ((status == L"playing" || status == L"paused") && !lyric.empty() && !IsPureMusicLyric(lyric)) {
                newLyric = lyric;
                // 从完整LRC中计算下一行歌词
                if (!fullLrc.empty()) {
                    auto lrcLines = ParseLrcLines(fullLrc);
                    newNextLyric = FindNextLyricLine(lrcLines, progress);
                }
            }
        }
        bool changed = false;
        {
            std::lock_guard<std::mutex> lk(g_mediaMtx);
            if (g_media.lyric != newLyric) {
                g_media.lyric = newLyric;
                changed = true;
            }
            if (g_media.nextLyric != newNextLyric) {
                g_media.nextLyric = newNextLyric;
                changed = true;
            }
        }
        if (changed) {
...
static Grid BuildPlayerGrid() {
    try {
        ...
        bool hasText = g_settings.showTrackTitle || g_settings.showTrackArtist || g_settings.lxEnabled;
        ...
        if (g_settings.lxEnabled) {
            RowDefinition rowTop, rowBottom;
            rowTop.Height({1.0, GridUnitType::Auto});
            rowBottom.Height({1.0, GridUnitType::Auto});
            panel.RowDefinitions().Append(rowTop);
            panel.RowDefinitions().Append(rowBottom);
        }
        ...
        if (g_settings.showAlbumArt) {
            ...
            if (g_settings.lxEnabled) {
                Grid::SetRowSpan(artContainer, 2);
            }
            panel.Children().Append(artContainer);
        }
        Canvas lyricScrollView{nullptr};
        TextBlock lyricBlock{nullptr};
        FrameworkElement lyricElement{nullptr};
        if (hasText) {
            ...
            if (g_settings.showTrackTitle || g_settings.showTrackArtist) {
                ...
                if (g_settings.lxEnabled) {
                    lyricBlock = TextBlock();
                    lyricBlock.Name(kLyricBlockName);
                    lyricBlock.FontSize((double)g_settings.lyricFontSize);
                    std::wstring lyricFontName = g_settings.lyricFont.empty()
                        ? (g_settings.artistFont.empty() ? g_settings.artistFontFamily : g_settings.artistFont)
                        : g_settings.lyricFont;
                    if (!lyricFontName.empty()) {
                        try { lyricBlock.FontFamily(Media::FontFamily(lyricFontName)); } catch (...) {}
                    }
                    if (!g_settings.artistFontWeight.empty()) {
                        try {
                            auto fontWeight = Markup::XamlBindingHelper::ConvertValue(
                                ...
                                winrt::box_value(g_settings.artistFontWeight))
                                .as<winrt::Windows::UI::Text::FontWeight>();
                            lyricBlock.FontWeight(fontWeight);
                        } catch (...) {}
                    }
                    if (!g_settings.artistFontStyle.empty()) {
                        try {
                            auto fontStyle = Markup::XamlBindingHelper::ConvertValue(
                                ...
                                winrt::box_value(g_settings.artistFontStyle))
                                .as<winrt::Windows::UI::Text::FontStyle>();
                            lyricBlock.FontStyle(fontStyle);
                        } catch (...) {}
                    }
                    if (g_settings.artistCharacterSpacing != 0) {
                        lyricBlock.CharacterSpacing(g_settings.artistCharacterSpacing);
                    }
                    lyricBlock.Foreground(MakeBrush(LyricColor()));
                    lyricBlock.TextWrapping(TextWrapping::NoWrap);
                    lyricBlock.TextTrimming(TextTrimming::CharacterEllipsis);
                    lyricBlock.TextAlignment(TextAlignment::Center);
                    lyricBlock.HorizontalAlignment(HorizontalAlignment::Stretch);
                    lyricBlock.Visibility(Visibility::Collapsed);
                    // 文字阴影（纯XAML双层TextBlock方案，仅非滚动模式生效）
                    TextBlock lyricShadowBlock;
                    bool useShadowGrid = g_settings.lyricShadow && !g_settings.lyricsScroll;
                    if (useShadowGrid) {
                        lyricShadowBlock.Name(kLyricShadowBlockName);
                        lyricShadowBlock.FontSize((double)g_settings.lyricFontSize);
                        lyricShadowBlock.Foreground(MakeBrush(winrt::Windows::UI::ColorHelper::FromArgb(140, 0, 0, 0)));
                        lyricShadowBlock.TextWrapping(TextWrapping::NoWrap);
                        lyricShadowBlock.TextTrimming(TextTrimming::CharacterEllipsis);
                        lyricShadowBlock.TextAlignment(TextAlignment::Center);
                        lyricShadowBlock.HorizontalAlignment(HorizontalAlignment::Stretch);
                        lyricShadowBlock.Visibility(Visibility::Collapsed);
                        lyricShadowBlock.Margin({1, 1, 0, 0});
                        if (!lyricFontName.empty()) {
                            try { lyricShadowBlock.FontFamily(Media::FontFamily(lyricFontName)); } catch (...) {}
                        }
                    }
                    // 第二行（演唱者，小号半透明）
                    TextBlock lyricSecondBlock;
                    lyricSecondBlock.Name(kLyricSecondBlockName);
                    lyricSecondBlock.FontSize((double)g_settings.lyricSecondLineSize);
                    lyricSecondBlock.Foreground(MakeBrush(LyricColor()));
                    lyricSecondBlock.Opacity(g_settings.lyricSecondLineOpacity / 100.0);
                    lyricSecondBlock.TextWrapping(TextWrapping::NoWrap);
                    lyricSecondBlock.TextTrimming(TextTrimming::CharacterEllipsis);
                    lyricSecondBlock.TextAlignment(TextAlignment::Center);
                    lyricSecondBlock.HorizontalAlignment(HorizontalAlignment::Stretch);
                    lyricSecondBlock.Visibility(Visibility::Collapsed);
                    if (!lyricFontName.empty()) {
                        try { lyricSecondBlock.FontFamily(Media::FontFamily(lyricFontName)); } catch (...) {}
                    }
                    if (g_settings.lyricsScroll) {
                        lyricScrollView = Canvas();
                        MakeScrollView(lyricScrollView, lyricBlock, kLyricScrollViewName, kLyricBlockName, kLyricCloneName);
                        lyricScrollView.HorizontalAlignment(HorizontalAlignment::Center);
                        lyricElement = lyricScrollView;
                    } else if (useShadowGrid) {
                        Grid lyricShadowGrid;
                        lyricShadowGrid.Children().Append(lyricShadowBlock);
                        lyricShadowGrid.Children().Append(lyricBlock);
                        lyricElement = lyricShadowGrid;
                    } else {
                        lyricElement = lyricBlock;
                    }
                    if (g_settings.lyricDualLine) {
                        StackPanel lyricStack;
                        lyricStack.Name(kLyricStackName);
                        lyricStack.Orientation(Orientation::Vertical);
                        lyricStack.Spacing(0);
                        lyricStack.Children().Append(lyricElement);
                        lyricStack.Children().Append(lyricSecondBlock);
                        lyricElement = lyricStack;
                    }
                }
            }
        ...
        if (lyricElement) {
            lyricElement.VerticalAlignment(VerticalAlignment::Bottom);
            lyricElement.Margin({2, (double)g_settings.lyricTopSpacing, 2, 3});
            Grid::SetRow(lyricElement, 1);
            // 歌词跨除封面列外的全部列：正常布局从列1起跨3列，镜像布局从列0起跨3列
            Grid::SetColumn(lyricElement, albumArtLeft ? 1 : 0);
            Grid::SetColumnSpan(lyricElement, 3);
            panel.Children().Append(lyricElement);
        }
        if (g_settings.showMediaButtons) {
            StackPanel ctrlPanel;
            ctrlPanel.Name(kControlPanelName);
        ...
        if (g_settings.vizEnabled) {
            try {
                ...
                if (vizEl) {
                    if (g_settings.vizPosition == L"overlay") {
                        // 重叠模式：不占额外列，跨文本+控件区域（排除封面列），置于文字后方
                        Grid::SetRow(vizEl, 0);
                        if (g_settings.lxEnabled) {
                            Grid::SetRowSpan(vizEl, 2);
                        }
                        Grid::SetColumn(vizEl, albumArtLeft ? 1 : 0);
                        Grid::SetColumnSpan(vizEl, 3);
                        Canvas::SetZIndex(vizEl, -1);
                        double opacity = (double)g_settings.vizOverlayOpacity / 100.0;
                        if (opacity < 0.0) opacity = 0.0;
                        if (opacity > 1.0) opacity = 1.0;
                        vizEl.Opacity(opacity);
                        vizEl.IsHitTestVisible(false);
                        vizEl.HorizontalAlignment(HorizontalAlignment::Stretch);
                        vizEl.VerticalAlignment(VerticalAlignment::Stretch);
                    } else {
...
static void RefreshPlayerContents() {
    if (!g_playerGrid || g_unloading || g_applyingSettings) return;
    std::wstring      title, artist, lyric, nextLyric;
    ...
    (void)hasMedia;
    bool lyricAvailable = g_settings.lxEnabled && !lyric.empty();
    bool hasSession = false;
    { std::lock_guard<std::mutex> lk(g_sessionMtx); hasSession = (g_currentSession != nullptr); }
    g_playerGrid.UpdateLayout();
    if (title != g_scrollCachedTitle || artist != g_scrollCachedArtist || lyric != g_scrollCachedLyric) {
        g_scrollCachedTitle  = title;
        g_scrollCachedArtist = artist;
        g_scrollCachedLyric  = lyric;
        ResetScrollState(g_titleScroll);
        ResetScrollState(g_artistScroll);
        ResetScrollState(g_lyricScroll);
        try {
        ...
        try {
            if (auto fe = FindChildByName(g_playerGrid, kLyricCloneName))
                if (auto cl = fe.try_as<TextBlock>())
                    cl.Visibility(Visibility::Collapsed);
        } catch (...) {}
        try {
            if (auto fe = FindChildByName(g_playerGrid, kLyricBlockName))
                if (auto lb = fe.try_as<TextBlock>())
                    Canvas::SetLeft(lb, 0.0);
        } catch (...) {}
    }
    ...
    if (auto fe = FindChildByName(g_playerGrid, kArtistBlockName))
        if (auto ab = fe.try_as<TextBlock>())
            try {
                ...
                if (lyricAvailable && g_settings.lyricsReplaceArtist) visible = false;
    ...
    if (auto fe = FindChildByName(g_playerGrid, kLyricBlockName))
        if (auto lb = fe.try_as<TextBlock>())
            try {
                std::wstring displayLyric = lyric;
                bool showArtistFallback = !lyricAvailable && g_settings.lyricNoLyricMode == L"show_artist";
                if (showArtistFallback) displayLyric = artist;
                lb.Text(winrt::hstring(displayLyric));
                lb.Foreground(MakeBrush(LyricColor()));
                lyricVisible = (lyricAvailable || showArtistFallback) && !displayLyric.empty();
                lb.Visibility(lyricVisible ? Visibility::Visible : Visibility::Collapsed);
                ToolTipService::SetToolTip(lb, nullptr);
                // 阴影层同步文字与可见性
                if (auto shadowFe = FindChildByName(g_playerGrid, kLyricShadowBlockName))
                    if (auto sb = shadowFe.try_as<TextBlock>()) {
                        sb.Text(winrt::hstring(displayLyric));
                        sb.Visibility(lb.Visibility());
                    }
                // 第二行：优先显示下一句歌词，无下一句时回退显示演唱者（小号半透明）
                if (auto fe2 = FindChildByName(g_playerGrid, kLyricSecondBlockName))
                    if (auto lb2 = fe2.try_as<TextBlock>()) {
                        std::wstring secondText = !nextLyric.empty() ? nextLyric : artist;
                        bool secondVisible = g_settings.lyricDualLine && lyricAvailable && !secondText.empty();
                        lb2.Text(winrt::hstring(secondText));
                        lb2.Foreground(MakeBrush(LyricColor()));
                        lb2.Visibility(secondVisible ? Visibility::Visible : Visibility::Collapsed);
                    }
                // 双行 StackPanel 整体可见性控制（collapse 模式无歌词时完全隐藏）
                if (auto stackFe = FindChildByName(g_playerGrid, kLyricStackName)) {
                    stackFe.Visibility(lyricVisible ? Visibility::Visible : Visibility::Collapsed);
                }
                if (g_settings.lyricsScroll && lyricVisible) {
                    try {
                        if (auto viewFe = FindChildByName(g_playerGrid, kLyricScrollViewName))
                            viewFe.Visibility(Visibility::Visible);
                    } catch (...) {}
                    if (auto panelFe = FindChildByName(g_playerGrid, kPanelGridName)) {
                        panelFe.UpdateLayout();
                    ...
                    lb.UpdateLayout();
                    double textW = lb.DesiredSize().Width;
                    if (auto viewFe = FindChildByName(g_playerGrid, kLyricScrollViewName)) {
                        if (auto viewCanvas = viewFe.try_as<Canvas>()) {
                            double viewW = textW;
                            double availW = GetAvailableLyricWidth();
                            if (availW > 0.0 && viewW > availW) viewW = availW;
                            if (std::abs(viewCanvas.Width() - viewW) > 0.5) {
                                viewCanvas.Width(viewW);
                                try {
                                    if (auto geo = viewCanvas.Clip().try_as<winrt::Windows::UI::Xaml::Media::RectangleGeometry>()) {
                            ...
                            bool wasActive = g_lyricScroll.active;
                            g_lyricScroll.textWidth = textW;
                            g_lyricScroll.viewWidth = viewW;
                            g_lyricScroll.active = (textW > viewW + 2.0);
                            if (!g_lyricScroll.active) {
                                g_lyricScroll.offset = 0.0;
                                g_lyricScroll.forward = true;
                                Canvas::SetLeft(lb, 0.0);
                            } else if (!wasActive) {
                                g_lyricScroll.offset = 0.0;
                                g_lyricScroll.forward = true;
                                if (g_settings.scrollMode == L"loop") {
                                    g_lyricScroll.pausing  = false;
                                    g_lyricScroll.pauseTick = 0;
                                } else {
                                    g_lyricScroll.pausing  = true;
                                    g_lyricScroll.pauseTick = g_settings.scrollPauseDuration;
                                }
                            }
                            if (auto cloneFe = FindChildByName(g_playerGrid, kLyricCloneName)) {
                                if (auto clone = cloneFe.try_as<TextBlock>()) {
                                    clone.Text(lb.Text());
                                    clone.Foreground(lb.Foreground());
                                    clone.Visibility(g_settings.scrollMode == L"loop" && g_lyricScroll.active
                                        ? Visibility::Visible : Visibility::Collapsed);
                                }
                            }
                        }
    ...
    if ((g_titleScroll.active || g_artistScroll.active || g_lyricScroll.active) &&
        (g_settings.enableTitleScrolling || g_settings.enableArtistScrolling || g_settings.lyricsScroll)) {
    ...
    double shift = (lyricVisible && g_settings.lyricsControlsUpOffset > 0)
        ? (double)g_settings.lyricsControlsUpOffset : 0.0;
    if (std::abs(shift - g_lyricsCtrlShift) > 0.1) {
        g_lyricsCtrlShift = shift;
        try {
            auto tt = Media::TranslateTransform();
            tt.Y(-shift);
            if (auto fe = FindChildByName(g_playerGrid, kControlPanelName))
                if (auto sp = fe.try_as<StackPanel>()) sp.RenderTransform(tt);
            if (auto fe2 = FindChildByName(g_playerGrid, kTextStackName))
                fe2.RenderTransform(tt);
        } catch (...) {}
    }
