...
UINT msgRainbowTimer = RegisterWindowMessage(L"Rainbow_effect");
...
std::mutex rainbowWindowsMutex;
std::unordered_map<HWND, std::pair<UINT_PTR, float>> g_rainbowWindows;

std::mutex g_drawTextMutex;
std::unordered_map<DWORD, bool> g_drawTextState;
...
using NtUserDestroyWindow_t = BOOL(WINAPI*)(HWND hWnd);
NtUserDestroyWindow_t NtUserDestroyWindow_Original;
...
HRESULT WINAPI HookedDwmSetWindowAttribute(HWND hWnd, DWORD dwAttribute, LPCVOID pvAttribute, DWORD cbAttribute)
{
    if(g_settings.BorderFlag && dwAttribute == DWMWA_BORDER_COLOR && g_settings.MenuBorderFlag &&  IsWindowClass(hWnd, L"#32768"))
            return DwmSetWindowAttribute_orig(hWnd, DWMWA_BORDER_COLOR, &g_settings.BorderActiveColor, sizeof(g_settings.BorderActiveColor));
    ...
    if(!g_settings.TitlebarFlag)
    {
        ...
        else if(dwAttribute == DWMWA_SYSTEMBACKDROP_TYPE)
        {
            if(g_settings.BgType == g_settings.AcrylicSystemBackdrop)
                return DwmSetWindowAttribute_orig(hWnd, DWMWA_SYSTEMBACKDROP_TYPE, &TRANSIENTWINDOW, sizeof(TRANSIENTWINDOW));
            else if(g_settings.BgType == g_settings.MicaAlt)
                return DwmSetWindowAttribute_orig(hWnd, DWMWA_SYSTEMBACKDROP_TYPE, &TABBEDWINDOW, sizeof(TABBEDWINDOW));
            else if(g_settings.BgType == g_settings.Mica)
                return DwmSetWindowAttribute_orig(hWnd, DWMWA_SYSTEMBACKDROP_TYPE, &MAINWINDOW, sizeof(MAINWINDOW));
        }
    }
            ...
            return DwmSetWindowAttribute_orig(hWnd, DWMWA_CAPTION_COLOR, &g_settings.g_TitlebarColor, sizeof(g_settings.g_TitlebarColor));
    ...
    if((g_settings.BorderFlag || g_settings.BorderRainbowFlag) && dwAttribute == DWMWA_BORDER_COLOR)
        return DwmSetWindowAttribute_orig(hWnd, DWMWA_BORDER_COLOR, &g_settings.g_BorderColor, sizeof(g_settings.g_BorderColor));
...
BOOL WINAPI HookedNtUserDestroyWindow(HWND hWnd)
{
    {
        // Unlike WM_DESTROY it captures all hwnd created from NtUserCreatWindowExW
        std::lock_guard<std::mutex> guard(rainbowWindowsMutex);
        if (g_rainbowWindows.contains(hWnd))
        {
            KillTimer(NULL, g_rainbowWindows[hWnd].first);
            g_rainbowWindows.erase(hWnd);
        }
    }
    return NtUserDestroyWindow_Original(hWnd);
}
...
bool IsUxThemeTextExThreadDrawing()
{
    std::lock_guard<std::mutex> lock(g_drawTextMutex);
    auto it = g_drawTextState.find(GetCurrentThreadId());
    return it != g_drawTextState.end() && it->second;
}

void SetCurrentThreadDrawing(bool state)
{
    std::lock_guard<std::mutex> lock(g_drawTextMutex);
    if (state)
        g_drawTextState[GetCurrentThreadId()] = true;
    else
        g_drawTextState.erase(GetCurrentThreadId());
}
...
{
    if (format & DT_CALCRECT || IsUxThemeTextExThreadDrawing())
...
int WINAPI HookedDrawTextExW(HDC hdc, LPWSTR lpchText, int cchText, LPRECT lprc, UINT format, LPDRAWTEXTPARAMS lpdtp) 
{
    ...
    if (!lpdtp && !(format & DT_CALCRECT) && !isCurThread && IsUxThemeTextExThreadDrawing()) 
    {
        isCurThread = true;
...
BOOL WINAPI HookedExtTextOutW(HDC hdc, int x, int y, UINT option, const RECT* lprect, LPCWSTR lpString, UINT c, const INT* lpDx)
{
    ...
    if (!(option & ETO_IGNORELANGUAGE) && !(option & ETO_GLYPH_INDEX) && 
        !isCurThread && !str.empty() && IsUxThemeTextExThreadDrawing())
    {
        isCurThread = true;
        RECT rect = { 0 };
        if (lprect)
...
{
    DwmSetWindowAttribute(hWnd, DWMWA_SYSTEMBACKDROP_TYPE , &TRANSIENTWINDOW, sizeof(TRANSIENTWINDOW));
}

void EnableMica(HWND hWnd)
{
    DwmSetWindowAttribute(hWnd, DWMWA_SYSTEMBACKDROP_TYPE , &MAINWINDOW, sizeof(MAINWINDOW));
}

void EnableMicaTabbed(HWND hWnd)
{
    DwmSetWindowAttribute(hWnd, DWMWA_SYSTEMBACKDROP_TYPE , &TABBEDWINDOW, sizeof(TABBEDWINDOW));
}

void EnableColoredTitlebar(HWND hWnd)
{
    g_settings.g_TitlebarColor = g_settings.TitlebarActiveColor;
    DwmSetWindowAttribute(hWnd, DWMWA_CAPTION_COLOR, &g_settings.g_TitlebarColor, sizeof(g_settings.g_TitlebarColor));
}

void EnableCaptionTextColor(HWND hWnd)
{
    g_settings.g_CaptionColor = g_settings.CaptionActiveTextColor;
    DwmSetWindowAttribute(hWnd, DWMWA_TEXT_COLOR, &g_settings.g_CaptionColor, sizeof(g_settings.g_CaptionColor));
}

void EnableColoredBorder(HWND hWnd)
{
    g_settings.g_BorderColor = g_settings.BorderActiveColor;
    DwmSetWindowAttribute(hWnd, DWMWA_BORDER_COLOR, &g_settings.g_BorderColor, sizeof(g_settings.g_BorderColor));
}
...
void CALLBACK MyRainbowTimerProc(HWND hWnd, UINT, UINT_PTR t_id, DWORD)
{
    ...
    {
        std::lock_guard<std::mutex> guard(rainbowWindowsMutex);
        for(auto& [hWnd, pair] : g_rainbowWindows)
        {
            if(pair.first == t_id)
            {
                auto& hue = pair.second;
                hue = fmod(hue + g_settings.RainbowSpeed, 360.0f);
                if (g_settings.TitlebarRainbowFlag)
                {
                    g_settings.g_TitlebarColor = HSLToRGB(fmod(hue + 240.0f, 360.0f), 1.0f, 0.5f);
                ...
                {
                    if (g_settings.TitlebarRainbowFlag)
                        g_settings.g_CaptionColor = HSLToRGB(fmod(hue + 120.0f, 360.0f), 1.0f, 0.5f);
                    else
                        g_settings.g_CaptionColor = HSLToRGB(hue, 1.0f, 0.5f);
                    DwmSetWindowAttribute(hWnd, DWMWA_TEXT_COLOR, &g_settings.g_CaptionColor, sizeof(COLORREF));
                }
                if (g_settings.BorderRainbowFlag)
                {
                    g_settings.g_BorderColor = HSLToRGB(hue, 1.0f, 0.5f);  
        ...
        case WM_ACTIVATE:
        {
            ...
            if (!isMinimized)
            {
                ...
                if ((activationState == WA_ACTIVE || activationState == WA_CLICKACTIVE))
                {
                    ...
                    {
                        g_settings.g_BorderColor = g_settings.BorderActiveColor;
                        DwmSetWindowAttribute(hWnd, DWMWA_BORDER_COLOR, &g_settings.BorderActiveColor, sizeof(g_settings.BorderActiveColor));
                    }
                }
                else if (activationState == WA_INACTIVE)
                {
                    ...
                    if((g_settings.BorderFlag && !g_settings.BorderRainbowFlag))
                    {
                        ...
                        DwmSetWindowAttribute(hWnd, DWMWA_BORDER_COLOR, &g_settings.BorderInactiveColor, sizeof(g_settings.BorderInactiveColor));
                    }
                }
            }
            break;
        ...
        default:
        {
            if (uMsg == msgRainbowTimer)
            {
                switch (wParam)
                {
                    ...
                    {
                        std::lock_guard<std::mutex> guard(rainbowWindowsMutex);
                        auto it = g_rainbowWindows.find(hWnd);
                        if (it == g_rainbowWindows.end())
                        {
                            UINT_PTR timersId = SetTimer(NULL, NULL, 32, MyRainbowTimerProc);
                            if (timersId)
                            {
                                g_rainbowWindows[hWnd].first = timersId;
                                g_rainbowWindows[hWnd].second = 0.0f;
                    ...
                    case RAINBOW_UNLOAD:
                    {
                        ...
                        if (it != g_rainbowWindows.end())
                        {
                            if (KillTimer(NULL, g_rainbowWindows[hWnd].first))
                            {
                                g_rainbowWindows.erase(it);
                            }
                        }
                        break;
                    }
...
void NewWindowShown(HWND hWnd) 
{
    ...
    if((g_settings.BorderFlag) || g_settings.CaptionTextFlag || g_settings.TitlebarFlag)
        if(WindhawkUtils::SetWindowSubclassFromAnyThread(hWnd, SubclassProc, 0))
        {
            ...
            SendMessage(hWnd, msgRainbowTimer, RAINBOW_LOAD, 0);
        }
}
...
void TextRenderingHook()
{
    Wh_SetFunctionHook(
        ...
        (void*)HookedDrawTextW,
        (void**)&DrawTextW_orig);

    Wh_SetFunctionHook(
        (void*)GetProcAddress(GetModuleHandle(L"user32.dll"), "DrawTextExW"),
        (void*)HookedDrawTextExW,
        (void**)&DrawTextExW_orig);
    
    Wh_SetFunctionHook(
        (void*)GetProcAddress(GetModuleHandle(L"gdi32.dll"), "ExtTextOutW"),
        (void*)HookedExtTextOutW,
        (void**)&ExtTextOutW_orig);
    
    Wh_SetFunctionHook(
        (void*)GetProcAddress(GetModuleHandle(L"uxtheme.dll"), "DrawThemeText"),
        (void*)HookedDrawThemeText,
        (void**)&DrawThemeText_orig);

    Wh_SetFunctionHook(
        (void*)GetProcAddress(GetModuleHandle(L"uxtheme.dll"), "DrawThemeTextEx"),
        (void*)HookedDrawThemeTextEx,
...
void RestoreWindowCustomizations(HWND hWnd)
{
    ...
    DwmSetWindowAttribute(hWnd, DWMWA_CAPTION_COLOR , &g_settings.TitlebarActiveColor, sizeof(g_settings.TitlebarActiveColor));

    g_settings.CaptionActiveTextColor = DWMWA_COLOR_DEFAULT;
    DwmSetWindowAttribute(hWnd, DWMWA_TEXT_COLOR , &g_settings.CaptionActiveTextColor, sizeof(g_settings.CaptionActiveTextColor));
...
BOOL Wh_ModInit(void) 
{
    ...
    NtUserDestroyWindow_t pNtUserDestroyWindow =
        (NtUserDestroyWindow_t)GetProcAddress(hModule, "NtUserDestroyWindow");
    if (!pNtUserDestroyWindow)
        return FALSE;

    Wh_SetFunctionHook((void*)pNtUserDestroyWindow,
                       (void*)HookedNtUserDestroyWindow,
                       (void**)&NtUserDestroyWindow_Original);
...
{
    {
        std::lock_guard<std::mutex> guard(rainbowWindowsMutex);
        for (auto& [hWnd, pair] : g_rainbowWindows)
            PostMessageW(hWnd, msgRainbowTimer, RAINBOW_UNLOAD, 0);
    }
}
