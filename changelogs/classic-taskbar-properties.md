## 3.2.0 ([Jul 7, 2026](https://github.com/ramensoftware/windhawk-mods/blob/2ca19110ad59c57cadefbfbf5f82d5a8c4732299/mods/classic-taskbar-properties.wh.cpp))

- Window should now use native system colors
- Removed hard-coded color values
- Added toolbar warning message when enabling classic toolbars
- Fixed Window colors
- Partially implemented toolbars (Address and Links)
- Added taskbar rotation (only bottom and top)
- Code cleanup

## 2.0.2 ([Jun 15, 2026](https://github.com/ramensoftware/windhawk-mods/blob/29bacda66445835d5b79e6657d124ca107540575/mods/classic-taskbar-properties.wh.cpp))

Initial release.
