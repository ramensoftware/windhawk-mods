...
- **Right DLL for your system**: Uses the Windows 8 DLL on Windows 8.1/10/11
...
- **Conservative resource handling**: No synthetic resource handles and no patching of the loaded system module. A private per-process copy is updated transactionally before being loaded as data
...
- **Windows 7 page on modern systems**: The Windows 7 version of the page is not compatible with Windows 8.1/10/11 (I"cannot load page"), so the Windows 8 DLL is always used there
...
- language: auto
  $name: Page language
  $description: This setting allows the user to select the language used by the restored page and its Control Panel tooltip. Automatic follows the current Windows UI language.
  $options:
...
- forceTranslations: true
  $name: This setting forces the 10-language translation inside the page
...
- keepFilesOnDisable: false
  $name: This setting keeps the downloaded files when the mod is disabled
